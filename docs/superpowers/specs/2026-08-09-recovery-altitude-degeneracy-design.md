...
基準 commit `2897d19`（未修改）；修改後 commit `986db9d`。兩組數字都是在同一
...
| `ai-command-channel` 的 `groundUnderOrder` | 0（與 `986db9d` 逐字相同） |

那 3 條紅是：`perf-gate`（全套並跑時 104 個檔案搶 CPU，**單獨跑 4/4 綠** ——
專案慣例本來就要求它單獨跑），加上 §13.7 的兩條既有紅。

（`986db9d` 那一版的對應數字是 2,298 / 2,302 與矩陣 301 / 301；差額 74 條
...
**第一批（對 `986db9d`）** —— 半正矢退掉之後重跑的六個（見 plan 的
「執行中的偏離」）。第 6 條當時守的是 `q > 0 ? … : Infinity`，那個設計在
`08a2660` 被 §13.8 的 D1 推翻，所以它連同測試一起被第 7 條取代：
...
**第二批（對 `08a2660`，也就是現行版本）** —— D1 修正之後的分界條件：
...
八個全部轉紅，且每一個都只打到它該守的那幾條。現行版本（`08a2660`）由
第 1–5 條與第 7–8 條共同守著。
...
### 13.8 Codex 對 `986db9d` 的審查（三條 Important，全部處理）

**D1 —— 真缺陷，我的推理錯了。** 見 §6 的「那裡的答案是 0，不是 `Infinity`」。
修正在 `08a2660`，並補一條測試把 `−1.5×10⁻⁸ / −10⁻⁸` 兩側的連續性釘住。
