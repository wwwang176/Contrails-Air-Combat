...
## 五、合併前把關的追加裁定（2026-08-25，A 案）

合併前全套回歸揪出兩條**新**紅，消融歸因都是 `CHASE_FLOOR_BUFFER` 300
（buffer 退回 100 即全綠；掃 150/200/250 沒有中間值能四格全過）：

- `ai-defence` 後上方 400 m：defend 佔時 42%→92% 而被掛著射擊解 56%→94%、
  掉血 0→1000 —— 攻擊者不再被自己的高度閂鎖從獵物尾巴上扯下來，舊綠有
  一半是那個人工中斷撿來的。**裁定 A**：保持 300，該格標
  `pursuitOverwhelmed` 停用射擊解佔比與掉血預算兩條斷言（撐大門檻到
  「死了也算過」比停用更糟），場景成立性照驗；「持續追擊世界的破防品質」
  立案為 task #335。
- `ai-visible-evasion` underFire 場景門檻 300→150：被瞄的 AI 閃得更徹底
  （實測 233），這條守的是「場景成立」不是「閃躲不夠好」。

附帶收穫：`ai-duel-matrix` L4-C 由紅轉綠。最終基準 **13 failed / 2943
passed**（perf-gate 三條全套並行必炸、單獨跑全過，不計）。
