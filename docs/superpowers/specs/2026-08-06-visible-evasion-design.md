...
> **實作後否決（2026-08-06）。** 重掃完了，`defendFloor` 三個值全數觸發
> §6.3 事前約定的否決條件。12 幾何 × 300 秒、VETERAN、警戒已上線：
>
> ```
...
> ```
>
> 選邊救不了它：三個值的被鎖定都更高、最低高度差 16~49 倍、觸地 5~9 場
> 對 1 場。而且**它已經沒有存在的理由** —— §5.1 的主判準在只做 3.1 之後
> 就達標了（涵蓋率 93~95%、位移 7.3~8.8°）。完整紀錄在 `steer.ts` 的
> `DEFAULT_STEER` 註解。
>
> 重開的前提改成：**先讓 `defend` 這條路徑有能量／高度管理**。在那之前，
> 任何更花俏的破防都只是把同一個洞挖得更深。
...
> **實作完成（2026-08-06）。** 警戒上線之後窗口變大了：`defend` 由 5% 升到
> 29~36%，第三個條件（「我正在破防」）不再是瓶頸。四個高接近率幾何合計
> **19 次 / 180 秒**（先前恆為 0）。參數掃描顯示品質（反轉期間拿到射擊解的
> 比例）在 5.7~10.2% 之間平坦、沒有最佳點，次數則單純隨判定放寬而增加 ——
> 真正的決定因素是「要花多少比例的時間**不閃躲**」，那是手感問題。取起始值
> 500 m / 90° / 2 s，掃描表寫在 `DEFAULT_STEER.reversalRange` 的註解，
> **待人工試飛定案**。
...
> **實作後結果（2026-08-06）。全部達標，而且只靠 §3.1 的觸發修正。**
>
> ```
>            挨打取樣  觸發涵蓋率  defend 中位  平常中位
> 修補前        2993      0.0%        —         3.0°
> 700 m        2398     93.5%       8.8°        5.1°
> 900 m        1157     94.7%       7.3°        3.8°
> ```
>
> §3.2 的異平面破防因此**不需要**，實測也證實它有害（見該節）。§3.3 的
> 反轉是額外的遊戲性，不是達標的必要條件。
