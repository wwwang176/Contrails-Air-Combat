...
> **實作前否決（2026-08-06）。** 這一次先量前提再決定要不要寫，結果是不寫。
> 受控場景（三架前後咬、紅 B 起始 TAS 280、六種被咬幾何、120 秒、判定
> 500 m / 90°）：
>
> ```
...
> ```
>
> 1. **目標請求那一半不需要**：目標 100% 已經是紅 B，`urgent` 解決的是一個
>    不存在的問題（記在 `src/ai/target.ts`）。
> 2. **窗口只佔 1.6% 的取樣**，其中 27% 本來就有射擊解；真正錯過的是
>    `extend`(25%) 與 `defend`(19%) 那些，也就是全場約 0.7% 的時間。
> 3. **延遲讓窗口更小**（660 → 236）：延遲的攻擊者追得比較鬆，接近率下降。
>
> 放寬判定不是出路——1200 m / 150° 時同速對照組（結構上不可能衝過頭）的
> 「衝過頭」比高接近率組還高（50.4% 對 30.4%）。
>
> 重試的前提：這一份量的是 AI 對 AI。真人玩家衝過頭的頻率可能高一個量級，
> 人工試飛若看到「玩家衝過頭而 AI 沒反應」就重開，而且該量玩家不是 AI。
> 完整記錄在 `src/ai/steer.ts` 的 `DEFAULT_STEER` 註解。
