...
**範圍**：意圖層 `arbitrate` 的一處分支、戰術層的名額、入場條件、期限去向
與暫停條件。
**不動**：`steer.ts` 的操舵、空層鎖、指揮層、編隊、機種 spec、
`turnEnter`／`turnExit`／`extendRange`／`enterRange` 的數值。
...
**只徵召 `role === 'fighter'`。** boom and zoom 是戰鬥機的打法。轟炸機轉不贏
攔截機是常態 —— B-17G 對 P-51D 的機體迴旋差值是 −0.045～−0.074，遠低於
`turnEnter` —— 但它的答案是編隊與防禦火網，不是爬升俯衝。

少了這個條件，整隊轟炸機會離開航線去蓄能。實測 `turrets.test.ts` 的
「P-51 也打下了東西 —— 砲塔不是無敵的」由正數變成 **0**：轟炸機不在攔截機
預期的位置上。那條測試在講砲塔的平衡，卻被一個 AI 打法的改動弄紅，成因
完全看不出來 —— 所以護欄裡另立一條「轟炸機不徵召」。
...
`settled` 的生命週期有**四處**，少任何一處會跨輪、跨目標或跨場殘留：

| 位置 | 動作 | 少了會怎樣 |
|---|---|---|
| `createTacticalState` | `false` | 型別缺欄位，編譯期就擋住 |
| `resetTacticalState` | `false` | 跨場殘留 —— 第二場開局就有飛機免除門檻 |
| `openCycle` | 清除 | 第一次撞期限之後每一輪都免除，等於門檻不存在 |
| `clearRelative` | 清除 | **換目標後仍沿用舊目標的免除** |

最後一處最容易漏，而且症狀不會報錯：`settled` 免除的門檻 `perchEnter` 拿
`energyRatio` 跟**當前目標**比。在 `perch` 中換到一個能量關係完全不同的
新目標，舊的免除會讓它跳過新目標的 `perchLatch`，最壞的情況是沿用既有的
`perch.dwell` 很快就俯衝 —— 帶著對新目標毫無優勢的能量。
...
`AiController` 餵給 `stepTactics` 的 `suspended`：
...
`defend` 與速度見底會讓意圖覆寫與 `tacticalCommand` 兩處都讓位，但相位機
原本照常前進。於是飛機可以在做防禦機動的同時把 `build.dwell` 累積到期限、
拿到 `settled` —— 而那個「本輪蓄能已盡力」是假的，接著它會帶著沒蓄到的
能量去俯衝。

實測（20v20、300 s）：不算暫停時 `build` 佔時 19%，算了之後 9%。那十個
百分點是掛在一個它根本沒在執行的相位裡空轉。

**下游指標落在雜訊裡。** 四格 A/B（`buildMax` 13/20 × 暫停 是/否）沒有一致
的排序：`buildMax = 13` 時暫停讓 `engage` 由 0.5% 升到 1.8%，`buildMax = 20`
時反而由 2.1% 降到 1.0%。單一種子單一情境，這種量級不足以當證據。採用的
理由是**語意正確**（`settled` 的前提），不是那些數字。唯一穩定的是 `build`
佔時腰斬，而那是機制而非運氣。
...
**這份掃描的前提後來被變更 F 改掉了。** 掃描時 `build` 每一段都跑滿期限
（停留中位 13.1 s、蓄滿率 0%）；加上「被覆寫時暫停相位機」之後，大多數
段落改為被 `defend` 打斷 —— 實測停留中位 **2.1 s**、蓄滿率 **24.5%**
（`build-gap.probe.ts`，f4f4 vs a6m5 20v20）。

也就是說 `buildMax` 現在**很少咬得到**，它回到了原本被設計的角色（止損）
而不是節奏。20 這個值目前的實際影響遠小於掃描時。日後要重調，要從這個
前提出發重掃，不要沿用上面那張表。

### 4.9 `perchEnter` 維持 0.5
...
`buildMax` 由 13 放寬到 20（§4.8）是配套 —— 讓「盡力」有合理的份量，而
...
| G5 | `f4f4 vs f4f4` 的**戰術層一次都不啟動** | 未啟動 | 未啟動 | `dive` 與 `build` 佔時皆為 0 |
| G7 | `b17g vs p51d` 的**轟炸機不徵召** | 未啟動 | 未啟動 | 同上 |
| G6 | `build` 佔時上限（防「無腦爬升」） | 0.0% | 17.7% | < 30% |

**G4 直接用公尺，不用 `energyRatio`。** 那是負責人給的判準單位（300～600 m），
也是唯一不需要換算就能讀懂的一欄。

**每一條各自一個 `it`。** 五條寫在同一個 `it` 裡的話，第一條失敗之後其餘
**根本不會執行** —— 於是「每一條都先驗過紅」是假的，只有第一條驗過。量測
本身放 `beforeAll` 共用（一次 20v20 300 秒要 20 秒）。

**G6 是改動後的回歸護欄，不是先驗紅的。** 改動前 `build` 佔時必然是 0.0%
（戰術層根本沒啟動），所以它在改動前一定通過。它擋的是改動**之後**才可能
出現的失效模式：飛機把時間全花在爬升，一次都不攻擊。它殺得死 —— 把 `build`
撞期限的去向改回 `cooldown`，循環會退化成「爬完逃、逃完爬」。

**G5 斷言的是機制惰性，不是數字相等。** 同機種的 `airframeTurnAdvantage`
恆為 0、徵召天然不成立、`quota` 出貨值是 0，所以戰術層一次都不啟動，相位
從未離開 `off`，下游就沒有任何東西會不同。不比對意圖分布的百分比 —— 那要
把單次量測寫死成常數，而「單次量測當門檻是變更偵測器不是設計判準」
（`ai-withdraw-anchor.test.ts` 的註解逐字警告過）。

**G5 與 G7 是變異測試的錨，而且已經抓過一次真錯**：掃描第一版把變更 C 寫成
「`quota = 1` 徵召所有人」，對照組的 `engage` 由 12.1% 掉到 0.0% 立刻報警。
主題那一組當時看起來是變好的（全隊存活），只有對照組看得出改法漏了出去。

**變異測試的結果**：把徵召條件改成恆真（`drafted = true`），G5 與 G7 兩條
都紅（同機種進入攻擊相位 28 次、轟炸機蓄能 40.2 s）。它們不是恆真的護欄。
...
4. **`order-of-battle-replay` 的 digest 會變 —— 已處理。** 只有
   `HEADON_20V20`（P-51D vs Bf 109 K-4，被徵召）改變，已用
   `rebaseline.probe.ts` 雙跑確認穩定後重錄。`ESCORT_B17`（轟炸機不徵召）
   與 `PURSUIT_MIRROR_8V8`（鏡像）維持原值。`tactics-baseline` 的
   `_BEFORE_TACTICS` 是歷史常數，不動。
