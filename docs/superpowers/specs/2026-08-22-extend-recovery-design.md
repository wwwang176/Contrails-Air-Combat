...
| | `axis-escort` | `allies-escort` |
|---|---|---|
| 段落總數 | 407 | 599 |
| 其中純能量型 | 168（41%） | 254（42%） |
...
**距離與射擊解合計只佔 15~21%，不是主要出口。§4 不是空轉。**

純能量型段落的長度，與 §4 的反事實（`min(實際長度, cornerRatio 首次 ≥
0.95 的時刻)`）：

| | p50 | p75 | p90 | max |
|---|---|---|---|---|
| `axis-escort` 實際 | 3.0 s | 7.7 s | 21.3 s | **99.8 s** |
| `axis-escort` 反事實 | 0.0 s | 0.6 s | **3.2 s** | 21.0 s |
| `allies-escort` 實際 | 3.2 s | 8.9 s | 27.5 s | **209.3 s** |
| `allies-escort` 反事實 | 0.0 s | 0.0 s | **3.2 s** | 41.0 s |

**§4 會縮短 83% 與 85% 的純能量型段落。**
...
`cornerRatio` 就已經 ≥ 0.95** —— §4 的合取在那一拍就是 false，所以它**不只
是縮短，它會直接擋掉這些進場**。
