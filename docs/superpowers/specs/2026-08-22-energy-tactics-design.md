...
## 14. 實作期間量到的東西（2026-08-22 回填）

### 14.1 `build` 的離場原本是無界的 —— 消融表抓到的缺陷

第一版的 `build` 一律 `flat.negate()`（朝目標的反方向飛），離場因此**只有
能量出口、沒有距離出口**。消融表（`test/tools/tactics-ablation.probe.ts`）：

```
             射擊解          與目標距離
掃蕩卡
  quota 0     5.5%             734 m
  quota 0.5   0.0%          12 301 m
遭遇戰
  quota 0     9.2%             705 m
  quota 1     0.0%          11 254 m
```

**它會傳染。** 實測全場只要 3 架進過 `build` 就足以造成上面那個崩潰 ——
跑掉的人會把鎖定它們的人一起帶出去。診斷時的關鍵一步是量「曾經離開 `off`
的架數」：`quota` 0.5 與 1 的結果**逐位元相同**，因為兩檔下都是同樣那 3 架
過得了 `farLatch`（>2500 m）那道閘，其他人根本沒機會啟動。

**修法不是加一個距離門檻**（那會在門檻上翻號，是同一類錯誤的另一版），而是
讓 `build` 直接共用 `perch` 已經在用的連續徑向律（`stationKeeping`）：
...
**建能是爬升，不是拉開距離**；水平方向該做的事本來就與 `perch` 一樣。兩者
現在只差一個爬升角。

### 14.2 雜訊帶：`seed` 不擾動任何東西

`createBattle` 的 `seed` **只餵飛行員名字** —— 出生幾何、速度、決策相位全都
與它無關。五個種子跑出來的數字因此逐位元相同，雜訊帶是 **0.000**，而「退步
不得超過雜訊」這個判準就永遠成立不了。

消融探針因此自己加一層擾動：把每一架的初速乘 `1 ± 0.5%`，擾動量由
`(seed, index)` 的整數雜湊決定（不得用 `Math.random`）。擾動初速而不是位置，
因為位置一動就等於換了一張卡。

### 14.3 消融表（七卡 × 三檔 × 五種子，150 s，中位）

`quota = 0 → 0.5 → 1`，單位 %：

| 卡 | engage | 射擊解 | 對轟 engage | 對轟射擊解 |
|---|---|---|---|---|
| 遭遇戰 | 9.4 → 13.9 → 10.1 | 8.9 → 9.9 → 7.4 | — | — |
| allies-sweep | 7.8 → 9.1 → 9.1 | 8.0 → 1.7 → 1.7 | — | — |
| allies-intercept | 7.9 → 8.8 → 12.9 | 13.5 → 8.2 → 4.9 | 13.7 → 24.5 → 52.3 | 40.0 → 32.7 → 14.4 |
| allies-escort | 30.3 → 32.0 → 30.7 | 22.6 → 22.5 → 20.1 | 20.9 → 38.5 → 38.6 | 16.5 → 17.8 → 15.6 |
| allies-evac | 11.2 → 11.2 → 11.2 | 9.6 → 9.6 → 9.6 | — | — |
| axis-intercept | 9.9 → 13.1 → 8.7 | 10.9 → 8.7 → 10.4 | 31.1 → 33.6 → 46.1 | 6.7 → 12.4 → 36.6 |
| axis-escort | 11.6 → 31.2 → 31.2 | 9.7 → 26.7 → 26.7 | 36.6 → 40.2 → 40.2 | 33.4 → 18.8 → 18.8 |

雜訊帶（`quota = 0` 的五種子全距，百分點）：

| 卡 | engage | 射擊解 | 對轟 engage | 對轟射擊解 | 距離 m |
|---|---|---|---|---|---|
| 遭遇戰 | 5.5 | 4.7 | — | — | 316 |
| allies-sweep | 21.2 | 11.8 | — | — | 1166 |
| allies-intercept | 2.5 | 8.2 | 19.9 | 10.7 | 287 |
| allies-escort | 6.9 | 6.9 | 2.5 | 2.5 | 227 |
| allies-evac | 15.2 | 16.4 | — | — | 605 |
| axis-intercept | 4.7 | 5.7 | 5.4 | 3.0 | 271 |
| axis-escort | 3.3 | 1.5 | 5.4 | 1.2 | 265 |

### 14.4 判準的結論

**`quota = 1` 過不了。** 兩格退步超出雜訊：`allies-intercept` 的射擊解
−8.6pp（雜訊 8.2）、對轟射擊解 −25.6pp（雜訊 10.7）。它把「追上去」做得太
徹底 —— 對轟 `engage` 由 13.7% 衝到 52.3%，但打中的比例掉一半。

**`quota = 0.5` 只有一格退步超出雜訊**：

| 格 | 0 → 0.5 | 雜訊帶 | 差 |
|---|---|---|---|
| `axis-escort` 對轟射擊解 | 33.4% → 18.8% | 1.2pp | **−14.6pp** |

同一張卡上整體射擊解是 9.7% → **26.7%**（雜訊 1.5pp，大幅超出的**改善**）。
也就是護航的 Bf 109 打戰鬥機打得多很多、打轟炸機打得少一些。**要不要按機型
分開定值是專案負責人的裁定**，實作不自己分（計畫 Task 12 Step 4）。

### 14.5 兩張卡沒有解析度

`allies-sweep` 與 `allies-evac` 的雜訊帶大到無法判讀（sweep 的 `engage`
全距 21.2pp、射擊解 11.8pp）。±0.5% 的初速擾動就造成那麼大的差異，表示那兩
張卡本身混沌。sweep 的射擊解 8.0% → 1.7% 看起來很糟，但它落在自己的雜訊帶
裡，**證明不了任何事**，也不該拿來當否決的理由。

### 14.6 距離普遍上升

多張卡的交戰距離中位上升 300–1250 m，超出雜訊帶。**那不是缺陷，那就是這個
戰術的定義**（保持在 `perchRange` 外側經營能量），但它是人工驗收時會直接感覺
到的變化。

### 14.7 `buildMax` 的回填式換掉了

計畫寫的是 `perchEnter / p10(d(energyRatio)/dt) × 1.3`，實測 p10 是 **−0.0099**
（中位 +0.0069）—— 一成的時間裡敵人累積得比自己快，那個除法沒有定義。改用
直接量到的 `build → perch` 秒數（n=71，p90 = 9.8 s），×1.3 得 **13 s**。

同一批量測推翻了一條設計期的假設「等待不該比建能久」（`perchMax < buildMax`）：
`build` 的停留中位是 **0.4 s**，也就是 `minDwell` —— 進場時 `energyRatio` 多半
已經高於 `perchEnter`，「建能要多久」根本不是這一層的限制。那條測試換成
「每個期限都要留得住它在等的那件事」，否則相位只會被期限踢走，機制退化成
計時器。

### 14.8 `quota = 0` 是逐位元乾淨的

`tactics-off.test.ts`：`quota = 0` 的 30 秒重播校驗和，與戰術層落地前一刻
（`5a0297f`）**一個字不差**，兩個場景都是。基準另存 `tactics-baseline.ts`，
沒有動 `spawn-baseline.ts`。

`PURSUIT_MIRROR_8V8` 開關沒有差別，量過才知道不是「沒接上」：那一場全程最遠
交戰距離 2174 m，而 `enterRange` 是 2500 m，`farLatch` 一次都沒開。成因也釘成
測試，日後 `enterRange` 一改那條就會紅。

### 14.9 全套回歸的紅測試歸屬

在分支起點（`af8f976`）跑同樣的檔案分出來的：

| 測試 | 起點 | 現在 | 歸屬 |
|---|---|---|---|
| `order-of-battle-replay` ×2 | 紅 | 紅 | 既有的債 |
| `ai-withdraw-anchor` ×1 | 紅 | 紅 | 既有 |
| `ai-command-channel` ×1 | 紅 | 紅 | 既有 |
| `ai-command-decision` ×1 | 紅 | 紅 | 既有 |
| `ai-targeting` ×1 | **綠** | **紅** | **§4 的方向修正造成** |
| `ai-command-tactics` ×1 | **綠** | **紅** | **§4 的方向修正造成** |

後兩條的數字：

```
ai-targeting        fireShare  0.0160  對門檻 0.025
ai-command-tactics  側翼的開火方位角  −0.252  對門檻 ≤ −0.436
```

兩條都**不是**戰術層造成的（`quota` 預設 0，那一層全程關著），而是 §4 的
方向修正 —— 也就是真正修掉人工回報那個症狀的那一項。**護欄重新定值是專案
負責人的決定，實作沒有動任何門檻。**

### 14.10 這一輪沒有 Playwright 冒煙

計畫 Task 13 Step 4 寫了 `npx playwright test`，但這個 repo **沒有
`playwright.config`、也沒有任何 `.spec.ts`** —— Playwright 在這個專案一直是
臨時寫腳本用的，沒有常設套件。那一步因此沒有執行，不是漏跑。
