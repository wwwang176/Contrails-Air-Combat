...
> **實作後修正（2026-08-06）。** 第 2 條照原樣寫出來是**恆真的**：20v20 在
> 4000 m 開打，120 秒內全隊最低高度 2210 m（`ACE`）／2614 m（`VETERAN`），
> 沒有任何一架靠近地面，安全層一次都沒動。改成五個**低空**受控場景
> （400~2000 m 起始、120 秒、子彈無傷害），判準也改了：
>
> ```
...
> ```
>
> **`VETERAN` 確實比 `ACE` 飛得更低（63 對 115 m）**，所以原本設想的「不得
> 高於 `ACE`」直接紅。實際採用的判準是「不觸海」——那才是玩家在意的，而且
> 非恆真（0.5 s 那一列有 644 步觸海）。
>
> 0.5 s 那一場的軌跡查出一個**與本案無關的既有缺陷**：`applySafety` 的閉式
> 解假設俯衝角不再變陡。t=113.0 高度 367 m、γ=−40°，需要 279 m，通過；
> 0.75 秒後 γ 已 −60°，需要 459 m，而高度只剩 292 m。零延遲的同一場也只剩
> 115 m，是同一個病。修它要動 `DEFAULT_SAFETY.factor`，會移動全部既有基準，
> 另案處理。
>
> 另外，「安全層接管時把延遲緩衝區倒掉」這個想法實作過（`CommandDelay.reset`），
> 掃描下來 644 → 667 步觸海，**沒有可量測的效果**，已移除。
