...
改用**解析**的表示法，而且兩個階段用不同的幾何：

```
  broad phase   當前航跡是一條**線段** —— 安全層不介入時飛機確實直飛。
                對島的膨脹圓做精確的線段-圓相交（不是加長的方框）
  驗算          「如果我往那邊轉」是一段**圓弧接一段直線** ——
                弧長 |turn| × R，之後沿新航向直飛補滿前視距離
```

轉彎半徑 `R = V²/(g·√(n²−1))` 是這一架此刻的物理量，不是設計參數：
重力在那個 −1 裡，失速由 `maxLoadFactorAero` 進到 n 裡。

弧與一個點的最小距離有閉式解 —— 距離平方是 `R² + d² − 2Rd·cos(θ − θ*)`
（餘弦定理），極小值只有一個。落在弧段內就是 `|R − d|`，否則在端點。

**沒有取樣就沒有 alias**，成本是 O(島數) 的加減乘，而且**知道自己在繞哪座島**
（島的索引），解除條件因此明確。

【驗算非用弧不可，而且弧必須跟著角度】初稿的驗算是「把航向瞬間旋轉，
再測一條直線」—— 那正是它用來否決射線法的同一個毛病。而第一版的弧算的是
「沿這一側一直轉下去」，與角度無關 —— 於是每個候選角度得到同一個答案，
挑不出東西來。路徑必須是**弧接直線**：飛機轉到目標航向之後就直飛。

【已經在某座島的圓裡時，那座島不參與驗算】「會不會進入圓」對它沒有意義，
飛機已經在裡面了，轉向的目的正是出去。不跳過的話每個候選角度都被判成
blocked，結果是**最該全力轉開的時候不轉**。
...
side: -1 | 0 | 1      // 承諾的方向，**獨立於 turn**
turn: number          // 這一次要下的航向偏移，rad
clearSamples: number
```
...
【`side` 必須與 `turn` 分開】初稿把承諾的符號塞在 `turn` 裡。而它會因為
「這一刻每個角度都被堵住」變成 0 —— 下一次感知就會把 0 當成正側，
**原本的負側承諾翻面**，那就是換一個觸發條件的乒乓。

【解除的判準是「不再有交會」，不是「通過了橫斷面」】初稿寫
 `along < 0 && centre > r`，用的是**當下航向** —— 沿著島的外圍
切線繞行時 `along` 趨近 0，即使早就沒有碰撞路徑也可能永遠不成立。實測有八組在 50 秒後
仍然鎖著。

【承諾側暫時受阻不得立刻反轉】單次讀值只能讓這一次的 `turn` 變 0
（交給安全層全力拉起），**`side` 保留**。

【設計時的「最短鎖存 0.5 秒」，實作時拿掉了】那條的用意是避免「才轉 15°、
山還在翼尖前方」就交還控制權。但上面那組幾何條件已經涵蓋它 —— 要離開膨脹圓、
**而且**照現在的航向不會再碰到它、**而且**連續三次，飛機轉一點點是滿足
不了的。
...
export interface TerrainSense {
  ...
  turn: number          // 這一次的航向偏移，rad；0 = 不轉
  side: -1 | 0 | 1      // 承諾的方向，獨立於 turn（見 §4.5）
