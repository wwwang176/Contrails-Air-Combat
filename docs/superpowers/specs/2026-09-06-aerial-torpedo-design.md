...
**差異全部落在水線之下。** 上面量的是**爆心到艦體的距離函數**，那一層
逐位元不變。

**但「炸彈進不到水線之下」是錯的**（2026-09-06 Codex 審查指出，本地重現）。
`onBombBlocked` 走的是**線段**，而炸彈有水平分量：跨越水面的那一步，線段
會一邊下沉一邊橫掃（120 m/s 下每步 0.50 m）。落點在舷外不到半公尺時，那
一段線在水線之下切進舷側，`segmentBox` 的進入面是**側面**而不是盒頂。

實測（Fletcher，水平 120 m/s，掃 501 次投放）：

```
  事件種類改變      21 / 501 次（投放位置差 0.42 m 的一條帶）
  第一個            落水 @x = −6.452（舷外 0.41 m）→ 命中舷側 @y = −0.157
```

**這是變好，不是變壞**：以 120 m/s 撞進舷外 41 cm 水面的炸彈，本來就會
打到水線下的艦體。舊盒把它算成近失（傷害 8,991／20,000），新盒算成直接
命中（9,000）——差 0.045%，而帶寬隨每步的橫掃距離走。

**留著不改。** Codex 開的藥方是「`onBombBlocked` 忽略 y < 0 的 hull 交點」，
那會讓炸彈穿過水線下的艦體，而且在熱迴圈裡多一個分支換回一個較差的行為。

護欄仍然要寫——上面是量測，護欄是為了讓它日後改盒時還會紅。
