...
### 4.5 船的標記高度只有**模型**答得出來

「物體的最高點」是 GLB 的包圍盒，不是任何碰撞體積。三次量測：

```
             甲板   盒頂    模型
  Fletcher    4.5   12.7    27.0
  Wichita     7.0   13.7    38.2
  Essex      14.0   23.1    45.2
```

- **甲板**（`deckHeightOf`）＝船體盒的頂。船體盒**一律止於主甲板**
  （`ships.ts` 的硬性不變量），所以上層建築整片在外。
- **盒頂**＝再加上砲位盒。砲塔進來了，但**桅杆與測距儀不在任何盒裡**。
- **模型**＝GLB 的包圍盒。這才是玩家看到的那艘船的頂。

前兩個都試過、都被退回（負責人 2026-09-07 試玩回報兩次：符號插在艦橋
中間）。定案是 `render/ships.ts` 的 `shipModelTop(id)` —— `preloadShipModels`
時 `Box3.setFromObject` 量一次存起來。

`fillMarkers` 因此多收一個 `shipTop: (ship) => number` 回呼：答案住在算繪層，
而 `hud/` 不能往那個方向依賴。載不到 GLB 就丟，不回一個猜的數字 —— 那正是
這一輪在修的東西。

`deckHeightOf` 留給轟炸解算（炸彈落在甲板上），並從 `ai/bombRun.ts` 搬到
`world/ships.ts`。
