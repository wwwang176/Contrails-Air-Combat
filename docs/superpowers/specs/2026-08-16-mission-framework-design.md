...
三個數字（`entry.ts` 的 `PURSUIT`）：

| 量 | 值 | 推導 |
|---|---|---|
| 後方間距 | 800 m | 負責人指定。機槍有效距離在 600 m 內（匯聚點 300 m） |
| 高度優勢 | +1,000 m | 我取的值。1,000 m 位能理想俯衝可換約 140 m/s（`√(2gh)`）—— 慢的飛機追得上快的飛機的唯一辦法 |
| 橫向錯開 | 0 | `lateralOffset = 1500` 是為了解**對頭**的匯聚問題。追擊沒有對頭，而 1,500 m 的錯開會把「後方 800 m」變成「側後方 62°」—— 那是並排飛不是被咬 |

合起來：開局斜距 `√(800² + 1000²) = 1,281 m`，在射程之外 —— 他們**必須俯衝
加速才咬得到你**。

**三個都是起始值，由試飛裁定。** 改的位置是 `src/battle/entry.ts` 的 `PURSUIT`。
