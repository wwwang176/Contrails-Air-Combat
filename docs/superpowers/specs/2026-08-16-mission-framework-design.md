...
| 軸心國時限 | **235 s** | `168.2 × 1.4 = 235.48`。【這裡原本寫 236】我把它心算成 235.5 才進位 —— 程式一直是對的（它算式），錯的是我的註解與這一格（Codex 審查）。兩個秒數現在由測試逐值釘住 |
...
§8.2 全表的玩家存活率是 **100%**（唯一一格掉的是僚機）。也就是說：對準圓環、
壓住 WEP、不理任何人，5 星任務就過了。

探針的 `Runner` 是完美飛行（全程 WEP、不迴避），所以那是**難度的下限**，
不是實際體驗。當時列了三個方向：收緊餘裕倍率、改開局幾何、接受它。

**專案負責人 2026-08-16 裁定：改開局幾何 —— 敵機放在身後、而且比較高。**

我先講了一個懷疑：敵機放身後**對同盟國那張卡可能反而更簡單**，因為現況至少
還有一次對頭交錯，而 P-51 跑得比 Bf109 快、身後的追兵只會愈落愈遠。負責人
裁定「就放，我自己測」—— 難度是體感，而探針量不到「一邊閃一邊走」（§10 第
3 條）。所以**不再量，直接做，由試飛裁定**。

#### 第一版失敗了：後方 800 m、高 1,000 m

專案負責人試飛：**「這樣的距離其實根本追不到。」**

成因是那 1,000 m 高度差 —— 斜距 `√(800² + 1000²) = 1,281 m` 幾乎全被**高度**
吃掉（俯角 51°）。追兵開場在**俯衝**而不是在**接近**，而玩家同時在平飛加速
跑掉；等他們把高度換成速度，橫向距離已經拉開了。

**「高度就是能量」沒有錯，錯的是我判斷了瓶頸。** 在這個場景裡能量不是瓶頸，
**位置才是** —— 追不上不是因為他們沒力氣，是因為他們一開始就不在後面。

#### 第二版：後方 400 m、高 200 m
...
**設計意圖也換了**：不再指望他們「追上來」，而是讓他們**開局就已經在射擊
位置上**。玩家必須閃，而閃就是掉速度、掉速度就是掉時間 —— 壓力來自倒數，
不是來自被追上。**跑掉本來就是撤離該有的結局。**

| 量 | 值 | 誰定的 |
|---|---|---|
| 後方間距 `gap` | 400 m | 我（第一版的 800 由負責人指定，試飛否決） |
| 高度優勢 `climb` | +200 m | 我 |
| 橫向錯開 | 0 | `lateralOffset` 是為了解**對頭**的匯聚問題。追擊沒有對頭，1,500 m 的錯開會把「後方 400 m」變成「側後方 75°」—— 那是並排飛不是被咬 |

**兩個數字仍是起始值，由試飛裁定。** 位置：`src/battle/entry.ts` 的 `PURSUIT`。
...
**專案負責人 2026-08-16**：「任務的擺位不一定只有對頭 OR 追我，應該要把擺位、
面向、初始狀態都寫成陣列，讓每個任務有不同的擺法。」

初版寫成 `'headOn' | 'pursuit'` 的可辨識聯集 —— 那是把「這一輪只需要兩種」
誤當成「結構上只有兩種」。改成 `src/battle/entry.ts` 的一張表：

```ts
export const ENTRY_PLANS = { headOn: HEAD_ON, pursuit: PURSUIT } as const
export type EntryPlanId = keyof typeof ENTRY_PLANS
```

每一份是兩隊的 `SideEntry`，六個欄位：

| 欄位 | 單位 | 意思 |
|---|---|---|
| `along` | `entryRange` 的倍數 | 沿 Z 的位置。+0.5 = 藍隊那一側 |
| `across` | `lateralOffset` 的倍數 | 橫向位置 |
| `gap` | 公尺（絕對） | 沿 Z 的**額外**偏移 |
| `climb` | 公尺（絕對） | 相對 `altitude` 的高度加成 |
| `heading` | 弧度 | 機首朝向。0 = 朝 −Z |
| `speed` | `tas` 的倍數 | 初速 |

每張卡帶一個 `entry: EntryPlanId`。**加一種擺法 = 表上多一個字面值 + 卡片改一個
字串，`createBattle` 一個字都不用動。**

### 8A.1 為什麼 `along`／`across` 是係數而不是絕對座標

`entryRange` 與 `lateralOffset` 是 `BattleConfig` 的欄位，而
`stall-loop.probe.ts`、`turn-shrink.probe.ts`、`ai-command-decision.test.ts`
**都靠覆寫它們來換場景**。擺法若寫死絕對座標，那些覆寫會**靜靜失效** ——
探針照跑、數字照印，只是量的不是它宣稱的東西。

`gap` 與 `climb` 才是絕對公尺，因為它們沒有對應的尺標：「後方 800 m」由機槍
的射程決定，與兩隊的開局間距無關。

### 8A.2 測試只驗擺位

**專案負責人 2026-08-16**：「任務的測試應該只要寫是否擺位正常就好，玩起來
怎樣還是以玩家自己測得為主。」

所以 `battle-mission-wiring.test.ts` 的 21 條**不量難度、不量存活率、不量
到達時間**，只回答一個問題：**表上寫的東西，有沒有原封不動變成飛機的出生
狀態。** 那正是把擺法變成資料之後唯一該由機器守的東西。

【一個讀錯的量】初版斷言對頭的 Z 間距等於 `entryRange`，實測 10,180；橫向
1,750 而不是 1,500。那 +180／+250 正是 `entryRange` 自己的註解寫的東西 ——
**它定義在分隊原點上，不是重心**（站位偏置讓重心多拉開 180 m）。改讀分隊
長機（`STATION_REFERENCE[0] < 0`，就在原點上）之後逐條精確。
