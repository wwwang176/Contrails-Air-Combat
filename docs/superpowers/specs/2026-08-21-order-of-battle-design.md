...
| 項目 | 結果 |
| --- | --- |
| 出生表逐字比對 | `HEADON_20V20` 51 行（40 架 + 10 小隊 + 玩家座位）、`PURSUIT_MIRROR_8V8` 21 行（16 架 + 4 小隊 + 玩家座位），**全部相同** |
| 30 秒重播 | 51,409 / 49,033 個元素，SHA-256 兩場都相同（值見 `test/fixtures/spawn-baseline.ts`） |
| 護欄不是空的 | 把 fixture 第一架的 x 由 `-2350` 改成 `-2351`，測試立刻紅，訊息指到「第 0 架的 x 欄位」 |
| 型別 | `npx tsc --noEmit` **0 errors** |
| 全套回歸 | 2,717 條、2,707 綠、7 skipped；紅的仍是既有那三條（`ai-command-channel` ×2、`ai-withdraw-anchor` ×1） |
| 單獨跑 | `rematch` 3 綠、`perf-gate` 6 綠 |
| 動到的檔案 | `src/` 6 支、測試與探針 18 支、新增 4 支 |

**改動前的基準**：117 個檔（2 紅）、2,690 條（3 紅、2,680 綠、7 skipped）。
新增 27 條（4 重播 + 17 order + 6 flights）。

### 8.6 一個 spec 沒有預料到的陷阱（實際踩了兩次）

**`tsc` 的錯誤清單不等於「全部的呼叫端」。** TypeScript 的多餘屬性檢查只對
「新鮮的物件字面值」生效，所以工廠函數回傳的字面值、以及先存進未標型別的
`const` 再傳出去的設定，在欄位刪掉之後**一個字都不會報錯**，而那些欄位被
靜靜忽略、`DEFAULT_BATTLE.units` 照舊生效。

兩次都是**基準測試抓到的**（出生表的行數不對），而失敗訊息都指向錯的地方。
第一次 Codex 複審預言過，第二次是全套回歸抓到。詳見 `docs/backlog.md` §2.25。

**這一條反過來證明了 §8.1 的設計是對的**：如果驗收只有「30 秒重播的校驗和」，
兩次都會表現成一個看不出原因的雜湊不符；是**逐字的出生表**讓「51 行對上
21 行」一眼可辨。
