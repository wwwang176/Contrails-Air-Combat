...
- [x] **Step 4:** 手動確認 `spawn-baseline` 的校驗和沒有變（**變了就是誤動了模擬，停下來報告**）
...
- [x] **Step 1:** 寫 e2e，凍結姿態讓像素可比，輸出目錄用參數帶
- [x] **Step 2:** 跑一次，兩張圖逐張看
- [x] **Step 3:** `page.on('pageerror')` 必須是空的
- [ ] **Step 4: 交給負責人試玩**
...
**一、「落點是不是水」抽成 `torpedoEntersWater`，而不是留在 `main.ts`。**
計畫原本讓 `main.ts` 直接寫那一條比較，靠 Task 8 的「島上空截圖」驗收。
實作時發現那條路兩頭都不好：`main.ts` 的比較與 `stepAir` 是兩份、而那張
截圖要飛到島上空才拍得到（遭遇戰的編組 UI 也已經改版，`#blue-add` 不存在
了 —— `turret-visuals.e2e.ts` 那一段是過期的）。抽成純函數之後兩邊共用一支，
規則由 5 條單元測試釘死，e2e 就不必再拍那一格。

**二、Task 1 的 Verification 原本寫錯了。** 「`rollOk` 的 `<=` 改成 `<`
之後合取律那條也要紅」不成立 —— `canRelease` 與期望值都由同一支述詞算出來，
合取律對述詞本身的改動免疫。兩個變異的對象不同，已就地改成分開列。

**三、包絡的姿態上界在試飛中被放寬到 45°／±60°**（負責人裁定，高度不動）。
連帶把 HUD 閘門測試裡寫死的 `30°` 改成相對包絡（`maxRoll × 2`），否則
下次再調又會因為「調參數」而不是「壞掉」變紅。

**四、Task 8 的四張截圖縮成兩張。** 陸地那一格改由單元測試守（見一）；
「帶坡度」那一格在包絡放寬到 45° 之後，用滑鼠在無頭環境擺不出出界的姿態，
留給人工試飛。
