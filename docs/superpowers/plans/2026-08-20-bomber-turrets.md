...
**配置表**（`position` 是**槍口**；半角與旋轉速率是設計值）。

**「來源」欄的兩個值**：`量測` = 逐站量自參考模型（`b17g.hull.ts` / `he111.hull.ts`
把砲塔標為「另外貼零件」而丟掉的那幾段，區間與極值都在那兩份檔頭裡，B-17G 的
尾砲塔另見 `docs/backlog.md` §2.15）；`推算` = **量到的機身剖面 + 史實站位** ——
z 取真機的槍位、x 由該站量到的半寬外推一段槍管。

四座 B-17G 與兩座 He 111 是推算而不是量測，理由記在 `weapons/b17g.ts` 的
`B17G_TURRETS` 檔頭：參考模型的 25 個 mesh 全叫 `Object_NN`、有四個三角形數
完全相同又落在同一段 z，包圍盒認不出是哪一座；射線法在這裡也失效（從機身裡
往外打會飛過砲塔打到對側機身，同一站 maxRadius 0.9 量到 0.89、1.4 量到 1.34，
兩個半徑永遠不一致 —— 那正是 Task 3 Step 3 的驗收判準抓到的）。**猜是坑 22
的犯法，所以不猜。** 這幾格日後若量得出來，以量測值為準。

B-17G —— 八座、`guns` 合計 12：

| id | position | 來源 | axis | halfAngle | rotationRate | guns |
| --- | --- | --- | --- | ---: | ---: | ---: |
| `chin` | `(0, −0.70, −5.43)` | 量測 | `(0, −0.34, −0.94)` | 45° | 60°/s | 2 |
| `cheekL` | `(−1.15, 0.35, −4.60)` | 推算 | `(−0.57, 0, −0.82)` | 35° | 90°/s | 1 |
| `cheekR` | `(1.15, 0.35, −4.60)` | 推算 | `(0.57, 0, −0.82)` | 35° | 90°/s | 1 |
| `top` | `(0, 2.15, −1.10)` | 量測 | `(0, 1, 0)` | 80° | 60°/s | 2 |
| `ball` | `(0, −1.20, 5.10)` | 量測 | `(0, −1, 0)` | 80° | 60°/s | 2 |
| `waistR` | `(1.35, 0.55, 6.20)` | 推算 | `(1, 0, 0)` | 60° | 90°/s | 1 |
| `waistL` | `(−1.35, 0.55, 6.80)` | 推算 | `(−1, 0, 0)` | 60° | 90°/s | 1 |
| `tail` | `(0, 1.0, 16.4)` | 量測 | `(0, 0.09, 1)` | 30° | 90°/s | 2 |

He 111 H-6 —— 五座、`guns` 合計 5：

| id | position | 來源 | axis | halfAngle | rotationRate | guns |
| --- | --- | --- | --- | ---: | ---: | ---: |
| `nose` | `(0.25, 0.35, −2.93)` | 量測 | `(0, 0, −1)` | 40° | 90°/s | 1 |
| `dorsal` | `(0, 1.72, 2.40)` | 量測 | `(0, 0.64, 0.77)` | 70° | 90°/s | 1 |
| `ventral` | `(0, −0.85, 5.30)` | 量測 | `(0, −0.64, 0.77)` | 60° | 90°/s | 1 |
| `beamR` | `(1.05, 0.20, 2.80)` | 推算 | `(1, 0, 0)` | 45° | 90°/s | 1 |
| `beamL` | `(−1.05, 0.20, 2.80)` | 推算 | `(−1, 0, 0)` | 45° | 90°/s | 1 |
