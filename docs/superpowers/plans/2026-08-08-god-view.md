...
計畫寫完後交獨立審查，**每一條 finding 都附了行號、而且審查者實際打開過
那些檔案**。四條 Critical、九條 Important 全部成立。以下是逐條的處置；
**下面各任務的內文沒有重寫，以這一節為準**。
...
| # | 問題 | 處置 |
|---|---|---|
| C1 | `attachInput` 的**回傳型別註記**（`bindings.ts:28`）沒改，加了 `clearHolds` 會被超額屬性檢查擋掉（TS2353），呼叫端再吃兩個 TS2339 | 註記改成 `{ detach(): void; clearHolds(): void; tick(dt: number): void }` |
| C2 | Task 2 第四條測試**會紅**：`applyThrottleRate` 是**彈簧回中**的（`throttle.ts:53-55`），放開 W 之後油門會以 0.6/s 掉回 0.7，不是停在 1.1 | 斷言改成 `toBeLessThan(raised)`。要驗的性質是「卡住的 hold 會**撐住**油門，清乾淨的會讓它衰減」 |
| C3 | Task 3 的 helper **會假綠**：`compactFlights` 把死者的 `flightOf` 填 −1（`flights.ts:132`），而 `orders[-1]` 是 `undefined`，`undefined !== null` 為真 → 陣亡的每一格都被算成「拿到命令」 | helper 加 `f >= 0` 守衛並用 `?? null`。這同時修好第二條（它本來會因為人類替身被打下來而**紅在錯的理由上**） |
| C4 | Playwright 進不了戰鬥：實際是**三次點擊** `開　始`（含全形空格 U+3000）→ `遭遇戰模式` → `開始戰鬥`，而且按鈕文字在 `index.html` 不在 `screens.ts` | 腳本改成三次點擊，註解改指 `index.html` |

### Important

| # | 問題 | 處置 |
|---|---|---|
| I1 | **陣亡分支吃掉滑鼠改道**：鏈是 `dying` → `aiFlying` → `else`，上帝分支插在中間的話，死亡那兩秒 `godInput.lookX/lookY` 不再更新、而清除又被 `if (!godView)` 擋住 → 鏡頭以上一幀的位移**等速自轉**。spec §7.1 說「不受影響」，這個順序下就受影響了 | 上帝分支移到**鏈的最前面**，排在 `dying` 之前 |
| I2 | `enterBattle()` 沒清 `godView`/`wasGodView` → 上帝視角 → ESC → 回主選單 → 開始戰鬥，新的一場直接開在上帝視角，鏡頭停在舊世界座標。`enterBattle` 本來就有一行 `input.pointerLockLost = false` 專治這一類殘留 | `restartBattle` 加的那兩行，`enterBattle` 也要加。**spec §7.5 只寫了重開，這是 spec 的漏洞**，回填時補 |
| I3 | Task 3 Step 7 說「唯一的例外是 `ai-command-decision`」**不成立**：`ai-targeting.test.ts:45` 與 `multi-battle.test.ts:225` 的玩家座位也放 `AiController` | 兩個檔加進「預期會變」的名單 |
| I4 | Task 2 第二條測試**假綠**：`throttle` 起始就是 `CRUISE_THROTTLE`，回中函數原地不動，有沒有 `if (!godView)` 守衛都會過 | 先按住 W 把油門推離巡航，再進上帝視角 |
| I5 | Playwright 的畫面差異判準**恆真**：比的是 PNG 位元組，改一個像素整條 deflate 流就變了，而海浪與 40 架飛機一直在動 → `d1/d2/d3` 恆為 ≈1，三條判準永遠綠 | 改成**驗得到的就驗、驗不到的就明說是給人看的**：#16 改成直接讀 **HUD canvas（2D context，`getImageData` 讀得到）** 中央區域的準星像素；#17/#18/#19 降為人工看的截圖，不再假裝是斷言；#20 的 console 錯誤照舊是真斷言 |
| I6 | `maxAltitude` 12000 與 10 km 的海面網格打架：網格以鏡頭為中心重新定位，邊落在 `atan(高度/5000)` 的俯角上 | **維持 12000，理由見下方「I6 的裁定」** |
| I7 | `lookSensitivity` 的來歷寫錯：餵進來的 `aimDelta` 已經乘過 `MOUSE_SENSITIVITY` 1.6（`bindings.ts:82`），再乘 5.5 之後實際靈敏度是 **8.8**，不是註解宣稱的「沿用自由視角的 5.5」 | 改成 `5.5 / 1.6 = 3.4375`，讓實際值真的等於自由視角，並在註解裡寫明這個除法 |
| I8 | Task 3 讓 `setup.ts` 的兩處註解過期（`452-453` 的「玩家那一隊自治」、`473-475` 的「本來就被 skipFlight 跳過」），而計畫只修了無關的第三處 | 兩處一併修 |
| I9 | 小地圖的**高度符號**在上帝視角下仍以自機為基準（`main.ts:641` 的 `deltaY`）—— 平面重新置中了，三角形的上下卻沒有 | `deltaY` 的基準高度改成上帝視角時取 `godCam.position.y` |

### Minor（採納的）

- **M1** e2e 檔頭指錯設定檔：沒有 `vitest.config.ts`，設定在 `vite.config.ts:19-24`。
- **M2** `"exclude": ["test/e2e"],` 的尾逗號要拿掉。
- **M3** spec §8.6 寫 `god-view.spec.ts`、計畫寫 `god-view.e2e.ts` —— 計畫是對的，回填時修 spec。
- **M4** Task 1 的考題數：計畫自我檢查寫「十七條」、§5.1 那一列寫「十三條」，**實際交付 28 條**。
- **M7** `S`（後）與 `A`（左）沒有被單獨斷言過 —— Task 1 補兩條。
- **M8** `expect(raised).toBeGreaterThan(0)` 是空洞的（油門起始就是 0.7）→ 改成 `toBeGreaterThan(CRUISE_THROTTLE)`。
- **M9** e2e 的三個 `throw` 都排在 `browser.close()` 之前 → 失敗時會留下孤兒 chromium。包 try/finally。
- **M10** §8.5 的「不得配置記憶體」沒有自動檢查，`perf-gate` 只涵蓋 `World.step` —— 自我檢查表那一列是誤導的。改成明說它靠**人工讀碼**保證（實作確實只有純量運算與 `Vector3.set`）。
- **M11** Task 5 Step 1 宣告、Step 3-4 才使用的三個常數會在中途觸發 `noUnusedLocals` —— Task 5 一次做完，不在中間跑 `tsc`。

**M5**（`menuCamera.ts` 是 51 行不是 40 行）與 **M6**（決定性測試只抓得到
`Math.random`/`Date.now`）是事實陳述，無需改動。

### I6 的裁定：`maxAltitude` 維持 12000

審查指出海面網格是 10 km 見方、以鏡頭為中心重新定位，所以網格的邊落在
`atan(高度 / 5000)` 的俯角上；`menuCamera.ts:4-11` 記載過同一個現象，
而且那正是選單鏡頭壓到 150 m 的理由。

**但這個幾何在上帝視角裡不是新的。** 戰鬥本來就發生在
`DEFAULT_BATTLE.altitude` = 4000 m，追逐鏡頭就跟在那個高度上，網格的邊
在 38.7° 俯角處 —— 也就是說**現在的遊戲已經天天在那個條件下跑**。上帝
視角的進場高度是 4800 m（邊在 43.8°），差別是 5°。而 `maxAltitude` 12000
對應的 67° 俯角，玩家本來也飛得到（P-51D 的升限就在那個量級）。

所以這不是這一份引入的問題，把 `maxAltitude` 砍掉也治不了那個現象。
**維持 12000**，並把它排進 §8.7 手動試飛的**第一項**：貼著上限俯視時
海面看起來如何。若真的難看，那是一個**既有的**算繪問題（海面網格的
尺寸與 LOD），要獨立處理，不該在這一份裡用一個參數把症狀藏起來。

**這一條要讓專案負責人知道** —— 它是我依上述推理自己裁定的。
