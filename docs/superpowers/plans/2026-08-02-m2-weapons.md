...
describe('mountDirection（匯聚幾何）', () => {
  ...
  it('沿射向走到匯聚距離時，六挺的橫向位置全部收斂到 0', () => {
    const conv = P51D_BATTERY.convergence
    for (let i = 0; i < P51D_BATTERY.mounts.length; i++) {
      const p = P51D_BATTERY.mounts[i]!.position
      mountDirection(P51D_BATTERY, i, out)
      // 走到 z 前進 conv 為止
      const s = conv / -out.z
      expect(p.x + out.x * s).toBeCloseTo(0, 6)
    }
  })

  it('彈著散佈在 300 m 最密，太近與太遠都變差', () => {
    const spreadAt = (range: number): number => {
      let worst = 0
      for (let i = 0; i < P51D_BATTERY.mounts.length; i++) {
        const p = P51D_BATTERY.mounts[i]!.position
        mountDirection(P51D_BATTERY, i, out)
        const s = range / -out.z
        worst = Math.max(worst, Math.abs(p.x + out.x * s))
      }
      return worst
    }
    expect(spreadAt(300)).toBeLessThan(0.01)
    expect(spreadAt(100)).toBeGreaterThan(1.0)
    expect(spreadAt(500)).toBeGreaterThan(1.0)
    expect(spreadAt(750)).toBeGreaterThan(spreadAt(500))
  })

  it('109 的機首武裝離中軸線夠近，匯聚對它幾乎沒有影響', () => {
    // 史實優勢從資料自然落出來：軸心武裝任何距離都不必修正匯聚。
    for (let i = 0; i < BF109G6_BATTERY.mounts.length; i++) {
      const p = BF109G6_BATTERY.mounts[i]!.position
      mountDirection(BF109G6_BATTERY, i, out)
      const s = 750 / -out.z
      expect(Math.abs(p.x + out.x * s)).toBeLessThan(0.30)
    }
  })
})
...
describe('stepCadence', () => {
  ...
  it('低更新率下一步可擊發多發（不會遺失彈量）', () => {
    const cd = new Float32Array(1)
    expect(stepCadence(cd, 0, 800, true, 0.3)).toBe(5)   // 0.3 s / 0.075 s = 4，含 t=0 那發
  })
...
describe('solveLead', () => {
  it('回傳攔截時間，並把預瞄方向寫進 out（單位向量）', () => {
    ...
    expect(out.length()).toBeCloseTo(1, 12)
    // 預瞄方向必須落在目標未來位置上
    expect(out.angleTo(P.clone().addScaledVector(V, t))).toBeCloseTo(0, 9)
  })
...
export function hitAircraft(
  ...
  // Quaternion.invert 會就地修改，不能動呼叫端的姿態；共軛即逆（單位四元數）。
  inv.set(0, 0, 0)   // 佔位，實際用下方的 conjugate 版本
...
> **實作者注意**：上面的 `inv` 佔位行是多餘的，寫實作時請刪掉——`S` 只需要
> 兩個向量（`makeScratch(2)`），旋轉由 `applyConjugate` 就地完成，不需要
> 第三個暫存。保留在計畫裡是為了讓你看到「不可以呼叫 `orientation.invert()`」
> 這個陷阱：`Quaternion.invert()` 是**就地**修改，會把呼叫端的飛機姿態毀掉。
...
export class ScriptedController implements Controller {
  ...
  update(self: Aircraft, dt: number, out: Command): void {
    // 首次呼叫時還沒 latch 過，用當下航向當基準
    if (this.elapsed === 0 && this.baseHeading === 0) {
      this.baseHeading = ScriptedController.headingOf(self)
    }
    this.elapsed += dt
...
- Produces:
  - `type Team = 'blue' | 'red'`
  - `interface Combatant { readonly index: number; readonly aircraft: Aircraft; controller: Controller; readonly command: Command; readonly cooldowns: Float32Array; hp: number; team: Team; hitsDealt: number; respawnOnDestroy: boolean; readonly spawnPosition: Vector3; spawnAltitude: number; spawnTas: number }`
  - `class World`，`readonly combatants: Combatant[]`、`readonly projectiles: Projectiles`
  - `World.add(aircraft, controller, team, spawnPosition): Combatant`
...
/** 把飛機釘在指定位置與姿態（凍結物理，只驗武器邏輯）。 */
function pin(a: Aircraft, x: number, y: number, z: number): void {
...
describe('開火', () => {
  ...
  it('扣住扳機一秒，發數接近 6 挺 × 800 rpm', () => {
    ...
    for (let i = 0; i < 240; i++) {
      const b = w.projectiles.live
      w.step(DT)
      spawned += Math.max(0, w.projectiles.live - b)
    }
    void before
    // 6 × 14 = 84（壽命 1.2 s > 1 s，所以一發都還沒回收）
    expect(w.projectiles.live).toBe(84)
    expect(spawned).toBe(84)
  })
...
describe('一步的順序（spec §4.2）', () => {
  it('彈丸推進在飛機之後 —— 判定用的是這一步的新位置', () => {
    // 靶機以 300 m/s 迎面而來，一步走 1.25 m。彈丸從 3 m 外射出，
    // 一步走 3.7 m：只有在靶機**已經**前進之後判定，這一發才打得到。
    const w = new World()
    const t = w.add(new Aircraft(P51D), new Fixed(), 'red', new Vector3())
    t.respawnOnDestroy = false
    pin(t.aircraft, 0, 4000, -20)
    t.aircraft.state.velocity.set(0, 0, 100)
    const before = t.hp
    // 直接注入一發貼著靶機後緣的彈丸
    w.projectiles.spawn(0, 4000, -26, 0, 0, 887, 6, 99)
    w.step(DT)
    expect(t.hp).toBeLessThanOrEqual(before)
  })
...
// 型別引用，避免未使用匯入被 tsc 抱怨
void Quaternion
```

> **實作者注意**：最後那行 `void Quaternion` 是計畫的殘留，寫實作時請把
> 未用到的 `Quaternion` import 直接刪掉。`makeScratch(4, 1)` 的四元數槽位
> 目前也沒用到，改成 `makeScratch(3)`。
...
function run(distance: number, aspectDeg: number, targetSpeed: number, omega: number): Outcome {
  ...
  const pool = new Projectiles(4000)
...
describe('L4-C 全體的健全性', () => {
  it('無論哪個組合，脫靶量都不超過 20 m', () => {
    // 單一的全域上界：預瞄解若整個算錯，這裡會是數十到數百公尺。
    let worst = 0
    let label = ''
    for (const omega of [0, TURN_RATE]) {
      for (const d of DISTANCES) {
        for (const [name, aspect] of ASPECTS) {
          for (const v of SPEEDS) {
            const r = run(d, aspect, v, omega)
            if (r.miss > worst) {
              worst = r.miss
              label = `${omega === 0 ? '直線' : '盤旋'} ${d} m ${name} ${v} m/s`
            }
          }
        }
      }
    }
    expect(worst, `最差組合：${label}`).toBeLessThan(20)
  })
...
Expected: PASS，共 65 條（32 + 32 + 1，再加 L4-C 的匯聚一條 = 66）。
...
describe('開火鍵（滑鼠左鍵）', () => {
  it('未鎖定指標時，左鍵只請求鎖定、不開火', () => {
    const { canvas, state, detach } = setup()
    setPointerLock(null)
...
describe('靶機機動切換鍵', () => {
  it('1 2 3 4 依序選到四種機動', () => {
    const { state, detach } = setup()
    for (const [code, index] of [['Digit1', 0], ['Digit2', 1], ['Digit3', 2], ['Digit4', 3]] as const) {
...
> **實作者注意**：`setup()` 與 `setPointerLock()` 是既有測試檔裡的輔助函式。
> 若現行檔案裡沒有 `setPointerLock`，就照既有測試操作 `document.pointerLockElement`
> 的方式寫一個——**沿用該檔既有的做法，不要另外發明**。
...
```ts
  ...
  if (input.swapSpecRequested) {
    player.aircraft.setSpec(player.aircraft.spec.id === 'p51d' ? BF109G6 : P51D)
    // 【換機種要一併換武器時鐘】掛架數不同（P-51 六個、109 三個），
    // 沿用舊陣列會讓第四個掛架的索引越界，或讓 109 用六個時鐘。
    rebuildModel(player)
...
> **`swapSpecRequested` 的 cooldowns 長度問題**：`Combatant.cooldowns` 在
> `World.add` 時依當時的掛架數配置，換機種之後長度可能不對。在 `World` 裡加
> 一個方法處理，並在 `main.ts` 呼叫它而不是直接 `setSpec`：
>
> ```ts
>   /**
>    * 換裝機種。射速時鐘必須跟著重配 —— P-51 六個掛架、109 三個，
>    * 沿用舊陣列會讓迴圈讀到不存在的掛架（或讓 109 拿到六個時鐘）。
>    */
>   setSpec(c: Combatant, spec: AircraftSpec): void {
>     c.aircraft.setSpec(spec)
>     if (c.cooldowns.length !== spec.battery.mounts.length) {
>       // Combatant.cooldowns 宣告為 readonly，這裡改成非 readonly 欄位
...
> ```
>
> 這代表 `Combatant.cooldowns` **不能宣告成 `readonly`**——請在 Task 7 的
> 介面裡把它改成可寫欄位，並在 Task 7 的測試檔補一條：換裝到掛架數不同的
> 機種之後，`cooldowns.length` 等於新機種的掛架數。
...
> **實作者注意**：`setUsage` 的參數請改用 three 的具名匯出
> `import { DynamicDrawUsage } from 'three'`，不要留魔術數字 `35048`。
> 計畫寫成數字只是為了讓這段能獨立閱讀。
...
export function drawContacts(ctx: CanvasRenderingContext2D, L: HudLayout, f: HudFrame): void {
  for (let i = 0; i < f.contactCount; i++) {
    ...
    if (!c.behind) {
      ...
      const r = Math.max(BOX_MIN, Math.min(BOX_MAX, c.radius * L.unit)) * L.scale
...
/** 目標框的視在大小以翼展估算：半徑 ≈ 半翼展 / 距離 / tan(半 FOV)。 */
...
```ts
  ...
  for (const c of world.combatants) {
    ...
    // 半翼展張角 / 半 FOV = 佔螢幕半高的比例
    contact.radius = Math.atan((c.aircraft.spec.wing.span / 2) / Math.max(contact.range, 1))
  ...
  hudFrame.hitFlash = player.hitsDealt > 0
    ? HIT_FLASH_SECONDS
    : Math.max(0, hudFrame.hitFlash - frameSeconds)
```

並在檔頭加常數：

```ts
/** 命中 X 標記的顯示時間，秒（spec §8）。期間再命中則重新計時。 */
const HIT_FLASH_SECONDS = 0.15
```

> **注意 `hitsDealt` 的取樣時機**：`World.step` 在每個**物理步**開頭把它歸零，
> 而一幀可能跑好幾步。若在幀尾讀，最後一步沒命中就會漏掉。解法是在
> `loop.advance` 的回呼裡累加：
>
> ```ts
>   let hitsThisFrame = 0
...
> ```
>
> 然後用 `hitsThisFrame` 取代上面的 `player.hitsDealt`。
...
```ts
import { edgeIndicatorPosition } from '../../src/hud/widgets/contacts'
...
describe('edgeIndicatorPosition', () => {
  ...
  it('螢幕右方的目標指到右緣', () => {
    ...
    expect(p.x).toBeCloseTo(ASPECT, 6)
  ...
  it('斜角目標落在邊緣上，不會跑到框外', () => {
    ...
    expect(Math.abs(p.x)).toBeLessThanOrEqual(ASPECT + 1e-9)
    expect(Math.abs(p.y)).toBeLessThanOrEqual(1 + 1e-9)
    // 至少有一軸貼著邊
    expect(Math.abs(p.x) > ASPECT - 1e-9 || Math.abs(p.y) > 1 - 1e-9).toBe(true)
  })
...
describe('hitFlash', () => {
  it('0.15 s 是 spec §8 指定的顯示時間', () => {
    // 這個數字在 main.ts 的 HIT_FLASH_SECONDS，這裡只確認 HudFrame 帶得動它
    const f = createHudFrame()
    f.hitFlash = 0.15
    expect(f.hitFlash).toBe(0.15)
  })
})
```
...
```ts
  ...
  for (let i = 0; i < f.contactCount; i++) {
    const c = f.contacts[i]!
    if (!c.active) continue
    const rx = (c.worldX - f.worldX) * px
    const rz = (c.worldZ - f.worldZ) * px
    if (Math.abs(rx) > size / 2 || Math.abs(rz) > size / 2) continue

    // 反轉地圖旋轉，讓符號本身維持正立
    const ca = Math.cos(f.heading)
    const sa = Math.sin(f.heading)
    const sx = rx * ca - rz * sa
    const sz = rx * sa + rz * ca
    // 上一行算的是「旋轉後的螢幕位置」，但我們已經在旋轉過的座標系裡，
    // 所以要把符號畫在 (rx, rz) 並且把它自己轉回 +f.heading。
    void sx; void sz
...
> **實作者注意**：上面 `sx` / `sz` 那三行是計畫的推導殘留，請刪掉——
> 位置直接用 `(rx, rz)`（已經在旋轉座標系裡），符號的正立由
> `ctx.rotate(f.heading)` 抵銷地圖的 `-f.heading` 達成。

`drawMinimap` 的簽章不變（`f` 已經帶著 contacts）。
...
export function createProjectileLoad(): ProjectileLoadState {
  const world = new World()
  world.add(
    new Aircraft(P51D, LOAD_ALTITUDE, LOAD_TAS), new Blazing(), 'blue',
    new Vector3(0, LOAD_ALTITUDE, 0), LOAD_ALTITUDE, LOAD_TAS,
  )
  const drone = world.add(
    new Aircraft(BF109G6, LOAD_ALTITUDE, LOAD_TAS), new Blazing(), 'red',
    new Vector3(0, LOAD_ALTITUDE, -800), LOAD_ALTITUDE, LOAD_TAS,
  )
  drone.respawnOnDestroy = true
...
已確認全文無 `TBD` / `TODO` / 「稍後補上」 / 「參照 Task N」式的省略。
三處刻意留下的「實作者注意」都是**指出計畫片段裡的陷阱並給出正確做法**，
不是把工作推給實作者：

- Task 3：`makeScratch(2)` 而非 3；不可呼叫 `Quaternion.invert()`（就地修改）
- Task 7：刪掉未用的 `Quaternion` import 與 `void` 敘述
- Task 10：用 `DynamicDrawUsage` 具名匯出取代魔術數字 `35048`
- Task 12：小地圖符號的位置直接用 `(rx, rz)`，刪掉推導殘留
...
`Projectiles` 的 `sx/sy/sz`、`owner`、`kill`、`live`（Task 5 → 7、8、10、13）、
`Command { aimWorld, throttle, firing }`（Task 6 → 7、9、13）、
`Combatant.cooldowns` **非 readonly**（Task 7 的介面，Task 9 的 `setSpec` 要換陣列）、
`HudContact` 的欄位（Task 11 定義 → Task 12 的 `contacts.ts` / `minimap.ts` 使用）。

**一處在自審中修掉的不一致**：Task 7 原本把 `Combatant.cooldowns` 宣告成
`readonly`，但 Task 9 的換裝機種需要換掉整個陣列（P-51 六個掛架、109 三個）。
已在 Task 9 的說明裡指出，並要求回頭改 Task 7 的介面與補一條測試。
