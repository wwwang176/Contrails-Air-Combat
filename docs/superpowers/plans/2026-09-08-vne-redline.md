...
Task 1～6 完成，commit `5b21097`…`5c56517`。與計畫的差異：

- **Task 5 多了一段**（`c179e7e`）：25° 俯衝 7 秒到不了對手放手的速度（實測
  IAS 最高 0.67 vne，拉到 20 秒也只到 0.68，航跡角實際 −15°），改成
  `divePitch` 45°、俯衝中不往錨點偏、上限換 `trackDiveMax` 20 s。SPEC §5.2、
  §8 已改。
- **守線不煞車**：試過加煞車，零戰停在 0.90 留在目標上方，F4F 20 架全滅；
  拿掉之後 11 架活著。SPEC §5.1 已記。
- **Task 6 的門檻由「關掉它」的量測定**，不量拉開速率與存活數（兩者在同一場
  改一個小地方就在 0～14 之間亂跳）。
- **Task 7**：G4M 投雷最高 0.89 vne（守線 0 介入、投放 18）；B-17 對頭場
  0.85；攔截任務判定前 0.63。判定後（約 160 s）B-17 有一段 −70° 俯衝到
  1.37 —— 既有行為、任務已結束，不在這份範圍。
- **Codex 審 PLAN 時被記憶體殺掉**（`memory allocation failed`），沒有產出；
  它列的六個疑點實作時逐一在程式碼核對過（vne 單位 m/s、守線排在撞地之後
  且 qbar 當步已更新、`extendPitchAngle(1, −Infinity, …)` 只剩離地項、
  `engageKnobs` 只寫兩個欄位、TAS 那行之前 fillStyle 是 dim、三個校驗和
  逐位元不變）。收尾再跑一次。
