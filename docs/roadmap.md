...
最後更新：2026-09-03（**三條戰役上線，12 關取代舊的 10 張卡，5 關可玩**；
...
**解鎖剩下的 7 關**：盟 M2 深入敵境、盟 M3 獵殺列車、盟 M4 艦隊上空、
德 M2 東線鐵路樞紐、德 M3 最後的 Gustav、日 M2 島嶼防衛、日 M4 最後的攻擊。
那七張現在是 `battle: null` 的**目錄卡** —— 標題與文案在，編制不填
（填進一張打不起來的卡只會變成一組沒人驗證過、卻看起來已經定案的數字）。
...
```
  ...
  盟 M4  艦隊上空          —       —       —      —         —              里程碑 2
  ────────────────────────────────────────────────────────────────────────────
  德 M1  攔截轟炸機群      Bf109   P-51D   B-17G  群島      60 s 波次      ✅
  德 M2  東線鐵路樞紐      —       —       —      —         —              里程碑 2、4
  德 M3  最後的 Gustav     —       —       —      —         —              里程碑 2
  德 M4  帝國最後防線      Bf109   P-51D   —      內陸      返航＋兩批     ✅
  ────────────────────────────────────────────────────────────────────────────
  日 M1  零戰              A6M5    F6F-5   —      群島      —              ✅
  日 M2  島嶼防衛          —       —       —      —         —              里程碑 2
  日 M3  護航              Ki-84   F6F-5   G4M    純海面    —              ✅
  日 M4  最後的攻擊        —       —       —      —         —              里程碑 2、4
```

**七張目錄卡的機種現在不填** —— 填進一張打不起來的卡，只會變成一組沒有人
驗證過、卻看起來已經定案的數字。它們要用什麼記在下面的設計段落裡：
盟 M2 玩家開 B-17G、盟 M4 敵方魚雷機用 G4M、德 M2 玩家開 He 111、
日 M4 玩家開 G4M。
