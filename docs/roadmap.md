...
**解鎖：盟 M2、盟 M3、德 M2、德 M3、日 M2、日 M3、日 M4（7 關）**
...
- [ ] Flak 陣地 —— 6 關（盟 M2 M3、德 M2 M3、日 M3 M4）
- [ ] 航空母艦 —— 3 關（盟 M4、日 M3 M4）
- [ ] 工廠／調車場 —— 2 關（盟 M2、德 M2）
- [ ] 驅逐艦 —— 2 關（盟 M4、日 M4）
- [ ] 火車（車頭 + 車廂）—— 1 關（盟 M3）
- [ ] 卡車／戰車 —— 1 關（德 M3）
- [ ] 登陸艇 —— 1 關（日 M2）
...
成本明確、收益乾脆：一台飛機換一整關。可以插在里程碑 2 的任何空檔做。
...
- [ ] 事件訊息／橫幅（里程碑 1 的第三種「效果」要用）
- [ ] 增援預警（要不要先給提示，還是讓玩家自己發現 —— 待裁定）

### 5.2 音效

`src/audio` 不存在，整個專案沒有音訊系統。
...
- [ ] **年代衝突**：He 111 的代表性情境是 1940，而規劃其餘落在 1944。
      規劃自己點出來了但沒有結論。選項：東線／地中海晚期，或放寬史實要求
- [ ] **日本第三架**：B6N 天山，還是讓 G4M 兼水平轟炸與魚雷攻擊、
      第三架改成 Ki-84（多一個「零戰 → 疾風」的成長感）
- [ ] **增援預警**要不要做（見 5.1）
- [ ] **音效**要不要做（見 5.2）
...
```
  ...
  德 M2  東線轟炸（開 He 111） ·  ●  ·  ●     年代待裁定
  德 M3  最後的 Gustav        ·  ●  ·  ·
  德 M4  帝國最後防線          ●  ·  ·  ·     撤退段 ≈ evacuate 現成
  ─────────────────────────────────────────────────
  日 M1  零戰                 ·  ·  ●  ·     只缺 A6M5
  日 M2  島嶼防衛             ●  ●  ●  ·
  日 M3  魚雷航跡（開 B6N）    ·  ●  ●  ●     需要 B6N
  日 M4  最後的攻擊（開 G4M）   ·  ●  ●  ●     需要 G4M
```

**●＝必要　·＝不需要**

三條 Campaign 共用大量資產：盟 M1 與德 M1 是同一批模型、同一張地圖的兩面；
盟 M4 與日 M3 是同一支艦隊、同一張海圖的兩面。表面 12 關，實際約 6～8 個場景。
