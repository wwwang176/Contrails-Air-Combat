...
```
  ⚑ A6M5              里程碑 3   擋住日 M1
  ⚑ G4M 一式陸攻       里程碑 3   擋住日 M3、日 M4、盟 M4
  ⚑ Ki-84 疾風         里程碑 3   擋住日 M3
```

**地面／水面目標要先決定「參考模型還是程序化」**（見下），決定之前不知道
要不要你找模型。

### 二、裁決：值與判準（我量得出來，值是你定的）

```
  ⚑ 地面目標用參考模型還是程序化生成          里程碑 2 的第一個決定，見下
  ⚑ 新機種的血量、防護力、難度星等            正比於質量是規則，絕對值仍要裁定
...
### 里程碑 2 的第一個決定：地面目標怎麼做

這一條決定你要不要去找一堆模型，所以要先問。

現有的房子、穀倉、教堂**全部是程序化生成**的（`src/render/floraShapes.ts`），
一棟房子是「牆 12 個三角形 + 人字屋頂 6 個」。同一條路可以做出卡車、Flak
砲位、火車車廂——它們在空戰的視距下本來就只是幾個盒子。

```
  甲  全部程序化      不必找模型，我可以直接做。外型陽春，但與現有聚落
                     同一個視覺語言
  乙  艦船用參考模型   航空母艦與驅逐艦的輪廓辨識度高、而且是兩關的主角，
      其餘程序化      值得一個真模型；卡車與 Flak 用盒子就好
  丙  全部參考模型     最好看，但要你找八種模型
```
