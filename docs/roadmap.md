...
最後更新：2026-09-03（**里程碑 1 完成**，帝國防空巡邏是第一張有第二波的卡；
日本三架的外型做完了，飛行規格在分支上處理中）
...
**🔧 = 外型做完了，還沒有飛行規格**（2026-09-03）。三台的 GLB 與
`*.model.ts` 都在，但沒有 `AircraftSpec`、沒有進 `buildAircraft` 的登記表，
所以遊戲裡還編不到。剩下的見里程碑 3。
...
- [x] 有波次的關卡按「重新開始」→ 整個 `Battle` 重建，不截斷
      （專案負責人 2026-09-01 裁定）
- [x] 任務卡上怎麼描述波次（`MissionCard.waves`）—— **相對描述**：卡片是
      陣營中立的，所以寫的是 `side: 'mine' | 'theirs'` 與
      `role: 'fighter' | 'bomber'`，機種由 `missionConfigFrom` 知道陣營之後
      才解析。條件也用 `side` 而不是 `team` —— 同一張卡上「敵方」不能有兩種
      寫法。第一張掛波次的卡是**帝國防空巡邏**（`axis-patrol`）

> **⚑ 兩個數字待你試飛裁定**（`missions.ts` 的 `axis-patrol`）：
> `atMost: 2`（第一批剩幾架時第二批來）與 `byLatest: 120`（兜底時限）。
> 離線探針把 AI 放在玩家座位上跑，400 s 只打掉 2 架紅機 —— 那條路上兜底
> 時限才是實際生效的那一個。人打起來快得多，但**合不合理是試飛的事**。
> 難度標籤同時由 2 改成 3。
