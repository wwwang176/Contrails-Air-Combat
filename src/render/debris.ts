 ...
 * 一次擊墜噴幾片。
 *
 * 【為什麼要這麼多】12 片 0.8–2.0 m 的方塊讀起來像「飛機掉了幾塊板子」
 * 而不是解體。小而多才像碎片。
 */
 ...
 * 最小／最大邊長，m。
 *
 * 【為什麼這麼小】0.8–2.0 m 的碎片有機翼弦長的一半那麼大，與機體並排看
 * 非常突兀。最大的一片 0.4 m 約是機身直徑的三分之一，才是碎片該有的尺度。
 */
 ...
 * 散射的初速，m/s。疊在母機速度之上。
 *
 * 【為什麼是 40】20 讀起來不夠散。這個值直接決定碎片雲擴張得多快：
 * 40 m/s 配阻尼 0.4 s⁻¹，一秒後雲的半徑約 33 m。
 *
 ...
 * 散射錐的半角。**等向（180°）** —— 不是錐。
 *
 * 【為什麼等向也不會炸成一顆球】零件是 360 度噴射，但又繼承飛機速度 ——
 * 一架 140 m/s 的飛機解體，40 m/s 的等向散射疊上去，**每一片的淨速度仍然
 * 朝前**，於是畫面上是一團往前擴張的碎片雲。
 *
 * 【所以「零件在母機前方」測不出散射錐】那條斷言在等向散射下照樣通過 ——
 * 繼承速度本來就主導。要驗散射錐得換一條量得到它的判準。
 *
 ...
 * 每一片各自的壽命範圍，s。**逐片隨機**。
 *
 * 【為什麼這麼短】讓碎片飛到落海要 40 s，但一片 0.4 m 的方塊在幾百公尺外
 * 只有一兩個像素 —— 飛那麼久換不到任何觀感，只是讓池子裡永遠有東西。
 *
 ...
 *
 * 【為什麼是方塊而不是真的把機體切開】同色的 BOX 就夠：真正切開程序化機體
 * 幾何是另一個量級的工作，而在交戰距離上一片 1 m 的碎片只有幾個像素
 * （M8 spec §15）。
 *
...
export function createDebris(capacity: number = DEBRIS_CAPACITY): Debris {
   ...
   * 碎片都沒有 —— 每幀白傳 92 KB。實測那一下要 3.92 ms，三十秒的量測裡
   * 佔掉 13.4 秒，將近一半的幀時間。
   */
  let touched = false
