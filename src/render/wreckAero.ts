 ...
 * # 殘骸的角向氣動
 *
 * 一具失去動力與操縱的機體怎麼翻滾。**只算力矩，不算力** —— 平移仍然是
 * 原本的彈道加指數阻尼（`wrecks.ts` 的 `WRECK_DRAG`）。少了升力，殘骸不會
 * 自己滑翔脫離；而力矩這一半讓它翻得像那個尺寸的機體該有的樣子。
 *
 * 【為什麼要有這一層】固定一個角速度上限的話，B-17 與 Bf 109 亂轉得一樣
 ...
 * 平衡轉速時推它、高於才阻尼。
 *
 * 0.065 讓 Bf 109 在終端速度下約每秒 60 度、B-17 約每秒 19 度。
 * **起始值，由試飛裁定。**
 */
export const WRECK_AUTOROTATION = 0.065
...
export function stepWreckSpin(
  ...
  // 大 dt 是正回饋，實測 dt = 0.1 時角速率在五秒內衝到 10⁹ rad/s。
