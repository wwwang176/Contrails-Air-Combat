 ...
 *
 * ── 【2026-08-26 由 14 m / 2 km 縮到 4 m / 571 m】────────────────
 *
 * **人工回報：「白點近距離確實像漂浮的紙片」。** 14 m 的塊在 120 m 高度
 * 貼海飛時佔 111 px —— 那個尺寸的白色多邊形讀起來就是水面上的紙片，而不是
 * 反光。
 *
 * 【不變量：遠處的螢幕大小 = SPARKLE_CELL ÷ SPARKLE_CELL_REF】兩個一起等比
 * 縮，鎖定之後的角張度一個字都不會變（14/2000 = 4/571 = 7.0 mrad = 6.67 px）。
 * 人工指定「遠距離跟現在一樣不變」，這條就是它的形式化。
 *
