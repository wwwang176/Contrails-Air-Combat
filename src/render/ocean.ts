 ...
 * 海面不吃霧（見下面兩個材質的 `fog: false`），沒有東西把它糊掉 ——
 * 它是 L 0.060 對 L 0.431 的硬階
 ...
 * 症狀是 5 km 處出現一條色帶。
 *
 * 【深色海 + 地平線淺色漸層】海本色是深藍 0x18344c，只有最貼地平線的
 * 0~5 度融向天空淺藍（見 SEA_AERIAL_HI、以及 SPARKLE_FRAGMENT 末的最終色
 * mix）。漸層只在極窄的地平線帶，所以 fog.test 的海天差幾乎不受影響。
 *
 ...
 *
 * 【為什麼不調低】GGX 的高光峰值是 `D_max = 1/(π·roughness⁴)`，0.72 只有
 * 1.18，等於高光被攤到不存在，所以海看起來只有霧面漫射。降到 0.10 峰值變
 * 2687×，面確實會變成太陽白 —— 但有兩件事擋著：
 *
 ...
 *
 * 【為什麼要用 MeshPhysicalMaterial】`MeshStandardMaterial`
 ...
 *
 * 【為什麼用 specularIntensity 而不是 ior】要的是「反光少一半」。
 ...
 *    4°    0.105    1.000    9.5×
 *
 * 【4.5° 是「往外擴散三倍」】從 1.5° 乘三。它落在上表 4° 那
 ...
 * 浪谷歸零）。
 *
 * 【為什麼不是純美術】鏡面條件只看**坡度**，不加偏置的話碎光落在浪的
 * **側面**，峰與谷（坡度 0）機會相同。真實海面不是這樣：長浪會調變短波的能量，短波
 ...
 * `fog: false` 要擋的「近到遠的顏色變化」。這一層只作用在碎光的加法項上，
 * 海的**基色**一個字都不動，所以那條規則仍然成立。
 *
 ...
 *
 * 【為什麼關掉】`N` 主要由波法線決定，而波是正弦的疊加，所以這一層的明暗
 * 會沿波形成**規則的橫條紋**。高空俯視最明顯：那裡短波全被 Nyquist 淡掉，
 * 只剩最長的幾道，圖樣最規則。條紋的代價大於明暗的收益，海面只留黑白塊。
 *
 ...
 * PBR 高光（太陽的 GGX 反光）壓成「少一半」。這裡管的是
 ...
 * 近深遠淺、柔和融入天空。
 *
 * 【它與 `fog: false` 的關係】海面不吃霧、海天靠硬色差是刻意的 low-poly
 * 選擇；這一層在那之上另外給地平線一點空氣感。**它不是把 fog 改回來的
 * 理由**，要改先問。
 *
 ...
 * 掠射、幾乎貼著水面的地方。用距離做會把整片中遠海都糊成一層霧氣。
 * `oceanV.y` 是視線與水平面
 ...
 * STRENGTH 0.3 壓淡 —— 對比小、含蓄。大範圍的「近深遠淺」不要用，那會糊掉
 * 整片中遠海。
 */
...
export function createOcean(shore: ShoreFieldData | null): Ocean {
  ...
  const material = new MeshPhysicalMaterial({
    ...
    flatShading: true,
    // 【海面不吃霧，spec §4.2】海面的近遠色差**完全**來自霧，而霧色是由
    // 天空色推導的，所以開霧時遠海必然往天空靠 —— 接縫因此糊成一片：那一
    // 階只有 0.092，而海面自己近到遠就變了 0.109。要的是「近到遠幾乎沒有
    // 顏色變化」。
