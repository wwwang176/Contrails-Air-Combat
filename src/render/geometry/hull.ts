 ...
 *
 * 【為什麼機身要真的挖洞】把座艙罩當成貼在封閉機身上的一片殼的話，透過
 * 半透明玻璃看到的還是機身蒙皮，讀起來就不是座艙。而想在 lofting 的封閉
 * 管子上做出凹口，唯一的辦法是壓扁該段截面，但截面一壓扁，同一站位的
 * **寬度**也跟著縮——實測艙緣高度的半寬會由 0.401 掉到 0.314，機身在
 * 座艙處出現一個腰身。
 *
 * 烘焙成顯式頂點就沒有這個限制：直接不要輸出艙緣以上的面即可。
 */
...
export interface GlassPatch {
   ...
   * 進去看到的是**機身的漆** —— 機腹玻璃與機身的連接處要像機背一樣切開成
   * 缺口。
   *
...
export function buildHull(
   ...
   * 那三件事合起來才讀得出「這裡凹進去了」。
   *
   * 玻璃與蒙皮**共面**的話 —— 只是同一批三角形換了材質 —— 低多邊形下那
   * 讀起來是一張貼紙，不是艙口。
   *
  ...
  /**
   * 【襯裡是碗底，不是內縮的玻璃】「玻璃那幾片往內縮 0.84」等於**跟著玻璃
   * 的形狀縮小一份**，而機背機槍座的玻璃是個隆起的罩子，縮小的複本於是也是
   * 個隆起 —— 那是凸起不是凹槽。
   *
   ...
   * 碗緣與罩緣接得起來，不必再補框壁。
   *
   * 【碗底一定要沉在**玻璃**之下，不是沉在蒙皮之下】寫成 0.985 ——
...
export function buildCockpitTub(
   ...
   *
   * 【繞向不可以跟機身同向】那會讓內裝的正面朝外、被機身擋住，而朝內的是
