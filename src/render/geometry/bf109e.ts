 ...
 * 螺旋槳直徑 3.10 m。
 *
 * 【為什麼外型是 E，而飛行模型不是】參考模型是 E-4，**外型維持 E**；
 * 飛行模型與武裝是 **K-4**（DB 605DC、MK 108），見 specs/bf109k4.ts。
 *
 * 這個檔案**刻意不跟著性能走**。要改外型是另一個決定，要先問過。
 ...
 * 座艙段補上加密的環。
 *
 * 【為什麼要在艙口前後各補一個環】開口的前壁是「最後一個完整的環」到
 * 「第一個被切開的環」之間那一段。最近的環在 z = 0.060、開口從 0.355 起
 * 的話，機背會從 0.823 一路斜降到艙緣 0.520，斜面長達 0.295 m，側面看就是
 * 機首後方塌下去一塊。z = 0.345 那個環讓前壁只剩 0.010 m，等於一道垂直的
 * 隔板。尾端同理補 1.805。
 *
 ...
 *
 * 前緣逐站差 1 mm。旁證：風擋底框落在翼弦的 36.5% —— 與 P-51D 量到的
 * 36.5% 完全一致，兩台真機的風擋都在這個位置。
 *
...
const BLISTERS: readonly Blister[] = [
   ...
   * 水平的方盒會內側陷進機翼、外側浮空，側面看得出一道楔形縫。
   *
   * 【長度與高度】機翼是翼型剖面，下表面只有在三成弦長處最低、往後
   * 逐漸抬起。方盒的頂面是平的，蓋不住這段起伏——0.95 長的盒子在
   * 後端會離開機翼 0.06 m，露出一條縫。0.65（涵蓋 28%～63% 弦長，
