...
const GLASS_PATCHES: readonly GlassPatch[] = [
   ...
   * ── 尾砲塔的玻璃艙 ────────────────────────────────────
   *
   * 專案負責人：「機尾還有一個玻璃艙」。B-17G 的 Cheyenne 尾砲塔在尾樑末端
   * 有一圈玻璃：兩側各一片給射手看側方，末端一片給瞄準具。
   *
   * 【量測】只打透明 mesh（`Object_50`），機體 z 14.4…16.8 每 0.1 一刀、
   * 逐 0.1 的 X 帶：
   *
   * ```
   *   機體Z   x0.20 的 y      x0.30 的 y      x0.00／0.10 的 y
   *   14.90   1.844…1.874    1.548…1.795          —
   *   15.30   1.906           1.462…1.825          —
   *   15.60   1.891…1.966    1.439…1.685      1.966
   *   15.90   1.695…1.859    1.392…1.398      1.916
   *   16.00   1.391           —                1.507…1.631
   * ```
   *
   * 真機那一段的玻璃坐在一個**比尾樑高出 0.3 的罩子**上（量到的 y 上界
   * 1.97，而尾樑頂只有 1.42）。那個罩子沒有做 —— 一個 0.3 高、0.8 寬的
   * 隆起在遊戲距離下看不出來，做進環裡反而會把尾樑撐胖（坑 20b：寧可少）。
   * 這兩片補丁只把玻璃貼在尾樑的**肩上與末端**，讀起來就是那個艙。
   *
   * 【索引 0…3】由中線一路包到肩膀。試過留中線那一格當背脊（索引 1…3），
   * 貼出來是兩條細長的窄帶，讀不出「艙」；真機那一段本來就是一整圈玻璃
   * 包住射手，包到中線才對。
   */
  { from: 14.85, to: 16.15, i0: 0, i1: 3 },
]
