 ...
 * **正上方那條射線與垂尾共面** —— Z 8.85 之後整段回 0。逐片查過也沒有任何
 ...
 * ```
 *
 * **對切報出的「垂尾前緣中位差 0.004」是假的**：參考那邊量到鋼索、自家
 * 那邊量到照鋼索造的垂尾。對切只能證明兩邊一致，證明不了兩邊都對。
 *
 * ══ 為什麼是一片，不是七片 ═══════════════════════════
 *
 * 拆成七片獨立的 `upright` 的話，**每一片都是一個封閉的實體**：相鄰兩片的
 * 翼尖端面與翼根端面完全重合，而且各自的厚度分佈獨立 —— 前緣配得再準，
 * 側視也是一疊有橫向接縫的階梯，人工回報就是「垂直尾翼好像沒有很平整」。
 *
 * **單一面板 + 七個轉折**（`WingParams.breaks`）：一個 mesh、沒有內部端面、
 * 厚度沿展向連續。
 *
 * ══ 量測來源：兩組互相印證 ═══════════════════════════
 *
 ...
 * 的。**取樣要 0.05 m，而且半寬不能讀窗口自己的寬度。**
 *
 * 【兩個坑，都是坑 6／坑 30 的同一件事】
 *
 ...
 * 乾淨到不像話。
 *
 * **（乙）`uWindow` 開太窄，半寬會變成窗口自己的寬度。** 太窄的窗是
 ...
 * 所以 −1.18／−0.98 由 −1.38（0.750）到 −0.78（0.642）內插，不然引擎側面
 * 會有一塊鼓包。
 *
 * 這一段大半被機翼蓋住，而上下輪廓（`halfHeight`／`centerY`）全程是
 * 量到的。
 *
 * ── 內外艙不是同一個形狀 ────────────────────────────────
 *
 ...
 * 尾段略粗一點 —— **不是一塊鼓包**，見下。
 *
 * ── 內艙腹線那 0.37 是放下來的主輪，不是鼓包（坑 15）──────────
 *
 * 內艙腹線在機體 z −1.8…−0.6 量到 −1.15、其餘各站 −0.78 上下。把那 0.37
 * 當成輪艙的鼓包烘進去的話，引擎下方會多一塊奇怪的突出。
 *
 ...
 * ```
 *
 * 13.7 之前那 0.02～0.05 是背線本身的偏差，不是凸起；13.8 開始爬，
 * **15.0 才滿高**。斜坡壓在 13.85…14.60（0.75 m）且前段只有 0.22 寬的話，
 * 看起來是「一個矮胖的方塊立在尾樑上」而不是由機背一路長上來的整流 ——
 * 它要向前延伸將近半個垂直尾翼。現在斜坡是 13.80…14.92（1.1 m），
 * 半寬一開始就給 0.16 → 0.30。
 *
 ...
 * 射手的玻璃艙。
 *
 * 【兩段都封口，而且**疊 0.02**】「前段不封後、後段不封前」會留下兩個開口
 * 的殼，而它們在 z ≈ 15 —— `geometry.test.ts` 的帶符號體積是對原點取的，
 * 開口的殼在那裡是一個很大的負數（見 `fuselage.ts` 封口那一段的註解）。
 *
...
const GLASS_PATCHES: readonly GlassPatch[] = [
   ...
   *
   * 寫成 `i0: 0, i1: 4`（90° 一路到 42° 連續）的話，機首上方整片會變成
   ...
   *
   * `GlassPatch` 是「z 範圍 × 環索引範圍」的矩形，做不出斜的邊界。拆成三片
   * 的話暗色襯裡逐片取弦會裂開；加一個 `sweep` 欄位的話邊界只能落在四邊形
   * 的格線上，斜邊是四階的階梯。兩種都比方帶醜。
   *
   ...
   * ── 機背的**兩片**頂窗：這裡只挖蒙皮 ──────────────────────
   *
   * **不可以做成一條 `2.47…4.34 × 索引 0…3` 的方帶**，那樣三件事都錯：
   *
   * ```
   ...
   *
   * 【挖到索引 2 為止，只有尾段退回索引 1】黑碗要連接到玻璃的邊框 ——
   * 只挖到索引 1（x 0.265…0.312）的話，0.92 m 寬的窗子兩側各
 ...
 * 看得出是兩片。只把 x 由 0 縮到 0.03、z 卻留在 x = 0 的值的話，上緣
 ...
 * 0.46 為止（索引 3 的 0.50…0.55 太寬）、後窗的尾端是**收尖的**。
 *
 ...
 * 開口、比罩口大 2% 的暗色管子」，照抄過來的話 `geometry.test.ts`
 ...
 * 照外框在蒙皮上鋪一片玻璃。
 *
 * 【格線要與蒙皮對齊，不能自己等分】橫向只切兩格（那一段的艙頂是平的，
 * 看起來切再細也一樣）會錯在**外框比平頂寬**：平頂只到索引 2（x 0.386…
...
export function buildB17G(): AircraftModel {
   ...
   * 薄板，下緣還從蒙皮戳出去 —— 駕駛玻璃內的黑碗要**內凹**。所以那幾片
   * 補丁掛了 `bare`（連襯裡都不生），暗艙由這裡按玻璃的框生。
   */
