 ...
 * ── 尾砲塔的玻璃艙罩 ──────────────────────────────────
 *
 * 專案負責人：「機尾還有一個玻璃艙 …… 機尾上方應該是一個高的圓角矩形凸起」。
 *
 * 【量測】只打透明 mesh（`Object_50`）與整台的頂線，機體 z 14.2…16.4：
 *
 * ```
 *   機體Z   x0.00  x0.10  x0.20  x0.25  x0.30  x0.40
 *   15.40   1.962  1.962  1.963  1.908  1.740  1.345
 *   15.60   1.966  1.966  1.966  1.922  1.685  1.332
 *   15.80   1.969  1.969  1.969  1.920  1.483  1.311
 *   15.90   1.971  1.971  1.820  1.346  1.398  1.268
 *   15.95   1.867  1.813  1.615  1.411  1.306  1.242
 *   16.05   1.394  1.347  1.326  1.316  1.281  1.048
 * ```
 *
 * 頂是一片**平的 1.97**、半寬到 0.35 為止（0.40 已經掉回尾樑），15.90 之後
 * 一路收掉。而那一段的尾樑頂只有 1.46～1.50 —— 所以它是一個高 0.5、寬 0.7、
 * 長 1.3 的**凸起**，不是尾樑自己。玻璃量到 y 1.44…1.97，整個罩子都是玻璃。
 *
 * 【z ≤ 15.33 那一段的頂線不能信】垂尾在 y 1.95 的半厚是 0.26，x ≤ 0.25 的
 * 帶在方向舵後緣（15.34）之前量到的 1.94～1.97 是**垂尾自己**。罩子的前端
 * 因此只能由玻璃那一組定（x0.30 在 14.9 就有玻璃），取 14.70 起。
 *
 * 【為什麼不烘進環裡】它比尾樑高 0.5、只有 0.7 寬 —— 烘進去等於把整圈撐胖
 * （與機腹吊艙、發動機艙同一個判準，見第 5b 步）。
 */
const TAIL_HOOD: LoftPart = {
  roundness: 3.2,
  segments: 16,
  sections: [
    { z: 14.70, halfWidth: 0.13, halfHeight: 0.15, centerY: 1.40 },
    { z: 15.00, halfWidth: 0.30, halfHeight: 0.32, centerY: 1.63 },
    { z: 15.30, halfWidth: 0.35, halfHeight: 0.36, centerY: 1.61 },
    { z: 15.70, halfWidth: 0.35, halfHeight: 0.36, centerY: 1.61 },
    { z: 15.88, halfWidth: 0.31, halfHeight: 0.33, centerY: 1.61 },
    { z: 15.98, halfWidth: 0.18, halfHeight: 0.20, centerY: 1.55 },
  ],
}

/**
 * 罩子裡的暗艙。**不能省** —— 45% 的玻璃罩後面沒有東西擋的話，從一側看穿到
 * 另一側就是天空（坑 24）。比罩子小一號、而且**前後各縮進去一節** —— 尾端與罩子
 * 同站的話，由正後方看是「玻璃的封蓋 + 暗殼的封蓋」兩個同心的圓角矩形，
 * 中間那一圈黑邊看起來像一個洞。法線仍然朝外，讀起來就是「玻璃後面有個
 * 暗艙」。
 */
const TAIL_HOOD_DARK: LoftPart = {
  roundness: 3.2,
  segments: 16,
  sections: [
    { z: 14.80, halfWidth: 0.16, halfHeight: 0.18, centerY: 1.46 },
    { z: 15.05, halfWidth: 0.26, halfHeight: 0.28, centerY: 1.62 },
    { z: 15.30, halfWidth: 0.30, halfHeight: 0.31, centerY: 1.61 },
    { z: 15.70, halfWidth: 0.30, halfHeight: 0.31, centerY: 1.61 },
    { z: 15.86, halfWidth: 0.24, halfHeight: 0.25, centerY: 1.60 },
  ],
}
...
export function buildB17G(): AircraftModel {
  ...
  // 尾砲塔的玻璃罩，與罩內的暗艙
  h.loft(TAIL_HOOD_DARK, h.dark)
  h.loft(TAIL_HOOD, h.glass)
