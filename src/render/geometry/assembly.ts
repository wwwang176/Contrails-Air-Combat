 ...
 * 機體組裝的共用鷹架 —— **不含任何機種的造型**。
 *
 * 【為什麼造型要一機一檔】共用一份 `Silhouette` 結構與一支 buildAircraft
 * 的代價：
 *
...
export interface FinParams {
   ...
   * 翼根的垂直位置。省略即 0。
   *
   * 【為什麼需要】機身後段的中心線是抬高的，垂尾若一律從 y=0 長起，下半截
   * 會埋在機身裡。等它從背線冒出來時，前緣已被後掠角帶往機尾 —— 109 可見
   * 的垂尾根部會落在全長 86.7%，設計值卻是 82%，看起來又小又靠後。
...
export interface HullSpec {
   ...
   * 沒有一條共用的推導規則對兩台都準。「取 |x| > 0.92 × 半翼展那批頂點的
   * 平均」那種啟發式對現在這兩台碰巧夠用，但下一台飛機（雙尾桁、大水平
   * 尾翼、橢圓翼）會把它默默弄壞，而壞掉的症狀是尾跡從機身中間冒出來，
   * 不會有任何錯誤。
   *
...
export function createHull(spec: HullSpec) {
   ...
   * 從機外看得到對側的骨架，而對側骨架給你看的正是它的背面 —— 單面材質
   * 下那些支架從背面看是透明的。
   *
   ...
   *
   * 【不可以做成一個被覆寫的閉包】`propeller()` 每次呼叫都把 `setPropSpin`
   * 整個換掉的話，對單發機沒差，但雙發時第二次呼叫會把第一具的驅動丟掉，
   ...
   * 看到的是它的**內側**，也就是背面。單面材質下背面被剔除，看起來就是
   * 側面內壁畫反了。`gl_FrontFacing` 會替背面翻法線，所以內壁
   * 照樣有明暗，不是一片死黑。
   *
   ...
   * 三角形邊緣的像素必然翻動（實跑 96 組有 82 組有 byte 差）。
   *
  ...
  const api = {
    ...
    glazedNose(
       ...
       *
       * **不可以把整組環往內縮一份做成暗色內襯** —— 縮小的殼從外面看就是
       * 罩子裡飄著一個小飛機。機頭要的是一片黑色剖面，垂直連接玻璃與機身。
       *
      ...
      if (patches?.length) {
         ...
         *
         * 【這一塊夠用，不要再加整圈的暗艙】它不是一張斜看會從邊緣漏過去的
         * 浮片：替每個補丁補一圈整圈的內殼，渲染出來**一個像素都沒變**
         * （把 `cockpitMat` 改成純紅重跑，機背罩內整片是紅的）。
         *
     ...
     * 暗色薄板，而且下緣還會從蒙皮戳出去 —— 駕駛玻璃內的黑碗要**內凹**。
     *
     ...
     *
     * 【`tipThickness` 是為 B-17G 加的】不給的話 `buildWingPanel`
    ...
    propeller(p: PropSpec, noseZ: number): void {
      ...
      // 【不可以做成貫穿直徑的長條】三根長條在畫面上是六片槳葉；真機 P-51D
