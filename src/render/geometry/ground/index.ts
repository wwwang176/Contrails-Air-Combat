...
export const GROUND_UNITS: readonly GroundUnit[] = [
  ...
  // 波爾塔瓦機場的四種
  {
     ...
     * 在畫面上只有 25 x 15 px（每個像素 35 個三角形）。低模一架 1,902 個
     * 三角形，24 架由 318,024 降到 45,648。
     *
     * 【畫面沒有變】`check_lod_silhouette.py` 在 25 px（就是投彈高度上的實際
     * 大小）拍六個方位逐像素比對，428 個覆蓋像素裡**差 4 個**；放大到 256 px
     * 才出現 1.99%。低模由 `tools/blender/build_b17g_lod.py` 從出貨的 GLB
     * 逐件重建。
     *
     * 【剩下的 1.19 ms 不是幾何】換上低模之後這 24 架仍然值 1.19 ms，而它們
     * 只剩全場 7% 的三角形 —— 那是 24 個 draw call 與填充率。再減面拿不到東西，
     * 下一步是把 24 架併成一批。
     *
