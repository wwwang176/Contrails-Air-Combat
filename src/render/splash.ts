...
/**
 * 抽到滿高要多久，s。12 m / 0.05 s = **240 m/s** —— 出水那一下是被打出來的。
 *
 * 【為什麼是秒數不是「佔壽命的比例」】寫成比例的話，想調下墜速度
 * 就得同時重算總時長與比例兩個數 —— 兩個旋鈕互相牽動。拆成兩段獨立的
 * 秒數之後，「上升快一點」與「下墜慢一點」是兩個互不影響的改動。
 *
 * 【上升與下墜的比是 1:14】要的是「快速拉高、慢慢落下」。0.1 / 0.4 那一組
 * 是 1:4，60 fps 下上升佔六幀 —— 讀起來是「長出來」而不是「被打出來」。
 */
export const SPLASH_JET_SECONDS = 0.05

/**
 * 從滿高落回水面要多久，s。12 m / 0.7 s = **17 m/s**。
 *
 * 【比自由落體快】12 m 自由落體要 √(2×12/9.81) = 1.56 s。水柱是一塊**實心
 * mesh**，掉得比重力慢會讀成一根柱子在下沉而不是一團水在塌。
 */
export const SPLASH_FALL_SECONDS = 0.7
...
export function createSplashes(capacity: number = SPLASH_CAPACITY): Splashes {
  ...
  // 一根水柱都還沒生就不必畫。InstancedMesh 建立時 count 等於容量
  object.count = 0
