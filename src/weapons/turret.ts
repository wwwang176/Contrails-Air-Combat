...
export interface Turret {
   ...
   * 不是這個點。三處都把這個點當成固定的管口的話，砲塔瞄右邊時整根槍會
   * 往左邊擺（實測 80° 時後膛甩到 x = −0.886，而管口一動也不動）。
   *
 ...
 * 腹部砲塔是 18 × 2 × 0.46875 = **16.875**。
 *
 * 【為什麼是 0.46875 而不是原本的 0.9375】試飛時 G4M 咬在後面兩秒就能
 * 打下一架 F6F-5。九九式二〇粍一號一門是 535 発/分 × 80，砍半之前等於
 * **三挺 B-17G 的 .50**，而一式陸攻背部與尾部各有一門：
 *
 * ```
 *              砍半前   砍半後      打死 F6F-5（hp 1270）
 *   一門 20mm    669       334       1.9 s  →  3.8 s
 *   兩門         1338      669       0.9 s  →  1.9 s
 * ```
 *
 * 【為什麼動倍率而不是只動九九式的 80】負責人裁定**全部轟炸機一起砍半**
 * —— 自衛火力該給的壓力是「你不能久留」，不是「你進來就死」，而那個分寸
 * 對三台是同一件事。只砍 G4M 的話 B-17G 與 He 111 的分寸就跟它分家了。
 */
export const TURRET_DAMAGE_SCALE = 0.46875
 ...
 * 而不是二十個砲手。
 *
 * 【stride 一定要是 `MAX_TURRETS`，不能是「這台有幾座」】用各機種自己的
 * 砲塔數當 stride 時，砲塔數不同的兩個機種**編號區間會重疊**。20 架
 * B-17G（8 座）加 20 架 He 111（5 座）共 260 座，實際只有
