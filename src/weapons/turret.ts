...
/**
 * 砲塔傷害的倍率。**專案負責人 2026-08-21 三次裁定。**
 *
 * ```
 *   1.00 → 0.50   試飛後：「轟炸機的機槍太痛」
 *   0.50 → 0.25   護送／攔截上線後：「可以把轟炸機的攻擊力再降一半」
 *   0.25 → 5/12   同日試飛後：「轟炸機的傷害 +2/3」
 * ```
 *
 * 【第三次是「加三分之二」不是「加到三分之二」】0.25 × 5/3 = 5/12 ≈ 0.4167。
 * 寫成分數而不是 0.4167，是因為那個小數看不出它從哪裡來 —— 而這一格的
 * 每一步都是一次裁定，不是一個調出來的數字。
 *
 * 【為什麼記成三行而不是直接記最後那個值】三次是三個不同場合的裁定 ——
 * 第一次是遭遇戰裡對上一群 B-17，第二次是護送任務讓四架轟炸機長時間貼著
 * 戰場，第三次是砍過頭之後往回加。記成一個數字的話，日後只知道「有人調
 * 過這個」，不知道是哪一種局面把它推到哪一邊。
 *
 ...
 * 腹部砲塔由 18 × 2 = 36 變成 **15**。
 */
export const TURRET_DAMAGE_SCALE = 5 / 12
