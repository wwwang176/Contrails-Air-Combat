 ...
 *
 * 【為什麼是切片而不是分類節點】解析 GLB 再挑出「機身」節點行不通：第三方
 * 模型有兩百多個節點，蒙皮、內裝、發動機、起落架、螺旋槳混在一起，靠幾何
 * 猜測分類並不可靠 —— 實測某一站位量到的半寬是 ±0.22，而被判為「機身」的
 * 節點包圍盒卻說 ±0.55。
 *
 ...
 *
 * 【axisU 是給發動機艙那種東西的】原點固定在 u = 0（機身軸心）對單發戰機
 * 夠用，但**掛在機翼上的發動機艙**在 u = ±3 m 外 —— 從機身軸心射出去的
 * 射線會先穿過機身、再穿過機翼，取到的「最外側」是機翼下表面或翼尖，
 * 不是發動機艙。
 *
 * 這與座艙罩那一招是同一個手法（`aircraft-from-reference` 第 2 步）：
 * **換一個射線原點，就能把被擋住的東西單獨量出來。** 那邊移的是 v，
 * 這裡移的是 u。
 */
function castRay(
