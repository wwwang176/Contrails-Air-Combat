 ...
 *
 * 【yaw：為什麼不自動判斷】「螺旋槳端的 X 延伸較大」這個判準會失效 ——
 * 109 那個模型的機尾也有寬達 ±1.31 的部件。判錯的疊圖是整台頭尾顛倒，
 * 而它讀起來像「機身形狀差很多」。
 *
 * 【yaw 是角度不是布林】He 111 H-6 那台模型的**長度在 X、翼展在 Z**，
 * 與另外兩台（長度在 Z）差 90°，不是 180°。
 *
...
const REFS: Record<string, RefSpec> = {
  ...
  // 【e4 不是筆誤】機種是 K-4，參考模型與造型都是 E-4。見
  // specs/bf109k4.ts 檔頭。
  bf109k4: { url: '/ref/bf109e4.glb', yaw: 180, pitch: 0 },
  /**
   * He 111 H-6。**量法與數字見下。**
   *
   * ```
   ...
   * 俯仰角的一階估計。自家模型出來之後要照坑 2 重量一次定案。
   *
   * 【不要用機身背線估】那樣估出來是 7~9°。後機身背線往上收是真的外形不是
   * 姿態 —— 與坑 2「不要用剪影目測」同一件事。
   *
  ...
  /**
   * B-17G-60-VE Flying Fortress。**量法與數字見下。**
   *
   * ```
   ...
   * **自家模型出來之後要照坑 2 重量一次定案**（那一套比的是「參考值 −
   * 本模型值」，需要自家模型存在）。
   *
...
function placeRef(): void {
   ...
   *
   * 【`raw` 必須在**轉向之後**量】排在 `rotation.y` 之前的話，量到的是
   * 模型**自己座標系**的 X 幅度。
   *
