 ...
 *
 * 【挖槽這條路不通】高度場一格 80 m，而薩勒河實際寬 50–80 m。**在 80 m 的
 * 格子上挖不出比 160 m 窄的槽** —— 挖了會被相鄰格點的內插填回去，水面反而
 * 被兩岸埋掉（實測：水面 36.5 m、河邊的地 37.5 m，只露出一條四十公尺的縫）。
 * 想挖得看得見就得挖成 240 m 寬，那不是河是運河。
 *
 * 所以水面**鋪在地表上**：每一點取當地地形的高度加一點餘裕。投彈高度看下去
 * 與挖出來的沒有差別；貼地飛過去會發現河沒有岸，那是這個解析度的代價。
 *
