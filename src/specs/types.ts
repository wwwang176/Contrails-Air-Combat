...
export interface AircraftSpec {
   ...
   *
   * 【為什麼是必填欄位而不是一份 id 白名單】寫成
   * `id === 'bf109k4' || id === 'he111' ? 'axis' : 'allies'` 的話，新機種
   * 漏掉的症狀是拿到錯的那一本名冊，不是錯誤，是一排讀起來怪怪的名字。
   * 必填欄位漏填是編譯錯誤。
   */
  faction: Faction
  /**
   ...
   *
   * 【戰鬥機不是一律 1000】spec §6.3 的那一條會讓**體型差被忽略**：
   * F6F-5 的正後方命中面積是 P-51D 的 1.42 倍，同樣的血量等於它天生脆
   * 29%，而 Hellcat 的招牌正好是耐打。
   *
   ...
   *
   * 【護欄】`specs.test.ts` 把每一格夾在 0.60～1.5，超出要負責人重新定值。
   * 不是 1.0 的每一格都要在該機種的註解裡寫出處。
   *
   * 【下限 0.60 是為 A6M5 五二型甲放的】那一台史實上沒有裝甲鋼板、沒有
   * 防彈玻璃、沒有自封油箱，一手史料見 `specs.test.ts` 那一條的註解。
   */
