 ...
 * `PILOT_G_POSITIVE` = 6.5，所以極限持續轉彎落在 (6.5−6)/(8−6) = **25%**
 * 強度 —— 舊註解記錄的原話是「看得到但還能打，正是要的效果」。
 *
 * 【2026-08-11 為什麼要搬】那個硬夾拿掉了（見 `control/limiters.ts`），
 * 過載上限變成結構極限 8 G（Bf109 7.5）。門檻若留在 6/8，P-51D 每一次
 * 極限轉彎都會是 **100% 全黑** —— 等於用「看不見」換「轉得緊」，把一個
 * 手感改動變成視野懲罰。
 *
 ...
 * 看得到」這個**意圖**；黑視該不該變得更兇是專案負責人的裁定。另一個
