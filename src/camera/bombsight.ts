 ...
 * 投彈視線的圓錐半角，弧度。
 *
 * 【上界是自己的機體】再開下去相機會掃到機身與機翼 —— 那是窗口的邊緣，
 * 不是可以再借的視野。
 *
 * 【它只在低空作用】量測 B-17G 巡航 90 m/s：落點離天底的角度在 4,000 m 是
 * 29°、1,000 m 是 50.5°，**508 m** 起才達到 60°。全速度域的門檻 ——
 * 60 m/s 起於 236 m、90 m/s 508 m、110 m/s 733 m、130 m/s 982 m。低於那個
 * 高度圓圈會離開畫面中心，再低就滑出畫面。
 *
 * 【軸是機體固定的】等於「機腹上的一個窗口」：側滾大了看到的就是天，機動中
 * 投不了彈。陀螺穩定的瞄具不會有這個行為。
 */
export const BOMB_CONE_HALF_ANGLE = (60 * Math.PI) / 180
