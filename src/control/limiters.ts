 ...
 * 飛行員持續耐 G 的參考值。
 *
 * 【它不是硬夾】`pitchRateLimit` 拿它當俯仰率上限的話，飛機在 6.5 G 就
 * **拉不動了**。那是錯的機制：真實的飛行員不會被擋住，他會拉過去然後眼前
 * 發黑 —— 硬上限是**結構**（P-51D 8 G、Bf109 7.5 G），生理極限是一個漸進
 * 的懲罰，不是一道牆。
 *
...
/**
 * `'pilot'` **從來不會產生** —— 生理極限以黑視呈現，不夾過載（見
 * `PILOT_G_POSITIVE`）。這個變體留著是為了將來若要重新引入生理夾制時有
 * 現成的標籤；目前任何讀到它的程式碼都是踩到死路。
 */
...
export function pitchRateLimit(
  ...
  // 【結構是唯一的硬上限，生理不是】不要再加一個拿 PILOT_G_POSITIVE=6.5
  // 夾的分支：6.5 低於兩款機體的結構極限（8 / 7.5），結構分支會變成死碼，
  // 而飛機在 6.5 G 就拉不動了。飛行員拉得過去，代價是黑視
  // （`hud/widgets/gEffect.ts`），不是操縱面突然不理他。
