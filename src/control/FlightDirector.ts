...
export interface DirectorGains {
   ...
   * 路徑的 0.146 s 更快，且一開始就朝正確方向。權限上限 0.5 換算是
   ...
   * （pitchOuter 由 2.0 加到 4.0，尾巴照樣是指數）。
...
export const DEFAULT_DIRECTOR_GAINS: DirectorGains = {
  rollOuter: 3.0,
  // 【為什麼調的是俯仰外環而不是內環或滾轉】要的是「末段更敏捷、容許微幅
  // 過衝」，而大幅度機動根本不受增益支配：
  ...
  // 要的是「飛機很靠近或穿越準星之後才開始回正」。
  // 【它與 `pitchOuter` 解決的不是同一件事】外環增益改善的是脫離限制器
  // 之後的末段速度，外環積分改善的是 0.1° 以下的尾巴。而「像 LERP」的
  // 觀感發生在**接近過程**：desiredQ = pitchOuter × 垂直誤差 是比例
  ...
  // 【要加大過衝就得增益與上限**一起**加倍】只加倍增益不會加倍過衝——
  // 實測現行設定已有 6~8% 的步數卡在積分上限，
