 ...
 *
 * 【上限 1.8】單一音源以 150 m/s 接近是 1.78，剛好進得來（那一側本來就比
 * 遠離側強得多，一次掠過的落差三分之二在接近時）。超過這個的只剩正面對進，
 * 那時徑向分量等於合速，照實算會衝到 8 倍。
 *
 * 【下限 0.5】單一音源就算以 200 m/s 遠離也只到 0.63，咬不到；咬得到的是
 * 交會後兩架往反方向飛開（0.26）。
 *
 * 【夾制也是最後一道防線】鏡頭瞬移、分母變號（音源超過音速）算出來的怪值
 * 都由它接住。
 */
const DOPPLER_MIN = 0.5
const DOPPLER_MAX = 1.8
