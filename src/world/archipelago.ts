 ...
 *
 * 【為什麼沒有 seed 參數】`main.ts` 只有一種群島。公開一個沒有需求的
...
/**
 * 島的三個級距。**要混**：少數大島當戰術核心，多數小島當景。
 */
const TIERS = [
  { count: 0, radius: [1300, 1600], peak: [400, PEAK_MAX] },
  { count: 6, radius: [500, 800], peak: [150, 250] },
  { count: 40, radius: [ISLAND_MIN_DIAMETER / 2, 260], peak: [30, 80] },
] as const
 ...
 * ```
 *
 * 【為什麼需要它們】戰場在原點附近，而隨機擺出來的那批島最高只有
 * tier-1 的 250 m，多半離原點又遠 —— 那對 600 m 的飛機不構成障礙，AI
 * 正確地直接飛過去，地形感知在真實的仗裡一次都跑不到。地形要進得了場，
 * 交會區就得有真正的山。
 *
 ...
 *
 * 【通道 1,608 m】在 `CHANNEL_MIN` 之上 —— 那個常數要的就是這個情境：
 ...
 * 只是在跟自己的護欄擦邊。
 *
 * 【動了位置就要重量】高度、規模、繞島佔時三者互相牽動。錨島與甲板開場
 * 的出生帶同一個量級，移動之後要重跑 `test/integration/terrain-in-play.test.ts`
 * —— 出生在山裡就是開局直接墜機。
 */
const ANCHORS = [
  { cx: 2500, cz: -950, radius: 1400, peak: 450, pa: 0, pb: 0, seed: 11 },
  { cx: -2500, cz: 950, radius: 1500, peak: 425, pa: 2.1, pb: 4.3, seed: 29 },
] as const
 ...
 * 兩座島在畫面上完全是圓錐。
 *
 * 【0.08 有多少】tier-2 的高度是 30～80 m，所以是 2.4～6.4 m；錨島
 * （450 m）是 36 m，而上限 `LOBE_LIFT_MAX` 是 100 m —— 兩者之間就是大島
 * 那批瓣的變化範圍。小島那批由 `LOBE_PEAK_MAX` 收尾。
 *
 * 沒有這條下界時，露出量的中位數不到峰高的 1%，最小的島在畫面上是圓錐。
 */
 ...
 *   掠過的剖面變高      爬不過去所以要轉 → 山擋得住路，飛機會繞島
 * ```
 *
 * 【這是量到的相關，不是驗過的因果】上面那張高度表與下面那張對照表都是
