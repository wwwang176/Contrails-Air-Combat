 ...
 * 確實會多知道事情。漏判的**量級**約是 `坡度 × 步長 / 2`。
 *
 * 【那是估計，不是上界】它沒有算進視線自己的仰角，也把解析的徑向坡度直接
 * 套到三角化之後的面上（面的梯度會同時吃 x 與 z 兩個方向）。下面那張表要
 * 當量級看，不要當保證。
 *
 * ── 【真正的坡度】────────────────────────────────────────────
 *
 * **不要用「坡度 ≤ 0.5，漏判 5 m 以下」那個數字** —— 剖面最陡處是
 * `1.5 × (peak − SEA_FLOOR) / (radius × wobble最小)`，而最陡的從來不是
 * 錨島，是最小的那一批島。
 *
 * ```
 ...
 * 換那 54 m 是負責人的決定，記在 `docs/backlog.md`。
 */
