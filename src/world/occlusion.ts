 ...
 * 不是錨島（1.37）是最小的那一批島（tier-2，160 m 高、150 m 半徑：2.37）。
 *
 * ```
 *                   最陡坡度    losBlocked（步長 20 m）   landHitT（步長 10 m）
 *   多瓣之前          2.37            24 m                    12 m
 *   多瓣之後          4.79            48 m                    24 m
 * ```
 *
 * 多瓣把它推高一倍，而上界是 `archipelago.ts` 的 `LOBE_SLOPE` 給的 ——
 * 沒有那一條的話小而尖的瓣可以再高三倍。
 *
 * **步長沒有跟著調**：這一層漏判的代價是 AI 多開一輪空槍（彈丸那一側仍然
 * 會收掉子彈），而 `LOS_DIVISOR` 在 20 Hz × 40 架的熱路徑上。要不要用 CPU
 * 換這 48 m 是專案負責人的決定，記在 `docs/backlog.md`。
 *
 * **沒有取樣數上限。** 加了上限等於讓實際步長超過標稱步長，那會直接推翻
 * 上面那句「漏判 ≤ 5 m」。長度由呼叫端保證 —— AI 的有效射程約 1 km，
 * 也就是 50 個取樣點。
 */
 ...
 * 10 m 的步長把漏判壓到約 24 m —— 見 `losBlocked` 那張表，數字是量出來的
 * 而不是假設的。
 */
