 ...
 */
export const DAMAGE_STRIDE = 6

export interface DamageEvents {
  readonly capacity: number
  /**
   * 每筆 `DAMAGE_STRIDE` 個 float：受害者索引, dx, dy, dz（來彈方向，世界座標
   * 單位向量）, 部位序號（`HIT_PARTS` 的索引）, 射手索引（沒有射手是 −1）。
   *
   * 【部位與射手是給音效用的】打在護甲厚的部位要比較低沉（`audio/curves.ts`
   * 的 `hitRate`）；而「我打中了誰」要靠射手索引過濾出來 —— 同一套曲線，
   * 被打中看自己那架，打中敵機看對方那架。受擊方向指示器只讀方向那三個。
   */
...
export function pushDamage(
  e: DamageEvents, victim: number, dx: number, dy: number, dz: number, part: number, shooter: number,
): void {
