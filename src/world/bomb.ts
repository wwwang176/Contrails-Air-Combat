 ...
 * 池子大小。
 *
 * 【池滿會吃掉空中的炸彈】寫入是環狀的，第 capacity + 1 顆蓋掉最舊的那一顆
 * —— 還在落的炸彈就從空中消失，不爆也不報錯。
 *
 * 【160 怎麼來】要裝得下一場裡所有飛機同時掛著的彈：盟 M2 是 12 架 B-17 ×
 * 10 顆 = 120，1,500 m 投下要十幾秒才落地，一輪齊投時全部同時在空中。
 * 逐關的檢查在 `campaigns.test.ts`。一顆一格 float，相對
 * `PROJECTILE_CAPACITY = 4000` 可以忽略。
 */
export const BOMBS_CAPACITY = 160
