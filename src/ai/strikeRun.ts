...
export interface StrikeProfile {
   ...
   * 兩支就是解兩次，而它們問的是同一條軌跡。
   *
   * 【為什麼鎖定距離不是常數】它必須等於「放手點的距離 ＋ 一段穩定用的
   * 長度」，而放手點隨高度與目標的動向變：前拋距離 1,000 m 約 1.26 km、
   * 4,000 m 約 4 km，再加上目標在飛行時間裡沿視線靠近的量。寫死的話高空的
   * 轟炸機進到那個距離時**早就飛過投彈點了**，落點誤差一路單調增加
   * （實測 4,000 m 寫死 3 km：503 → 1101 → 1694 → 2283 m）。
   *
   * 【它必須是一個固定的接戰距離】隨著飛機接近而一起縮的量不能拿來當閘門
   * —— 鎖定條件會永遠差一點點而不成立（`ai/torpedoRun.ts` 記過那個實測）。
   *
