 ...
 * AI 戰鬥機點放的起始值。**由試飛裁定。**
 *
 * ── 為什麼有這一層（專案負責人 2026-08-21）──────────────
 *
 * 「AI 的戰鬥機也要跟轟炸機的機槍一樣，會有冷卻時間（只是頻率可以高一點）」。
 * 在這之前 AI 只受 `shouldFire` 的四條幾何條件節制 —— 只要解成立就**一路
 * 按著扳機**，而砲塔早就有點放（`weapons/burst.ts`）。畫面上的差別是一條
 * 不斷的曳光線 vs 一串。
 *
 * ── 「頻率高一點」高多少 ────────────────────────────────
 *
 ...
 * 扳機」—— 86% 是唯一還留在門檻內的檔位。
 *
 * 【`rearShare` 在 33.5 … 36.6 之間亂跳，不是這個參數的單調函數】與
 * 2026-08-07 掃描 `turnTimeScale` 得到的結論同一型：那是混沌雜訊。它在
 * 點放**關掉**時就已經是 33.5%（門檻 35%），餘裕本來就只有 1.5 個百分點。
 *
 * 【要更慢的節奏就要重新定值兩條門檻】0.3 s 的停頓在畫面上更明顯，但
 * `rearShare` 與 `fireShare` 都會紅 —— 那是專案負責人的決定，不是這裡
 * 可以自己放寬的東西。
 *
 * 【玩家不受影響】玩家扣扳機走 `World.fire`，完全不經過 `AiController`。
 * 代飛（AI 接手玩家那一架）時會受影響 —— 那時開火的本來就是 AI。
 * 專案負責人 2026-08-21：「AI 代飛的時候也要有這個機制，但代飛結束要改
 * 回來（人接手的時候不需要冷卻）」——`main.ts` 換的是 `player.controller`
 * 這個參考，所以那件事是換控制器的結果，不是一段要維護的程式碼。
 */
