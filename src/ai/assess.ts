...
export interface Situation {
   ...
   *
   * 【它不可以限縮成「當前目標」】`threatFactor(target, self)` 只看得見自己
   * 正在打的那一架，而 20v20 實測長機被鎖定的時間裡有 **97.8% 的鎖定來自
   * 不是它目標的敵機** —— `threatInstant` 於是恆為 0、`defend` 結構上不可能
   * 觸發：被鎖定的 48 段裡只有 1 段閃過，**4843 點傷害 100% 是在沒有閃躲
   * 的狀態下吃的**，畫面上就是「AI 好像不太會閃」。
   *
 ...
 *
 * 【它是選目標的「代價」項】只問「這架敵機有多值得打」而不問「我要花多少
 * 代價才打得到」時，43% 的新目標在後半球，其中只有 1.7% 咬得到 —— 那
 * 560 秒總共只開了 0.6 秒的火（spec §3.3）。
 *
 * 【角度由速度向量量，不是機首】
 *
 * 這個函數**只被選目標消費**（`target.ts` 與 `wingman.ts`），而選目標是
 * 慢決策 —— 10 Hz 評估、最少停留 1~2 秒。拿瞬時機首當基準等於把一個
 * **一秒能甩 75° 的快變量**餵進慢決策。
 *
 ...
 * 機首」這條規則（見 `turnTime` 的註解）自動適用於兩邊 —— 不會出現「轉向
