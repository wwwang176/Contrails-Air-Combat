...
export const DEFAULT_STEER: SteerConfig = {
  ...
  // ## defendTilt 20° → 31°（P-51D 改用試飛重量之後重掃）
  //
  // 【為什麼要重掃】質量由 3,900 校準值換成 4,427 kg 的試飛重量之後，P-51D
  // 的氣動可用過載由 11.63 掉到 10.43 G，破防的角速度跟著變小。四條行為
  // 護欄同時紅：**四條都只跟質量有關**（把質量退回 3,900、其餘修正全留，
  // 四條全綠）。所以要動的是 AI 的動作幅度，不是機體資料，也不是門檻。
  //
  // 【為什麼是這一項】它同時對得上那四條：抬角加大既讓預瞄點的垂直位移
  // 變大（「看得出在閃」），又減少破防時往海面掉（安全層介入）。
  //
  // ```
  ...
  //
  // 【試過但不行的另一個旋鈕】`defendOffset` 75° → 85°（tilt 留 20°）：
  // 閃躲角修好了（2.62°），但後上方 400 m 反而更糟（96.2%、還是被打死），
  // 而且一樣打破戰術對照。偏轉角改的是「往哪邊閃」，那一格缺的是「別掉高度」。
  //
  // 【對別的機種】這是共用參數，109 也照吃。20v20 的重播校驗和因此改變
  // （已重錄，雙跑逐字相同）。
