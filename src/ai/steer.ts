...
export interface SteerConfig {
   ...
   * ```
   *
   * 行為品質（安全層佔時、最長單次 extend、陡航跡佔時）七格都沒有方向性，
   * 傷害卻在相鄰格之間差 3~5 倍、勝負翻號。
   *
   ...
   *
   * ```
   * speedRecover / unload   高能量開局      低於失速的佔時最差（@4000 三場）
   ...
   * 「紅 305 : 藍 0」。行為品質也幾乎相同，唯一的差別是 1.55 讓安全層
   * 佔時由 0.75% 升到 1.75%（往壞的方向）。
   *
   * **這個旋鈕在現行包絡下沒有影響力。**
   *
   ...
   *
   * 【它只作用在能量戰】1v1 機動六場（P-51 對 P-51、TAS 200 起始）
   * 在 1.6 與 1.8 之間**逐位元相同** —— 那六場衝不到那個速度，規則一次都
   * 沒觸發。變的只有高能量開局，而那正是這條規則唯一會觸發的地方。
   *
   * 【往下調是死路】1.20 讓最長單次 extend 拉長到 56.5 s（現值約 48 s），
   ...
   *
   * **15° 待人工試飛定案。** 已知代價：1v1 機動側舷 @1000/@4000 的最長
   * 單次 extend 拉長到 75.5 / 70 s。換到的是最大半徑由 10.7 km 降到 6.2 km。
   */
   ...
   * 一起堵死 —— 距離永遠到不了 1,500 m，轉彎又補不回能量。實測 1v1 機動
   * 側舷 @1000 m 的最長單次 extend 暴增到 **284.5 s** —— 300 秒的場次有
   * 284 秒在脫離，那不是空戰，是繞圈。**這個機制修的是「直直飛不合理」，
   * 不是攻擊效率。**
   *
   ...
   *
   * 實測六場 1v1 開局低於失速速度的佔時全部落在 0.08%，這個增益不需要再調。
   *
   * 【現值離懸崖有兩格】
   *
   * ```
   *        高能量開局              共速共高        最長 extend  陡航跡佔時
   ...
   * ```
   *
   * **6×EP 是明確的懸崖**：最長單次 extend 由 48 秒暴衝到 96.25 秒，而且
   * 高能量開局的傷害交換**翻號** —— 藍方單方面挨打。
   ...
   * **目前的場景一個都量不到它。** 掃過兩組：
   *
   * 一、高能量開局與共速共高兩種 1v1 開局：1~4 倍逐位元相同。
   *
 ...
 * 手感倍率一動分母同步跟著走 —— 持續迴旋率動過 +87%，這一層的行為沒有
 * 變壞。
 *
...
export const DEFAULT_STEER: SteerConfig = {
  ...
  // 10.43 G，破防的角速度因此偏小：閃躲看不出來、被咬住甩不掉、破防時往
  // 海面掉。抬角加大同時治這幾件：既讓預瞄點的垂直位移變大（「看得出在
  // 閃」），又減少破防時往海面掉（安全層介入）。
  //
  // ```
  //   tilt  正下方閃躲  後上方400m  400m掉血  安全層  其他退步
  //   20°     1.73°       93.2%     1000死   3.04%   —
  //   30°     1.97°       31.0%      214     0.64%   —
  //   31°     2.15°       30.7%      304     0       —              ← 出貨
  //   32°     2.31°       30.8%      326     0       戰術效果 ×2
  //   33°     2.37°       30.5%      317     0       shootableShare
  //   34°     2.47°       30.7%      326     0       撞海 + 戰術
  //   36°     2.67°       30.7%      614     —       —
  //   40°     2.85°       30.5%      815     —       —
  // ```
  //
  // 【窗口很窄，這件事本身要記住】31° 是**唯一**每一項都沒有退步的值。
  // 三個方向各自把它夾住：
  //
  ...
  //
  // 中間的 32° 與 33° 各自讓一項 20v20 的混沌聚合量退步（戰術開關對照、
  // shootableShare）。**所以不要把這個值當成可以隨手微調的旋鈕**：任一方向
  // 動 1–2 度就會有東西退步，而且退步的不是同一項。
  //
  // 【不要改用 `defendOffset` 代替】偏轉角改的是「往哪邊閃」，而這幾格缺的
  // 是「別掉高度」。把它放到 85°（tilt 留 20°）閃躲角是修好了（2.62°），
  // 但後上方 400 m 反而更糟（96.2%、還是被打死），而且戰術對照一樣退步。
  //
  // 【對別的機種】這是共用參數，109 也照吃 —— 動它等於同時改掉所有機種的防禦機動。
