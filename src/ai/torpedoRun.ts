 ...
 * 會短暫地指對方向**：一台 G4M 帶著坡度進入直飛段、一邊改平一邊讓解掃過去
 * 時，誤差會在還沒改平的那幾拍穿過艦體正中，而等姿態真的收乾淨時誤差已經
 * 長到窗（`hitWindowOf` 的 across 只有幾公尺）的好幾倍。用包絡當鎖的閘門，
 * 鎖的就一定是投得出去的那一拍。
 *
 ...
 *
 * 【為什麼要更緊】直飛段能用來修正航向的坡度受 `TORPEDO_ENVELOPE.maxRoll`
 * 限制，鎖得鬆就是鎖在一個修不回來的航向上。**起始值，由試飛裁定。**
 */
