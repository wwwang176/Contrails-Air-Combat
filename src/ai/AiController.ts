 ...
 * 追擊時允許沉到敵人下方多深，m。
 *
 * 【為什麼與 `FLOOR_BUFFER` 分家】兩條線守的東西不同：沉到**被護送者**
 * 下面是失職（緊，100）；沉到**敵人**下面 150~300 m 是低位 yo-yo ——
 * 教科書的纏鬥動作，切進圈內側換速度抄近路。共用 100 的帶寬時，跟著敵人
 * 迴旋的每一次 yo-yo 都觸發 extend(高度) 拉高 1~3 秒，一拉高敵人就飛走、
 * 咬住的尾就丟了。這條護欄要擋的是持續深潛（實測鑽到編隊下 280 m 不
 * 回頭），不是三秒的戰術下沉。
 */
...
export class AiController implements Controller {
   ...
   * （「閃躲永遠優先」）。那對戰鬥機是對的 —— 撤退途中被咬住還硬飛就是
   * 送死。
   *
   * 對**被護送的**那幾架不是：轟炸機被瞄準就滾轉不合理，它需要一個無條件
   * 移動到集合點的行動狀態。實測值印證那個「不合理」：接觸之前滾轉恆為 0.0°，
  ...
  update(self: Aircraft, dt: number, out: Command): void {
    ...
    // ── 轟炸機的對艦排在空中接戰與站位之前 ────────────────────
    //
    // 【為什麼要插在這裡】對艦分支若包在 `if (!target)` 裡、站位又排在它
    // 前面，實跑 `japan-m4` 48 步，五架 AI 一式陸攻的 `shipAim.ship` 全是
    // −1：長機選了空中目標，僚機都有站位參考。**整個功能會靜靜地不動作。**
    //
