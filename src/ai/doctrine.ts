 ...
 * 同機種對打（例如 P-51 對 P-51）會完全失去紀律。見 spec §3.3。
 *
...
export interface DoctrineConfig {
   ...
   *
   * 【0.35 也已經在靶子區】被敵機從後下方 400 m 咬住時，0.35 的藍方甩不掉
   * 對方、一路挨打。這個下限直接決定「能量見底時還轉不轉得動
 ...
 *
 * ## 為什麼不拿 20v20 的目標選擇指標定值
 *
 * 那一組**解析不出這個尺度的改動**。用一個與打法無關的等量擾動（無條件把
 * 拉桿係數乘 0.99）跑同一場 20v20：
 *
 * | | rearShare | fireShare | holdMedian |
 * |---|---|---|---|
 * | 基準 | 0.3260 | 0.0417 | 1.400 |
 * | 無意義的 ×0.99 擾動 | **0.3640** | 0.0326 | 1.500 |
 * | 本層（minPull 0.65） | 0.3615 | 0.0295 | 1.400 |
 *
 * 無意義的擾動造成的偏移**比本功能還大**。掃描值對它也非單調（0.65 →
 * 0.3615、0.75 → 0.3248、0.85 → 0.3089）。拿它定值等於擬合單一顆種子的
 * 軌跡發散。
 *
 * 防禦場景則是**解析得出來**的：×0.99 擾動下後下方 400 m 的掉血是 0（與
 * 基準相同），而本層未修正時是 605。所以定值用它。
 */
