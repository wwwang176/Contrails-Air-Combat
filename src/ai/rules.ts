...
export interface RuleConfig {
   ...
   *
   * 【定值由實測掃出】高能量開局的「能量優勢方不得超支」與 1v1 機動的
   * 最長單次 `extend` 一起掃：
   *
   * ```
   * enter / exit   花掉的比能量（開局優勢 3213）   最長單次 extend
   ...
   *
   * ```
   * enter/exit    1v1 機動的安全層佔時                  閃避可見威脅的場景
   *  0.75/0.95    全部 < 5%                             正常
   *  0.65/0.85    全部 < 5%                             三項退步，其中一項
   *                                                     是動用了安全層
   *  0.55/0.75    側舷 @4000 **65.6%**、對頭 @4000 16.2%  三項退步
   * ```
   *
   * 0.55/0.75 下**安全層佔了 65.6% 的時間**。失速數字好看是因為安全層一直
   * 在替它飛，那不是健康。0.65/0.85 是較輕的版本：1v1 機動看不出來，閃避
   * 可見威脅時就開始靠安全層。
   *
   ...
   *
   * ```
   *   20v20 fireShare          0.0160 → 0.0256
   *   20v20 onNose             0.171  → 0.192
   *   護送關 extend 離場長尾    1,874  → 573 m
   *   護送關 extend 進場次數    90     → 58
   *   20v20 holdMedian         2.00   → 1.40     變差
   *   20v20 rearShare          0.312  → 0.354    變差
   * ```
   *
