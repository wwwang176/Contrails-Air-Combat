 ...
 * 難度參數。
 *
 * 【基準是天花板】兩個欄位的基準值都是 0，也就是「零延遲、零瞄準誤差」，
 * 難度由此往下調（spec §10）：**不可能從一個沒量過的天花板往下調難度**。
 *
 * 【視野盲區不在此列】它需要「最後已知位置 + 搜索行為」，是一整套行為而
 * 不是一個參數。明確延後，記在 spec §2 以免被當成遺漏。
 *
 * 【`aimError` 沒有人讀】`reactionDelay` 接在 `ai/delay.ts`，`aimError`
 * 沒有接。它是另一個旋鈕，沒有量測需求驅動，刻意不做 —— 寫在這裡以免被
 * 當成遺漏。
 */
