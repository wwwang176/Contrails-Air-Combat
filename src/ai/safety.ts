...
export interface SafetyConfig {
   ...
   * 不作為判準。
   *
   * **上界 —— 六場 1v1 機動（P-51 對 P-51、300 秒）**
   *
   * 只有側舷@1000 有反應，其餘五場逐值不變：
   *
   * ```
   * lookahead   最長單次 extend   機首偏離目標 > 90° 的佔時
   * 0                23.8 s              47.7%
   * 0.25             34.3 s              54.9%
   * 0.50             42.0 s              60.0%
   * 0.75            285.1 s              96.3%
   ...
   *
   * 可用區間是 0.25~0.50。取小的：離那道懸崖遠一點，而且低空最低高度也比
   * 較好（46 對 16 m）。
   */
  lookahead: number
}
...
export const DEFAULT_SAFETY: SafetyConfig = {
  ...
  // 【0.25 s 是兩組掃描夾出來的，見 `lookahead` 欄位的註解】上界由六場 1v1
  // 機動給（≥ 0.75 時側舷@1000 會炸開），下界由「不觸海」給（0 會觸海）。
  lookahead: 0.25,
  overspeedRatio: 0.90,
 ...
 * 兩件無關的事：四千公尺高空的介入不可能是撞地，是失速接管，卻會被算進
 * 「差點墜海」。
 */
...
export function applySafety(
  ...
  if (margin <= needed) {
    ...
    // 觸發」是結構保證的，不是一個要靠人維護的約定 —— 介入的頻率因此不會
    // 高過撞地接管本身。
    //
