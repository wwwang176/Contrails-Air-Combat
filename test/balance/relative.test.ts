 ...
 *
 * 【成因：不是出力檔不對等，是 P-51D 的爬升模型上不去】兩架**都**在緊急
 * 出力上，而且都對：
 *
 *   P-51D  V-1650-7 @67″ WEP   1,490 HP 海平面 / 1,720 HP @1,900 m
 *                              1,290 HP        / 1,370 HP @5,900 m（二檔）
 *   109K-4 DB 605DC @1.98 ata  2,000 PS 海平面 / 1,800 PS @6,000 m
 *                            = 1,973 HP        / 1,775 HP（換成同一個單位）
 *
 * 真正的成因是海平面爬升，而它**早就被記錄、也早就裁決過**（見
 * `specs/bf109k4.ts` 與 `performance/historical.test.ts` 的檔頭）：
 *
 *   K-4    1,452.9 對史實 1,470    −1.16%   ← 守死
 *   P-51D    907.0 對史實 1,060   −14.45%   ← **本模型物理上達不到**
 *
 * 於是 K-4 對 P-51D 的爬升優勢比史實再多 **15.5%**，升限與高空能量跟著一起
 * 翻過來。`historical.test.ts` 為此開了一條 P-51D 專用的 `[−15%, −12%]`
 * 區間，並註明理由 —— 那是 2026-08-25 專案負責人「海平面爬升要跟上史實」
 * 那次裁決的已知代價，不是調參失手。
 *
 * 【要復原比值的話】把 K-4 的 `prop.vRef` 由 50 調回 55.7：爬升比回到 1.512
 * （史實 1.3868），但 K-4 自己的爬升變成 −6.70%，`historical.test.ts` 的
 * ±5% 會紅。要完全對上比值需要 vRef ≈ 71，爬升會掉到 −19%。**兩者不可兼得**，
 * 而現況是「K-4 對史實」勝過「兩機比值對史實」的那一邊。
 *
 * 【一段錯誤的更正】本檔案 2026-08-27 第一版把成因寫成「P-51D 沒開到 WEP、
 * 兩架不在同一個出力檔上比」。那是錯的：當時把瓦特值用 PS（735.5 W）換算，
 * 而 `p51d.ts` 用的是 HP（745.7 W），又拿 P-51 的**海平面**值去比 109 的
 * 海平面值 —— V-1650-7 的 WEP 峰值本來就在 1,900 m 而不是海平面。
 */
