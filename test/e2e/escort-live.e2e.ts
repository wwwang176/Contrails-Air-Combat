 ...
 *
 * 【它量的不是護送】「護送 He 111」那張卡在 12 關改版
 * 後那張卡沒了。現在指名 `germany-m1`：玩家在**攔截方**，
 ...
 * 固定步長迴圈撞到 `maxSubsteps` 會丟時間。回報的症狀發生在**遊戲裡**
...
async function main(): Promise<void> {
  ...
  try {
     ...
     * 同名環境變數是同一套值，A/B 才對得起來。消融掉迴旋平面的那一組是
     *
     ...
     */
    // 【用 id 選卡，不要找標題】找標題含「He 111」的卡的話，那張卡在 12 關改版
    // 時就沒了 —— 這支從那天起就是壞的。改指名 `germany-m1`
    // （攔截 B-17，德軍唯一還有 convoy 的卡）。**量的是攔截不是護航**：不是護航 AI
    ...
    //
    // 【錨點用交會而不是「掉到轟炸機下面」】後者正是要修掉的行為，拿它
    // 框窗的話**修好之後窗就框不出來、摘要變成空的** —— 成功反而讀不到
    // 數字。交會不論護航做得好不好都會發生。
