...
describe('多瓣的島', () => {
  ...
  it('參數空間的四個角落，三條不變式都成立', () => {
    // TIERS 的兩端加上錨島的尺寸
    const SIZES = [[150, 30], [150, 80], [260, 80], [500, 150], [800, 250],
      [1400, 450], [1500, 425]] as const
    const ENDS = [0, 0.5, 1]
  ...
  /**
   * 【起伏真的存在】單瓣的島沿任何一條半徑走出去高度都是**嚴格遞減**的 ——
   ...
   * 重跑 `terrain-in-play`** —— 甲板開場高度與錨島同一個量級，錨島一變，
   * 出生點的離地餘裕就要重驗。
   */
  it('錨島的瓣是常數', () => {
    ...
    expect(shape(0)).toBe(JSON.stringify([
      [0, 1400, 450], [1165.45, 457.76, 153.67], [1001.22, 562.82, 209.38],
      [1123.71, 476.52, 197.98], [1047.06, 487.03, 208.38]]))
    expect(shape(1)).toBe(JSON.stringify([
      [0, 1500, 425], [1040.89, 651.79, 253.93], [1244.55, 473.16, 150.54],
      [1204.95, 544.4, 163.39], [1193.59, 512.14, 205.8]]))
  })
})
