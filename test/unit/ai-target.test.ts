 ...
 * 900 m 內。
 *
 * 【三個條件都不能少】只看「敵機首朝不朝我」而不管距離、不管有沒有預瞄
 * 解、不管機首在不在射擊錐內的話，三公里外一架剛好朝我飛的敵機，看起來
 * 跟貼著我開火的一樣危險。實測換上後半球目標的當下，`threatInstant`
 ...
 * —— 這一項是方向相反的取捨（後方追逐變好、A→B→A 變差），預設由負責人
...
describe('visionFactor —— 視野折扣', () => {
   ...
   * A→B→A 變差六場六場 —— 兩個都是實際回報過的事，孰輕孰重是負責人的
