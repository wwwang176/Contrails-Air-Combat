...
describe('防護力', () => {
  /**
   * 【0.60～1.5 是訂好的區間】超出要負責人重新定值，不是實作者自己放寬。
   ...
   * 沒標是**實質判斷不是樣板漏印**。
   *
   * 【下限放到 0.60 只為了零戰】其他機種最低的是 P-51D 的 `fuselage` 0.80
   * （機腹散熱器一發就報銷），離 0.90 那條舊下限還有 0.10 的餘裕 —— 放寬
   * **只為了讓一台史實上毫無防護的飛機能照實寫**，不是為了讓誰變綠。
   */
  ...
  /**
   * 【血量正比於質量】取代 spec §6.3 的「戰鬥機一律 1000」。
