 ...
 */
/**
 * 【每一台都要掃】只掃三台戰鬥機的話，轟炸機的命中盒就**沒有被驗過
 * 覆蓋率** —— 實測 He 111 有 412 個頂點、B-17G 有 6,012 個落在所有盒之外
 * （最大的兩塊是尾砲塔 2,528 點與上部砲塔＋背脊 2,214 點）。那些地方打得到
 * 但不扣血，而且不會有任何症狀。「有些機種不掃」本身就是缺陷的溫床。
 */
 ...
 *
 * 【為什麼一定要把它納進這份掃描】照整機包圍盒目測切的命中盒沒有跑過
 * 覆蓋率 —— F6F-5 的尾段腹部有 256 個頂點落在六個盒之外（打不到），
...
describe('AircraftSpec 的新欄位', () => {
  /**
   * 【這一條搬走了】原本斷言「戰鬥機一律 1000（spec §6.3）」。
   * 現在是**血量正比於質量**（P-51D 1000 / F6F-5 1270 /
