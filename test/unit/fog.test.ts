...
/**
 * 【這一組守的是需求本身，不是實作細節】要的是「遠方可以考慮
 * FOG，但是要看得出地平線」，加上「海面要比天空深、海面近到遠
 * 幾乎沒有顏色變化」。
 *
 * 【地平線由**海色**與天空色的差（0.454）撐起來，海面完全退出全域霧】
 * 改用**霧色**與天空色的差不行（遠海化進霧色、霧色比天空暗一階）：海面
 * 自己近到遠就變了 0.109，比地平線那一階（0.092）還大 —— 那條線讀起來
 * 只是一整片洗白裡的一段。所以這一組的主角是 `SEA_COLOR` 加上兩個材質的
 * `fog` 旗標，不是 `FOG_COLOR`。
 *
 * 【比的是 `skyColorAt(0)` 不是 `SKY_HORIZON`】天空著色器的
 * `t = dirY × 0.5 + 0.5`，地平線（`dirY = 0`）落在漸層的**正中間**；
 * `SKY_HORIZON` 只出現在正下方、被海擋著，畫面上永遠不會出現。拿它當對象
 * 的話，霧色比天空**亮**也照樣綠 —— 而那一飛就看得出來。本組最後一條就是
 * 為了這件事而設。
 */
describe('地平線要看得出來', () => {
  /**
   * 【地平線由海色與天空色的差撐起來，不由霧色】要求的原文
   ...
   *
   * 【0.25 是怎麼來的】實測階差 **0.454**（天空 0.495、海 0.041）。霧化後的
   * 遠海只有 0.092（遠海 0.169 對天空 0.261），那正是「接縫太怪」的成因。
   * 門檻取 0.25：留浮動空間，而且遠高於那個會出事的 0.092。
   */
  ...
  /**
   * 【天空整體要夠亮】地平線 0.261、天頂 0.115 那個量級太暗；提亮後
   * 是 0.431 與 0.277；再單獨提下半，成為 **0.495 與 0.277**。
   *
   * 【天頂的下限就是這條在守】「頂部不動」是一條要求，而
   ...
   * `clamp` 那兩行是字串，測不到）。指數是 uniform 而不是字面值，
  ...
  it('天空球的 uniform 用的是同三個常數', () => {
    ...
    // 【指數也要走 uniform】指數若寫成 FRAG 字串裡的字面值 0.65，就與
    // SKY_GRADIENT_POWER 成了兩份獨立的常數 —— 改一份忘另一份，畫面與霧色
    // 就分家而沒有東西會紅。
    expect(mat.uniforms.power!.value).toBe(SKY_GRADIENT_POWER)
  })

  /**
   * 【霧色必須跟著天空走】關係是「相等」：霧色就是地平線上的那個天空色。
   *
  ...
  /**
   * 【補】天空球是自寫的 `ShaderMaterial`，three **不會**替它做
 ...
 *
 * 半邊 250 km 的話，上帝視角上限（12,000 m）時那條邊在幾何地平線以下
 * 2.751°，1080p / 65° 下是 40.7 px，而且**隨高度往下跑**。海面不吃霧，
 * 它就是 L 0.060 對 L 0.431 的一道硬階，沒有東西會糊掉它。
 * 半邊 3,000 km 只剩 0.229°（3.4 px）。
 */
describe('地平線接近幾何地平線', () => {
