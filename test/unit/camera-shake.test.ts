 ...
 * 補不了**：超出點陣的部分會被畫布裁掉。
 *
 * 【敵我標示與目標框也在那一張】它們是用已經套上震動的相機投影的，本來就
 * 貼著飛機晃；再吃 `#hud` 的變換就震兩次，框與飛機脫開。
 *
 * 【為什麼用掃原始碼】這是「畫面上多了一道亮邊」「框跑離飛機」，在 node 環境
...
describe('滿版的遮罩與敵我標示不跟著震', () => {
  ...
  it('不震的清單是暗角、黑視與三種標示，而且走另一個 context', () => {
    expect(hud.replace(/\s+/g, ' ')).toContain(
      `const UNSHAKEN: readonly HudWidget[] = [ ${unshaken.map((w) => `'${w}'`).join(', ')}, ]`)
    expect(hud).toContain('WIDGET_DRAW[w](UNSHAKEN.includes(w) ? maskCtx : ctx, L, f, dt)')
  })
  ...
  /**
   * 【順序要靠排序維持】不震的那一張疊在 `#hud` 底下，所以清單裡凡是走那一張的
  ...
  it('每一份清單裡，不震的都排在會震的前面', async () => {
    ...
    for (const [god, bombing] of [[false, false], [false, true], [true, false]] as const) {
      ...
      const last = list.reduce((k, w, i) => (unshaken.includes(w) ? i : k), -1)
      const first = list.findIndex((w) => !unshaken.includes(w))
