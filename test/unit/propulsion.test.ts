...
describe('enginePower', () => {
  ...
  /**
   * 【守的是「換檔的谷 + 高檔的峰」，不是「兩個峰」】數峰的個數等於要求
  ...
  it('Bf 109 K-4 海平面 WEP 接近 2,000 PS', () => {
    // 【1,475 → 2,000】機種由 G-6（DB 605A）換成 K-4
   ...
   * 【斷言的對象是保留率，不是絕對功率】
   *
   * 「P-51 在 8,000 m 的**絕對功率**高於 Bf 109」對 K-4 不成立 —— 不是因為
   * 增壓器，是因為 K-4 海平面就多了 32% 的功率：
   *
   * ```
   *              海平面      8,000 m     保留率
   ...
   * ```
   *
   * 【0.02 是怎麼來的】P-51D 的保留率 85.4%、K-4 82.6%，差 2.8 點。海平面
   * 功率若填成 1,490 hp（61″Hg 的軍用值）會低估分母，保留率被灌成 91.3%
   * —— 門檻訂在 0.05 就是照那個假數字訂的。
   *
   * 雙級增壓器的高空優勢在**保留率**上，而那一項 P-51D 贏。斷言保留率也讓
   * 這一條對日後的動力調參免疫 —— 它問的本來就是「增壓器好不好」，不是
   * 「誰的引擎大」。
   */
