...
describe('World 的組裝', () => {
  it('add 回傳的 Combatant 帶滿血與遞增索引', () => {
    ...
    // 【對各自的 spec.hp，不要寫死 1000】血量正比於質量，
...
describe('一步的順序（spec §4.2）', () => {
  it('彈丸推進在飛機之後 —— 判定用的是這一步的新位置', () => {
    ...
    const before = t.hp
    w.projectiles.spawn(0, 4000, 56, 0, 0, -887, 6, 1, 0, PROJECTILE_LIFETIME, 12.7)   // owner 1 = 不存在的射手
    w.step(0.05)
...
describe('命中事件（M7 spec §2.2）', () => {
  ...
  it('打中飛機時推一筆事件，命中點落在紅機附近', () => {
    const { w, r } = duel()
    w.projectiles.spawn(0, 4000, -280, 0, 0, -887, 10, 0, 0, PROJECTILE_LIFETIME, 12.7)
  ...
  it('沒打中就沒有事件', () => {
    const { w } = duel()
    w.projectiles.spawn(500, 4000, -280, 0, 0, -887, 10, 0, 0, PROJECTILE_LIFETIME, 12.7)
    w.projectiles.step(HIT_DT)
    w.resolveHits()
  ...
  it('一步之內多發命中就有多筆', () => {
    const { w } = duel()
    for (let i = 0; i < 3; i++) w.projectiles.spawn(0, 4000, -280, 0, 0, -887, 10, 0, 0, PROJECTILE_LIFETIME, 12.7)
    w.projectiles.step(HIT_DT)
    w.resolveHits()
  ...
  it('緩衝滿了不會越界，dropped 會計數', () => {
    ...
    for (let i = 0; i < w.hitEvents.capacity + 5; i++) {
      w.projectiles.spawn(0, 4000, -280, 0, 0, -887, 1, 0, 0, PROJECTILE_LIFETIME, 12.7)
    }
    w.projectiles.step(HIT_DT)
    w.resolveHits()
...
describe('入海回收與水柱事件（M7 spec §4）', () => {
  ...
  it('線段跨過水面時推一筆水柱事件，交點內插正確', () => {
    ...
    const i = w.projectiles.spawn(100, 10, 200, 20, -20, 40, 10, 0, 0, PROJECTILE_LIFETIME, 12.7)
  ...
  it('水柱的法線朝上 —— 它就是「法線朝上的撞擊」', () => {
    const w = empty()
    w.projectiles.spawn(0, 10, 0, 0, -20, 0, 10, 0, 0, PROJECTILE_LIFETIME, 12.7)
    w.projectiles.step(1)
    w.resolveHits()
    const d = w.splashEvents.data
  ...
  it('一發彈丸只噴一根柱子', () => {
    ...
    const w = empty()
    w.projectiles.spawn(0, 1, 0, 0, -100, 0, 10, 0, 0, PROJECTILE_LIFETIME, 12.7)
    for (let n = 0; n < 5; n++) {
  ...
  it('低於 SEA_KILL_Y 才回收，不是一入水就回收', () => {
    const w = empty()
    const i = w.projectiles.spawn(0, 1, 0, 0, -100, 0, 10, 0, 0, PROJECTILE_LIFETIME, 12.7)
