...
describe('曳光彈池', () => {
  ...
  it('有東西動時一定上傳', () => {
    ...
    for (let i = 0; i < 3; i++) { p.spawn(0, 0, 0, 0, 0, -887, 6, 0, 0, PROJECTILE_LIFETIME, 12.7); p.kill(i) }
    p.spawn(10, 20, 30, 0, 0, -887, 6, 0, 0, PROJECTILE_LIFETIME, 12.7)
    p.step(0.02)
    t.update(p)
  ...
  it('死掉的那一格會被歸零', () => {
    ...
    const p = new Projectiles(64)
    p.spawn(10, 20, 30, 0, 0, -887, 6, 0, 0, PROJECTILE_LIFETIME, 12.7)
    p.spawn(40, 50, 60, 0, 0, -887, 6, 0, 0, PROJECTILE_LIFETIME, 12.7)
    p.step(0.02)
    t.update(p)
  ...
  it('走一段腳本之後，畫得出來的那一批永遠恰好是活著的那一批', () => {
    ...
    const script: (() => void)[] = [
      ...
      () => { p.spawn(5, 0, 0, 0, 0, -887, 6, 1, 0, PROJECTILE_LIFETIME, 12.7) },
      () => { p.kill(0) },
      () => { p.spawn(9, 0, 0, 0, 0, -887, 6, 0, 0, PROJECTILE_LIFETIME, 12.7) },
      () => { p.kill(2) },
      () => { p.kill(1) },
      () => { p.spawn(3, 0, 0, 0, 0, -887, 6, 1, 0, PROJECTILE_LIFETIME, 12.7) },
    ]
