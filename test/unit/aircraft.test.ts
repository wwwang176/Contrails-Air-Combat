 ...
 */
/**
 * 「維持轉彎」場景的滑鼠速率。**必須略高於飛機轉得動的速率** —— 整條測試
 * 靠這個前提：滑鼠一直跑在飛機前面，誤差角於是維持不墜，飛機被迫持續轉彎。
 *
 * 【0.008 → 0.012】不是判準放寬，是**場景參數跟著飛機能力走**。
 * 那天拿掉了指揮儀的飛行員過載硬夾（`control/limiters.ts`，6.5 G → 結構
 * 極限 8 G），飛機因此追得上原本的滑鼠速率，誤差角由 11.1° 塌到 1.56°，
 * 整個場景退化成「機首咬住瞄準點」——要測的東西消失了。
 * 0.012 讓滑鼠重新跑在前面（誤差角回到 11° 上下），場景恢復原意。
 */
const TURN_DX = 0.012
...
/**
 * 【世界固定瞄準點的核心行為】（見 task-20-report.md §13）
 *
...
describe('Aircraft：世界固定瞄準點會收斂（沒有滾轉跑步機）', () => {
  ...
  it.each([
    ...
    // 明確要求的行為。單點門檻與「必須逐點下降」在設計上就與它衝突：
