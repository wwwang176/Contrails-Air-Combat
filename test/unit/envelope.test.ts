...
describe('bestSustainedTurnRate', () => {
  ...
  /**
   * 【兩台真實的轉彎關係：K-4 全面領先，只有一格例外，而那一格有名字】
   *
   * ```
   *   高度      Δ = P-51 − K-4          高度       Δ
   *       0 m   −0.0130  ← 超過死區      4,000 m   −0.0094
   *   1,000 m   −0.0023                  6,000 m   −0.0096
   *   2,000 m   **+0.0106**              8,000 m   −0.0084
   *                                     10,000 m   −0.0100
   * ```
   *
   * **2,000 m 是唯一 P-51D 轉贏的一格，而且它是增壓器的產物，不是機翼的。**
   * P-51D 的二段增壓在 1,900 m 換檔，低檔的臨界高度隆起剛好落在 2,000 m 的
   * 取樣點上，把它抬過 K-4。往上一格（4,000 m）優勢就回到 K-4 手上。
   * 領先幅度 0.61°/s，也就是說這不是「P-51 在中空是纏鬥機」——它是取樣點
   * 壓在換檔隆起上的結果。
   *
   * 【與 AI 的關係】`DEFAULT_RULES.turnEnter` 的死區是 0.02 rad/s。七格裡
   * 只有海平面那一格（−0.0130）接近而未達死區，其餘全部在死區之內 —— 也就是
   * 說**這條規則對這兩台目前等於休眠**，P-51D 不會因為轉不贏而判定 extend。
   * 2026-08-25 的 K-4 換裝曾經讓六格超過死區（最大 −0.0379），2026-08-27
   * 的 P-51D 質量修正把差距收了回來。要留意的是這件事會再翻 —— 動任一台的
   * 質量、出力或 oswald 都會動這張表。
   *
   * 【所以這條斷言守什麼】
   *   1. K-4 的領先幅度有上界（不准再擴大到讓 `turnEnter` 醒過來）。
   *   2. P-51D 只准在 2,000 m 那一格反超，而且幅度有上界。
   * 方向在別的高度翻掉，或 2,000 m 的隆起長大，都會紅。
   */
  it('K-4 的最佳持續轉彎率領先 P-51，唯一例外是 1,900 m 換檔隆起', () => {
    /** P-51D 低檔臨界高度 1,900 m 的隆起剛好落在這個取樣點上 */
    const GEAR_BUMP_ALT = 2000
    for (const alt of ALTS) {
      const d = bestSustainedTurnRate(P51D, alt) - bestSustainedTurnRate(BF109K4, alt)
      if (alt === GEAR_BUMP_ALT) {
        expect(d).toBeGreaterThan(0)
        expect(d).toBeLessThan(0.02) // 不得長到吵醒 turnEnter 的死區
      } else {
