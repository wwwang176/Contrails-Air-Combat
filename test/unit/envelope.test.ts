  ...
  it('等於全速度範圍的最大值（對照 0.25 m/s 步長的暴力掃描）', () => {
    for (const spec of [P51D, BF109K4]) {
      for (const alt of ALTS) {
        ...
        // 【要雙邊容差，不能寫成單邊「不得低於暴力解」】
        //
        // `fast >= brute - 1e-9` 要求細化解**永遠**不低於 0.25 m/s 網格的
        // 暴力解，而那不是 `searchBestTurn` 保證得了的性質：它跑 12 次黃金
        // 分割，收斂殘差約 1e-5 rad/s（該函數的註解自己就寫了這個數字），
        // 而暴力解本身也是從下方逼近的近似值。兩個近似誤差誰大誰小是碰
        // 運氣 —— 0.25 m/s 的網格夠粗時單邊寫法碰巧成立，網格加密到
        // 0.05 m/s 之後 P-51D 在 8,000 m 是 −4.6e-6（K-4 在海平面 −9.1e-6）。
        //
        // 兩個方向各有各的容差，因為兩個誤差來源本來就不同量級：
