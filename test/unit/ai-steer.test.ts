...
describe('失速的兩種診斷', () => {
  ...
  /**
   * 【`unload` 不換掉瞄準方位】`unloadAim(self, 0)` 會把瞄準點整個搬到自身
   * 速度向量上，人工驗收的症狀是「右彎時瞬間抖一下」：
   *
...
describe('steerCommand', () => {
  ...
  /**
   * 【超前時不收油門】加上 `throttle = THROTTLE_FLOOR` + `brake = 1` 的話，
   * 三個手段（後置、高 yo-yo、減速）會同時消耗能量。實測的
   ...
   *
   * 所以只留幾何手段（瞄準點的後置與高 yo-yo），能量交給既有的角落速度
   ...
   * **97.8% 的鎖定來自不是它目標的敵機**。`defendAim` 若吃對當前目標建的
   * `EngageBasis`，破的就是錯的人。
   */
  ...
  /**
   * 【退化備援：不能變成「指著攻擊者」】升力向量平行視線時直接回傳視線的話
   * 破防就變成零，而那個幾何在轉彎中並不罕見（對方咬在我的轉彎平面內
   * 時就會發生）。
   *
  ...
  /**
   * 【speedRecover 與 unload 不同】它主動壓機頭。兩者不能合成同一個 mode
   * 共用 `unloadAim(self, 0)` —— 那對「拉太猛」正確，對「沒空速」是無操作
   * （spec §5.1）。
   */
...
describe('速度判準沒有仰角前提', () => {
  ...
  /**
   * 【不准有仰角前提】要求「目標仰角 > 45° **或**自己航跡角 > 45°」才觸發
   * 的話，同一空層平飛追擊時速度掉到 1G 失速速度的一半也一次都不動。實測
   * 74° 仰角、速度裕度 1.49 時仍然不動，等到 1.34 才觸發 —— 已經 78 m/s 了。
   *
  ...
  it('目標不在高仰角、自己也沒在爬升時，速度不足照樣觸發', () => {
    ...
    evaluateGeometry(self, target, sit)
    buildEngageBasis(self, target, basis)
    // 前提：目標仰角與自身航跡角都接近 0 —— 帶仰角前提的閘門在這裡是死的
...
describe('extend 的俯仰是連續量', () => {
  ...
  /**
   * 【俯仰角對高度必須連續】寫成
   * `(energyReserve < 0 || y < 1000) ? +25° : −sign(ΔE) × 25°` 的話是兩個
...
describe('steerCommand：拉桿紀律', () => {
   ...
   * 方位讀成滾轉需求。違反這條時滾轉指令由 2–3° 暴增到 27–29°、副翼打到
   * 滿舵。
   *
...
describe('steerCommand：追不上就改為佈局', () => {
  ...
  /** 【問題窗的 53.2%】閘關上時 `approach` 是純預矄追擊、零旋鈕。 */
