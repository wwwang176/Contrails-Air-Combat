...
describe('pitchRateLimit', () => {
  ...
  // 【不要拿 qMax(70) 去比 qMax(160)*0.6】qMax(V) 是單峰函數，V=70 與 V=160
  // 分屬峰值兩側，比較它們代表不了「低速能量不足」。下方三則才是：直接釘住
  // 「低速時限制來源是氣動且遠低於結構極限」「高速時限制來源是飛行員且氣動
  // 遠超過飛行員上限」「qMax 峰值落在轉折速度附近，與 envelope.cornerSpeed
  // 有可解釋的偏移」。
  ...
  it('高速時限制來源是結構，且氣動可用過載遠超過結構極限', () => {
    ...
    //
    // 【之前這條測的是 'pilot'】當時 6.5 G 的飛行員硬夾比結構
