 ...
 * ── ⚠ 追瞄負載的敵機一定要擺成包圍 ─────────────────────────
 *
 * 直接用 `PURSUIT` 起始的話敵機全部在同一側 —— 暖機之後只有 **114 / 160
 * 座**取得目標，`top` 與 `tail` 是 **0 座**，門檻完全沒有涵蓋合法的最壞
 * 情形。`bench/turret-load.ts` 把敵機擺成包圍，實測 **160/160**（八種
 * 砲塔各 20 座）。
 *
 ...
 * ```
 *
 * 【不要把差距全記在 SSR 頭上】用 `vite-node` 跑同一支探針（同樣走 SSR）
 * 可以把「vite 的 SSR transform 讓跨模組呼叫難以內聯」這件事單獨隔離出來：
 * **SSR 只解釋到 1.9×**（770 / 397），而 vitest 是 4.5×（1774 / 397）。
 *
 ...
 * 形狀的 `World` 負載之後的 JIT 狀態** —— 實測「vitest 只跑追瞄那一條」
