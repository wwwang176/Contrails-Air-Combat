 ...
 *
 * 【為什麼不是每台都守五項】兩台戰鬥機守滿五項（係數
 * 全部掃過）。兩台轟炸機的史實校準比較晚，還有四項落在
 * 5.6%–9.5%，超過 ±5% 但不到需要重做模型的程度。
 *
 * **沒有為了讓它們變綠而放寬門檻** —— 護欄重新定值是負責人的決定。
...
/**
 * 兩台轟炸機**還沒守住**的四項，連同 實測偏差。
 *
 * 【He 111 的升限是刻意讓掉的】要的是「5,000 m 極速與海平面
 * 爬升都提高，可以超過史實」。那兩項各自的旋鈕（`powerCritical` 與
 ...
 *                                 襟翼**的值，淨形對不上是應該的
 *   B-17G   升限          −9.5%   質量取 22,000（19,017 與 24,500
...
const PENDING: readonly { id: string; check: Check; reason: string }[] = [
  { id: 'he111', check: 'ceiling', reason: '+8.9%。取捨：高空性能優先於升限的絕對值' },
  ...
  { id: 'b17g', check: 'ceiling', reason: '−9.5%。質量取 22,000（中間值）；19,017 時是 −0.1%' },
...
const CASES: {
   ...
   * 第 11 頁寫著「Performance — None obtained」）。所以
   * 改用日方實測的平均，詳見 `specs/ki84.ts` 的檔頭。
   *
...
describe('L2 史實性能（極速／失速／升限／爬升率 ±5%，並另有比值斷言）', () => {
   ...
   *
   * 【由 `checks` 算出「誰沒守」，再要求 `PENDING` 逐項對得上】**不可以用
   * 兩份手維護的清單**（一個字串陣列、一個寫死的預期值）—— 加一台新機種
   * 時兩邊都要記得改，漏一邊才會紅。從 `CASES` 推導的話漏填會直接指出是
   * 哪一台的哪一項。
   */
