 ...
 * 轟炸機不徵召。
 *
 * 【為什麼要有這一條】boom and zoom 是戰鬥機的打法。轟炸機轉不贏攔截機是
 * 常態 —— B-17G 對 P-51D 的機體迴旋差值是 −0.045～−0.074，遠低於
 * `turnEnter`（−0.02），所以只看那個閂鎖的話**整隊轟炸機都會被徵召**。
 *
 * 【壞掉會怎樣】轟炸機離開航線去爬升蓄能，於是它不在攔截機預期的位置上。
 * 實測 `turrets.test.ts` 的「P-51 也打下了東西」由正數變成 **0** —— 那條
 * 測試在講砲塔不是無敵的，卻被一個 AI 打法的改動弄紅，成因完全看不出來。
 */
describe('轟炸機不徵召', () => {
  let s: Sample
  beforeAll(() => { s = measure('b17g', 'p51d', 4) }, 600_000)

  it('戰術層一次都不啟動', () => {
    expect(s.build, '轟炸機不得進入蓄能相位').toBe(0)
    expect(s.dives, '轟炸機不得進入攻擊相位').toBe(0)
  })
})
