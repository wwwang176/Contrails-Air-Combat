...
describe('地形進得了場', () => {
  ...
  /**
   * ── 【停用：錨島 450 m 對 600 m 開打的仗不構成障礙】──────────
   *
   * 實測 180 秒 20v20：離地最低 304 m，地形層一次都沒介入（touched 0/40）。
   * 山高減半之後，「甲板」這個高度在群島上已經沒有山擋路。
   *
   * 【重啟條件】甲板高度（`ALTITUDES[0]`）降到錨島峰高減 150 m 以下，或錨島
   * 改高。哪一邊動是設計決定；動了就把 `.skip` 拿掉重跑。
   *
   * 【它不在的期間誰在守】只剩下面的「沒有人生在山裡」。「撞山只能是來不及
   * 的那一種」在這個場景裡是空過 —— 它逐一檢查撞山的架次，而撞山是 0。
   * 地形感知的機制本身由 `terrain-avoidance` 的飛行掃描守。
   */
  it.skip('甲板高度：山真的擋得住路', () => {
   ...
   *
   * 把固定餘裕拉高可以讓這一格歸零，但那是全域 AI 常數：所有空戰與對地
   * 的飛行軌跡都會跟著變，對戰矩陣、六場機動、命令通道全部要重錄。
   * 那個代價不付。
   *
   * 【所以判準換成兩條，都說得出理由】
   *
   ...
   * —— 島最高 500 m。可以選 600 之後，出生在山裡就變成**開局直接墜機**，
   * 而畫面上看起來像是隨機的白畫面。
   */
  it('甲板高度：沒有人生在山裡', () => {
 ...
 * 這張圖在真實的仗裡長什麼樣子。
 *
 * 【為什麼與群島分開】丘陵的峰高上限是 120 m，而群島的錨島是 450 m ——
