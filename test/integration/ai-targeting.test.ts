...
const LIMITS = {
   ...
   *
   * 【2026-08-11：1.5 → 1.35，專案負責人裁定接受】拿掉飛行員過載硬夾之後
   * 實測 1.40 s（原 1.5）。AI 現在在破防時拉到 8 G、掉速更快，交戰的節奏
   * 因此更碎，換目標的頻率跟著上升。專案負責人裁定「AI 拉爆自己這件事
   * 本身合理，接受」。
   *
   * 【1.35 而不是 1.40】留 0.05 s 的餘裕給後續無關改動的擾動；再低就該
   * 重新檢視而不是繼續調 —— 這個量是 target-churn 那一輪工作的產出，
   * 它掉下去代表那一輪的成果在流失。
   */
  holdMedian: 1.35,
  rearShare: 0.35,
