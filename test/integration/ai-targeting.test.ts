...
const LIMITS = {
   ...
   */
  // 【2026-08-27：1.5 → 1.2】實測 1.40 s。這個數字是**下游**的：K-4 換裝
  // （803c1b8）把中軸砲換成 MK 108，TTK 由 0.53 s 壓到 0.28 s，目標死得更快，
  // 持有時間自然跟著短。它守的性質（AI 不在目標之間猶豫）仍然成立 —— 20v20
  // 裡中位數持有 1.40 s 仍是實在的承諾，而 1.2 抓得到崩到 0.5 s 那種真正的
  // 猶豫。餘裕 14%，比原本 1.5 對 1.90 的 27% 薄，**因為它現在綁在武器上**：
  // 火力一調就要重新推導，不要只把它再往下挪一次。
  holdMedian: 1.2,
  rearShare: 0.35,
  fireShare: 0.025,
  onNose: 0.12,
