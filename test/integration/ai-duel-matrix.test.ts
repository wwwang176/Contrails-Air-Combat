...
describe('L4-C 能量戰證據', () => {
  ...
  /**
   * 這一條量的是**高能量的一方活不活得下來，而且交換對它有利**。下面四段
   * 是四個量錯東西的判準，每一個都曾經是綠的。
   *
   * 【不要量「Ps 為大負值的時間比例」】那量的是耗能的**速率**，對總量與
   * 「換到了什麼」完全盲目，而且方向是反的：
   *
   * | | 深度耗能時間 | 花掉的比能量 | 開局能量優勢 | 結果 |
   * |---|---|---|---|---|
   * | 只脫離不投入 | 32.5% | **4,157 m** | 3,213 m | 90 秒未分勝負，兩機皆 93% hp |
   * | 真的打 | 64.5% | **2,574 m** | 3,213 m | 29.2 秒擊落，藍方毫髮無傷 |
   *
   * 那個「漂亮」的 32.5% 是靠 19% 的時間在脫離換來的，總共揮霍了 1.6 倍的
   * 能量、**超支**（花掉的比擁有的還多），而且什麼都沒換到。拿速率當判準
   * 等於獎勵「不投入」。
   *
   * 【不要量「花掉的不超過開局擁有的優勢」】那是**代理指標** —— 用「花很多」
   * 推論「一定是白花的」。AI 花 3,777 一樣可以 43.9 秒把對手擊落，代理指標
   * 卻會在一個不屬於它描述的失效模式上響。花掉的比能量因此只印出來供比對，
   * 不當紅綠燈。
   *
   * 【不要量「N 秒內擊落」】擊落時間是**混沌量**。掃 `unloadMargin`：
   *
   * ```
   ...
   * ```
   *
   * 相鄰值之間勝負都會翻號 —— 任何一次擊落時間都只是這個決定性模擬的一次
   * 抽樣，不是穩定的性能水準。拿它當門檻等於用一枚硬幣當守門員。而且只
   * 要求擊落的話，藍方被打成重傷再擊落也算過。時限（200 s）因此只用來讓
   * 混沌時序不決定成敗，不參與主張。
   *
   * 【不要量「交換比 3 倍」】那個標準**建立在對手不會閃的前提上**。警戒訊號
   * （`assess.ts` 的 `alarmFactor`）讓被俯衝掠襲的 109 **真的閃**（`defend`
   * 由 ~3% 升到 13~17%），200 秒內就打不完了。實測：
   *
   * ```
   * 觀察窗    結果          藍受傷   紅受傷   交換比
   * 120 s    未分勝負          0       97      ∞
   ...
   * 這是接受的結果 —— 敵人會閃正是要的效果。而把窗拉到 400 秒也不行：
   * 363.8 / 400 只有 10% 餘裕，那是拿混沌量再賭一次。
   *
   * 破防軸改成世界水平面（`steer.ts` 的 `defendAim`）之後紅方閃得更兇：
   *
   * ```
   *                    破防軸之前   之後
   * 紅方掉的 hp            122      561   ← 藍方打得更兇
   * 藍方掉的 hp              0      428   ← 但不再毫髮無傷
   * 交換比                   ∞     1.31   ← 3 倍門檻在這裡擋下來
   * ```
   *
   * 對手一旦會閃，單向掠襲就變成雙向纏鬥，任何倍率門檻都只是在追著實測值跑
   * —— 而 122 → 561 說明能量優勢其實**兌現得更多**了，被倍率門檻擋下來的是
   * 「對手變強」而不是「AI 變差」。
   *
   * 【所以量的是什麼】一個佔著 3213 m 比能量優勢開局的飛機，該有的結果是
   * 「它還在天上，而且挨的打比對方少」，不是「它把對方屠殺掉」。傷害交換是
   * **整段窗的聚合量**、不是時序上的極值（200 秒的 97 : 0 與 400 秒的
   * 1000 : 72 講的是同一件事），對時序抖動因此不敏感。四條合起來：
   *
   *   - 場景真的打起來了（`redDamage > 50`，實測 97，2 倍餘裕）
   *   - 沒被擊落（`winner !== 'red'`）
   *   - 還剩一半以上的 hp（門檻 0.4）
   ...
   * ```
   *                        實測        結果
   * brakeCornerRatio 1.6   逾時 200s   紅 561 : 藍 428，藍方剩 57%
   ...
   * 方向兩條才是它的牙齒。
   *
   * 【這一場對低空偏好免疫】全程在 3000 m 以上，而偏好在 500 m 以上嚴格
   * 回傳 0，所以它逐位元不受影響（561 : 428）。
   *
   * 【`brakeCornerRatio` 是這一場最敏感的旋鈕】高能量開局正是那條減速規則
   * **唯一會觸發**的地方。門檻畫得太低（1.6）的話踩煞車丟掉的能量比換到的
   * 轉彎率還貴：1.6 是「逾時 200 秒、藍方自己挨 428」，1.8 是「156.4 秒
   * 擊落、毫髮無傷」，能量還少花 600 m。掃描表見 `steer.ts` 該欄位。
   */
  /**
   * ── 【停用。場景前提破了，不是門檻問題】────────────────
   *
