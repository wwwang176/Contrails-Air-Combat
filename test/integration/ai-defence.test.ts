...
describe('被夾擊時的閃躲（1 藍 2 紅、30 秒）', () => {
  for (const c of CASES) {
    it(`${c.name}`, () => {
      ...
      // 【二：破防要有效果】**目前效果不好，這個門檻是寬鬆的下限。**
      // 實測 81.0 / 55.8 / 56.0 / 38.0%（修補前 92.7 / 96.3 / 88.3 / 39.2%）
      // —— 有改善但還遠不夠。原因是現在的破防只是「由視線轉開 75°」，
      // 不看對方的轉彎平面，很可能是**同平面**的，對方只要維持追擊就跟得住。
      // 異平面破防是下一批的主項，屆時這個門檻要收緊。
