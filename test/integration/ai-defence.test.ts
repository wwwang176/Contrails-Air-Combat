 ...
 *
 * 【為什麼不是寫死 150 HP】單發傷害一律 ×3 時，這個場景的行為**一格都
 * 沒變** —— 四種幾何的 `defendShare`、反應秒數、`huntedShare` 全部逐位元
 * 相同（30 s 內沒有人死，傷害只是被累加），只有掉血量精確地變成三倍：
 * 40.2 / 68.4 / 0 / 108.0 → 120.6 / 205.2 / 0 / 324.0。寫死 150 的話會紅
 * 兩組，而紅的原因與閃躲品質無關 —— 是尺的單位被換掉了。
 *
 * 寫成「紅方開火 0.3125 s 的量」，這條線就與單發傷害脫鉤：480 DPS 下是
 * 150、1440 DPS 下是 450，四組的餘裕以秒為單位不動。實測
 * 0.120 / 0.324 / 0 / 0 s。
 *
 * 【0.3125 是怎麼來的】150 HP ÷ 480 DPS。150 這條線承接的是回填值
 * （實測 103 / 126 / 0 / 25），沒有重新挑過。
 */
 ...
 * 轉去對付他。這正是乙案（閃躲只改動作、不直接改目標）想要的效果
