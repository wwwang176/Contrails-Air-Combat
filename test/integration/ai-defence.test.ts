 ...
 * 四組的餘裕以秒為單位一字未動。2026-08-27 實測 0.120 / 0.324 / 0 / 0 s。
 *
...
const CASES: { name: string; behind: Vector3; budgetSeconds?: number }[] = [
  { name: '正後方 400 m', behind: new Vector3(0, 0, 400) },
  {
    ...
    /**
     * 【2026-08-27：這一格由 0.053 s 退化到 0.324 s，改用局部例外 0.35】
     *
 ...
 *
 * 一、**正後方 400 m 的 `budgetSeconds: 0.55`**（2026-08-11 開）。拿掉飛行員
 * 過載硬夾之後這一格由 0.084 s 惡化到 0.518 s，負責人裁定「AI 拉爆自己這件事
 * 本身合理，接受」。2026-08-27 的 P-51D 質量校準之後實測 0.120 s，回到共用
 * 預算之內，例外因此收回。
 *
 * 二、**後上方 400 m 的 `pursuitOverwhelmed`**（2026-08-25 開）。
 * `CHASE_FLOOR_BUFFER` 由 100 放寬到 300 之後，上方的攻擊者不再被自己的閂鎖
 * 從獵物尾巴上扯下來，實測 defend 佔時 42% → 92%、掉血 0 → 1000（陣亡），
 * 於是停用射擊解佔比與掉血預算兩條斷言。2026-08-27 實測掉血 0.0、
