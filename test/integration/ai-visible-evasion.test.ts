...
interface Result {
   ...
   * 「最後一刻被護欄硬拉起」。這裡直接問同步硬護欄是否曾介入；柔性政策若
...
function measure(standoff: number, aspect: Aspect = 'tail'): Result {
  ...
  for (let s = 0; s < SECONDS * 240; s++) {
    ...
    //
    // 所以只數**離地餘裕在 `clearanceScale` 以內**的撞地接管 —— 那正是柔性
    // 高度偏好的作用帶，也正是這條護欄要問的「政策失職了沒」。
...
describe('看得見的閃躲（三機、腳本射手、180 秒）', () => {
  ...
  /**
   * 【這一條是 #136 的主判準】目標是**飛機不以任何方式墜海**。目前的戰術層
   * 不再按絕對高度或 Worker 風險改寫瞄準線；真正跨過改出線才由安全層
   * 接管。這六個確定性場景仍應在不動用 `safety.ts` 120 m 同步硬護欄下通過。
   *
