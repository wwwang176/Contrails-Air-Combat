 ...
 * 2026-08-06 那份 spec 已經寫明的既知行為。這裡把地形場景的值記下來，
 * 要不要為它立護欄是專案負責人的決定。
 *
...
/**
 * 【方位只取兩個，不是八個】島近似圓對稱，繞著它換方位**不增加任何覆蓋**
 * —— 實測八個方位的結果逐位元相同（766 m／−463 m／57% 接管）。留兩個是
 * 為了讓「旋轉對稱真的成立」這件事本身仍然被驗到。
 *
 * 真正有資訊的維度是下面的 OFFSETS：航跡到島心的垂距。
 */
const BEARINGS = [0, 115]
