 ...
 *
 * 【由單場改成合併】原本只跑 `VICTIMS[0]`。血量改成正比於質量
...
describe('強制注入側翼（20v20、120 秒 × 九個受命分隊）', () => {
   ...
   *
   * 九場合併：403/654896 個取樣在開火，其中 391 是 `defend`、
...
describe('強制注入集火（20v20、120 秒）', () => {
  ...
  it('受命分隊鎖同一架的比例高於對照', () => {
    ...
    // 【0.30 的來歷】實測 0.476（四架的分隊約當兩架同時咬同一
    // 個），取約 1.6 倍餘裕。**這是護欄不是參數** —— 它由
    // 實測定值，不隨掃描移動
   ...
   *
   * 【三方對照，門檻是中點】`onRate > flankRate * 5` 那種寫法（
   * 實測 8.9 倍，取 5 倍）。第三份加上配額之後掉到 4.77 倍
 ...
 *
 * 【0.55 的來歷】實測：側翼 1736、對照 2764，比值 **0.628**。
 * 依這個實測值取 0.55，留約 13% 餘裕 —— 比第一份的傷害
...
describe('戰術的效果（20v20、開／關對照）', () => {
   ...
   *
   * 【門檻在跑之前先定死】`n ≥ 300` 且「比對照低 0.05 以上」。
   * 看到結果再定門檻等於量到綠為止 —— 這一條的目的正是
   * 要讓這條判準**有可能失敗**。
   */
  it('側翼讓開火時的方位角往後側方移動', () => {
    ...
    // 【n 是判準的一部分】取樣不足時「有沒有差」根本問不出來。300 與
    // 下面那個 0.05 都是在跑之前先定死的
