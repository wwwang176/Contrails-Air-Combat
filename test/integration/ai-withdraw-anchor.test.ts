 ...
 * ── 【它守的是什麼】────────────────────────────────────
 *
 * 試飛回報「Bf109 在戰場周圍一直上下繞，看起來也不像在累積
 * 能量」。實測**兩隊完全一樣**，不是機種問題。
 *
 ...
 * ── 【為什麼是 300 秒而不是 120】────────────────────────
 *
 * 那 22 組掃描的窗口是 120 秒，而外擴在接敵（約 60 秒）之後才開始
...
describe('撤退令不得把戰鬥推出戰場（20v20、300 秒）', () => {
   ...
   *
   * 【成因】與 `ai-duel-matrix` 同一個：`ab5587d`（K-4 換裝）把中軸砲換成
