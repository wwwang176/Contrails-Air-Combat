/**
 * A6M5 的 GLB 走一遍 `parseGlbTemplate`：材質對得上、槳葉節點找得到、
 * 各材質的頂點數、包圍盒（翼展／全長／高度）、轉軸位置、法線朝外。
 *   npx vite-node test/tools/a6m5-glb.probe.ts
 */
...
const buf = readFileSync(`public${A6M5_MODEL.url}`)
const bytes = buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength) as ArrayBuffer
const t = await parseGlbTemplate(bytes, A6M5_MODEL)
...
console.log('spinning mesh 數', spinning)
