...
── ${label} ─────────────────`)
  ...
  bake: async (_page, _probe, slice) => {
     ...
     * ── 上緣要**逐站解**，不能配一條直線 ──────────────────
     *
     * 上緣的定義是「平面追上艙頂」，也就是 `paneY(x, z) = t`：
     *
     * ```
     *   x = (PANE_A + PANE_C·z − t) / PANE_B
     * ```
     *
     * 前兩版都是拿量到的兩點配一條直線（先是 `−3.07 + 0.813x`，後是
     * `−2.95 + 0.60x`）。直線在兩個錨點上對，**中間就跑掉**：
     *
     * ```
     *   機體z    直線版      解出來的     差
     *   -2.90    x 0.083     x 0.062     0.02
     *   -2.68    x 0.450     x 0.290     0.16   ← 最大
     *   -2.54    x 0.683     x 0.451     0.23
     * ```
     *
     * 上緣跑到外面，玻璃就提早結束、艙頂提早接手，兩邊在交界處差 0.13 ——
     * 人工回報的「玻璃頂部跟艙頂沒有銜接好」就是這個。解出來的版本讓
     * `paneY(xLo) = t` **恆等成立**，交界處是零。
     *
     * 【`t` 是量到的中線背線】所以上緣自動跟著艙頂走，艙頂抬多少它跟多少。
     *
     * 【超過 `BOX_FLAT` 就是風擋結束了】艙頂由 0.45 起有冠（`BOX_CROWN`），
     * 掉得比平面快，兩者不再相交 —— 那一站已經在風擋之後。實測解出來的 x
     * 在機體 −2.54 是 0.451、−2.45 是 0.558，正好對上參考模型（−2.43 的
     * x0.55 量到 1.892，就是艙頂的高度，不是風擋）。
     */
    /**
     * 【上緣要留一道上框，不能貼齊艙頂】訂成「平面剛好等於艙頂」的話玻璃太寬。
     * 逐條 X 帶比參考模型量到的玻璃 z 範圍：
     *
     * ```
     *   x0.05  −3.23…−3.03  寬 0.20        我的（貼齊）0.34
     *   x0.35  −3.03…−2.68  寬 0.35        我的 0.40
     *   x0.65  −2.88…−2.53  寬 0.35        我的 0.47
     * ```
     *
     * 真機的玻璃在碰到艙頂之前就收了，上面留一道上框。留 `PANE_LIP = 0.05`
     * 之後是 0.28 / 0.35 / 0.42，對得上。上框只有 0.05 —— 一站 0.09 的話是
     * 29° 的面，不是牆（前一版留 0.16，那才是人工看到的垂直斷面）。
     */
    const PANE_LIP = 0.05
    const paneLoAt = (zb: number, t: number): number =>
      (PANE_A + PANE_C * zb - t + PANE_LIP) / PANE_B
    ...
    const ringAt = (zm: number): [number, number][] => {
      ...
      if (bw > 0) {
         ...
         */
        const xLo = Math.min(PANE_X - 0.03, Math.max(0.02, paneLoAt(zb, t)))
       ...
       */
      const eLo = paneLoAt(zb, t)
      const eHi = Math.min(paneHi(zb), PANE_X)
      // `eLo > BOX_FLAT`：艙頂的冠掉得比平面快，兩者不再相交＝風擋已經結束
      if (eLo <= BOX_FLAT + 0.02) {
        for (let i = 0; i < half; i++) {
          const x = pts[i]![0]!
