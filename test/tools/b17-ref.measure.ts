...
── ${label} ─────────────────`)
  ...
  bake: async (_page, _probe, slice) => {
    ...
    const blendAt = (zb: number): number => {
      ...
      return Math.min(
        smooth(Math.min(1, (zb - PANE_Z0) / 0.12)),
        smooth(Math.min(1, (PANE_Z1 - zb) / 0.10)))
    }
    ...
    const ringAt = (zm: number): [number, number][] => {
      ...
      if (span && bw > 0) {
        /**
         * 【下界為 0 的那幾站，索引 1 改坐在四邊形**內部**】前段（機體
         * −3.276…−3.061）玻璃一路吃到中線，上緣不在 x ≥ 0 上。索引 1 這時
         * 坐哪都行 —— 只要在區間內就仍然共面 —— 取 0.06 讓 (0,1) 那一格不
         * 退化。`lo` 長到 0.06 的那一站兩個分支剛好接上，沒有跳。
         */
        const xLo = Math.max(span[0], Math.min(0.06, span[1] * 0.35))
        const xHi = Math.max(span[1], xLo + 0.03)
        /**
      ...
      if (span) {
        for (let i = 0; i < half; i++) {
          const x = pts[i]![0]!
          if (x < span[0] - 1e-9 || x > span[1] + 1e-9) continue
          pts[i]![1] = paneY(x, zb)
        }
        /**
