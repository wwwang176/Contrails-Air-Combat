...
const stages: Record<string, Stage> = {
   ...
   *
   * 【不要用機身背線估】後機身背線本身就往上收——那是真的外形不是姿態，
   * 拿它算會估成 7~9°。機翼比機身可靠得多。
   */
  pitch: async (_page, _probe, slice) => {
    /**
     * 【窗口要避開發動機艙】`wing` 那一格量到內艙佔 X 2.40…3.80、外艙佔
     * 5.80…7.20（艙的厚度是機翼的兩倍，中厚線會被整個抬上去）。像
     * 「內段 [1.6, 3.2]」那樣壓到內艙的話，殘差 RMS 0.067 是乾淨窗口的 2.4 倍
     * —— **殘差自己會告訴你窗口髒了**，不必猜。
     *
  ...
  tail: async (_page, _probe, slice) => {
     ...
     * 把 `uWindow` 當成濾 Z 用的話，[7.0, 17.0] 於是濾成「只留 |x| 7…17」
   ...
   *
   * 【為什麼不用射線】把射線原點放進艙裡（座艙罩那一招）不行 —— 艙與機翼
...
── ${label} ─────────────────`)
   ...
   * ══ 第二版：三條錨點曲線的量法全部換掉 ═══════════════════
   *
   * 三條線各有一個**取樣幾何自己失效**的坑，三個都不會報錯、三個
   ...
   * 偏窄，機體 z 8.9 實測差 **−0.12**。只在 `rear` 那一段換到 y = 0.90 不夠：
   * 門檻放在量測 8.60 ＝ 機體 9.72 已經太晚。
   *
  ...
  bake: async (_page, _probe, slice) => {
    ...
    /**
     * 【腹線也不用射線】「四個原點的朝下射線取最低」是錯的 —— `castRay` 取的
     ...
     * ── 風擋：先定平面，再讓 poly 遷就它 ──────────────────
     *
     * 玻璃是兩個斜的正方形 —— 先決定玻璃位置，再讓
     * 機身的 poly 跟著變形。這一段就是那個做法。
     *
     ...
     *    **漸變**的（`blendAt`），不能在進出風擋那一站突然切換 —— 讓索引
     *    跟著最近的自然位置走的話，索引 2 的 x 在相鄰兩站之間由 0.48
     *    跳到 0.02，loft 整個扭掉。
     *
     ...
     * ── 後段的錐度重配────────────────────────
     *
     * 把「甲板一路收到零」交給 `boxWAt` 的外層錐度不行（機體 3.10 → 4.30
    ...
    const fuseAt = (k: number, nn: number): number => {
       ...
       *
       * 沒有這個閘門的話，機首那幾站的半寬只有 0.4～0.9、`sideC` 整條落在
     ...
     * 尾錐：**逐站量測烘頂點，不要等比縮最後一環。**
     *
     *       「量到機體 13.97 之後整圈打不到」是假的，五環會憑一條曲線收
     *       到 15.30 的 0.10 倍，中心再以 0.16/m 抬。做出來是一根 1.4 m 就
     *       收成一點的尖錐；真機那裡是**幾乎不收的尾樑 + 一顆圓鈍的尾砲塔**。
     *       機尾是厚的，但長度不夠 —— 真機那裡是幾乎不收的尾樑，不是
     *       一路收到尖的錐。
     *
     ...
     * 運算，這台是整段打不到。（把角度比對寫成「theta 當度數」而 theta
   ...
   *
   * 只比四行的結果（半寬 0.006、腹線 0.001、主翼前緣 0.045、垂尾前緣
  ...
  align: async (page, probe, slice) => {
     ...
     *
     * 【窗口要蓋滿】只開到 Z=9 而機身到 16.8 的話——後半段整個沒
