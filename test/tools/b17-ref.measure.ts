...
── ${label} ─────────────────`)
  ...
  bake: async (_page, _probe, slice) => {
    ...
    for (let pass = 0; pass < 2; pass++) {
      expo = fill(rs.planes.map((_, k) => expoAt(k, halfW[k]!, top[k]! - AXIS_V)))
     ...
     * 整段低 0.17～0.20。
     *
     * 做法：那一段的上半圈，`x ≤ w(z)` 的點一律拉到頂點的高度，再由平頂邊緣
     * 以**固定斜率的一條直線**掉下去，掉到低於超橢圓就收手 —— 那個交點就是
     * 艙緣的稜。兩端的 `w` 都收到 0 就自然過渡，不必另外做混合。
     *
     * ── 為什麼是直線不是平滑步進（第二版）─────────────
     *
     * 第一版用 smoothstep 把平頂併回超橢圓。S 曲線**兩端都相切**，所以平頂邊緣
     * 沒有稜、尖端也是圓的：實測 Y = 1.90 的俯視輪廓，x 0.00 與 x 0.15 兩條帶
     * 的前緣只差 **0.001 m**，等於一個平頭。人工回報要的是「俯視是一個尖銳的
     * 向前夾角，玻璃是平的」。
     *
     * 直線的斜率是量到的。第一版取 1.00（那是平頂邊緣**附近**的斜率），結果
     * 斜面一路撐到 x 0.82 都還沒碰到機身，整個上半圈被吹起來 —— 機體
     * z −2.53 那一站的 i2 比前一站跳 +0.143、i4 跳 +0.144，算圖上是一片
     * 鋸齒。斜率要取「平頂邊緣**到機身**」那一整段：參考在 z −2.68 是由
     * (0.30, 1.957) 掉到 (0.78, 1.24)，**1.49**。取 `FALL = 1.50`。
     *
     * 後端的收尾也拉長了。原本 0.40 m（兩站）由滿寬收到 0，一站掉 0.150／
     * 0.189，側視是一道崖；改成 1.20 m。
     *
     * ── 第二個來源：另一台 lowpoly 的 B-17 ────────────────
     *
     * `Thunder/textures/B17.json` 是 SketchUp 出的 `B17.dae`，整個座艙罩只有
     * 八個點，但把結構講得很乾淨（已換算到機體座標，它的翼展 1244.9 in =
     * 31.62 m 與真機一致，發動機艙橫向 2.97 / 6.53 也對得上我們的 3.05 /
     * 6.57，所以那份的比例可信）：
     *
     * ```
     *   風擋   中線前端 ( 0    , 1.941, −2.718)   ← 脊的前尖
     *          中線後端 ( 0    , 2.171, −2.245)   ← 脊的後端
     *          上緣外角 (±0.471, 2.172, −1.888)
     *          下緣外角 (±0.678, 1.704, −2.160)   ← 艙緣
     *   罩身   平頂 y 2.215、半寬 0.540 一路往後
     * ```
     *
     * 兩條緣線與機身軸的夾角是 **50.1° 與 52.8°**；我們的參考 GLB 量到
     * **49.1°**。兩個獨立來源對得上，所以那個角度是真的。第一版是 63°。
     *
     * ── 【尚未處理】為什麼俯視的前半段還是圓的 ─────────────
     *
     * 現在量到的俯視夾角是 **47.2°**（參考 50.6°，改之前是 63° 而且尖端是
     * 平的）。但那個角只有**後半段**是真的稜 —— 機體 −3.06 與 −2.88 兩站的
     * 斜面**完全沒有生效**，那裡仍然是超橢圓自己的圓頂。
     *
     * 根因是結構性的，不是參數沒調好：
     *
     * ① 超橢圓 `|x/a|^n + |(y−cy)/b|^n = 1` 在頂點的斜率**恆為 0**（只要
     *    n > 1）。它做不出「脊」，任何 n 都不行。
     * ② `bUp` 取的是量到的中線最高點，而在座艙那一段那個點**就是座艙脊**。
     *    於是整個上半圈被撐到脊的高度：機體 −3.06 的 x 0.22 超橢圓給 1.789，
     *    參考只有 ~1.66。
     * ③ 斜面程式碼只能 `max`（往上加），砍不下去 —— 一算就低於那個被撐平的
     *    超橢圓，立刻收手。
     *
     * 逐條 X 帶量參考的機身（`extent`，NOWIRE），機體 −3.43…+2.82：
     *
     * ```
     *   機體Z    x0.00  x0.30  x0.60  x0.80  x0.95
     *   -3.43    1.519  1.480  1.340  1.192  1.023
     *   -2.93    1.917  1.695  1.427  1.263  1.041
     *   -2.68    1.957  1.934  1.693  1.254  1.068
     *   -0.18    1.996  2.129  1.881  1.279  1.108
     *    1.07    2.000  1.974  1.863  1.317  1.165
     * ```
     *
     * `x0.80` **整條機身都是 1.19…1.35**，而中線由 1.52 升到 2.00 —— 這是
     * 一個「平頂 + 硬肩」的剖面，不是超橢圓：z −2.68 由 x 0.30 到 0.60 的
     * 斜率是 0.80，0.60 到 0.80 卻是 2.20。單一 n 兩段只能對一段（n = 1.6
     * 對得上 x 0.80、n = 2.0 對得上 x 0.30／0.60）。
     *
     * 要修得讓剖面產生器長出「肩」這個自由度（每站兩個參數：肩的位置與
     * 落差），那是重烘不是調參數，**交專案負責人決定要不要做**。
     */
    const WSHIELD = [-4.22, -1.40] as const    // 量測座標，機體 −3.10…−0.28
    const WSHIELD_FULL = [-3.60, -2.60] as const   // 機體 −2.48…−1.48
    const DECK_W = 0.45                        // 平頂的半寬
    const FALL = 1.50                          // 平頂邊緣往外掉的斜率
    ...
    const ringOf = (k: number): [number, number][] => {
      ...
      if (w > 0) {
        ...
        for (let i = 1; i < pts.length / 2; i++) {
          const deck = Math.min(y0, y0 - FALL * (pts[i]![0]! - w))
          if (deck <= pts[i]![1]!) break         // 掉到機身之下：這裡就是艙緣的稜
          pts[i]![1] = deck
        }
      }
      return pts
     ...
     * 尾錐：量到 12.85（機體 13.97）之後整圈打不到，那之後是尾砲塔。最後一個
     *       乾淨環是 1.6 × 1.5，直接收在那裡會露一個洞。五環按同一條曲線縮，
     *       中心以 0.16/m 繼續抬 —— 那個斜率取自腹線後段量到的趨勢。
     */
    const shrink = (
    ...
    for (const [z, f] of [
      ...
      console.log(line(z, shrink(last, f, 0.16 * (z - lastZ))))
    }
