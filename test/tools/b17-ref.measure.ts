...
── ${label} ─────────────────`)
  ...
  bake: async (_page, _probe, slice) => {
     ...
     *
     * 【上緣第一版早了 0.12，那就是「玻璃到側窗頂部中間有一個垂直斷面」】
     * 我當初把「玻璃在每條 X 帶上最後一次讀到 y ≈ 1.80」當成上緣，但那只是
     * 我停止讀表的地方 —— 同一條帶再往後還有玻璃（x0.25 在機體 −2.78 是
     * 1.918、−2.68 是 1.942）。真正的上緣是**平面追上甲板**的地方。
     *
     * 對照量到的中線背線與平面在 x = 0 的值：
     *
     * ```
     *   機體zb   背線 t   平面 4.8235 + 0.985·zb
     *   -3.20    1.671    1.6715   ← 一樣
     *   -3.02    1.847    1.8483   ← 一樣
     *   -2.90    1.917    1.9665   ← 開始分岔
     * ```
     *
     * **中線的背線在 −2.95 之前就是平面本身** —— 風擋一路延伸到中線。上緣是
     * x=0 收在 y 1.92（z −2.95）、x=0.65 收在 y 1.783（z −2.56），也就是
     * `z = −2.95 + 0.60·x`。訂成 `−3.07 + 0.813·x` 的話中線上早了 0.12 m，
     * 那一段憑空變成甲板，平面接不上去 —— 算圖上就是一道垂直的牆。
     */
    const paneLo = (zb: number): number => (zb + 2.95) / 0.600
