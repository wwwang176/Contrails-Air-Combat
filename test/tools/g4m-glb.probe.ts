/**
 * G4M 的 GLB 走一遍 `parseGlbTemplate`：材質對得上、兩具槳葉節點找得到、
 * 各材質的頂點數、包圍盒（翼展／全長／高度）、法線朝外（帶符號體積為正）。
 *   npx vite-node test/tools/g4m-glb.probe.ts
 */
...
const buf = readFileSync(`public${G4M_MODEL.url}`)
const bytes = buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength) as ArrayBuffer
const t = await parseGlbTemplate(bytes, G4M_MODEL)
