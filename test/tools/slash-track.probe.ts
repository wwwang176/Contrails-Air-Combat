 ...
 *
 * 【它在回答什麼】人工回報（2026-08-23）：真人飛行員在敵機由下方穿越時，
 ...
 * 也爆表，依這個量收手會把掠襲一起打壞。
 *
 * 【為什麼一定要先量這個】上一輪（迴轉平面紀律）就是沒有先在掠襲場景驗證
 * 判準，結果 `ai-duel-matrix` 的 1v1 由「42 秒擊落」變成「200 秒互不接觸」。
 *
 * 【場景】`ai-duel-matrix.test.ts` 的 `HIGH_ENERGY`，逐字照抄：
 * 藍方 P-51D 在 5500 m / 250 m/s，紅方 Bf 109G-6 在 4000 m / 170 m/s，
 * 相距 800 m 對頭。零延遲、零瞄準誤差、無亂數。
 *
