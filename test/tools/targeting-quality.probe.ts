 ...
 *
 * ## `rearShare` 的門檻為什麼是 0.35 而不是 0.30
 *
 ...
 *
 * ## `rearShare` 由 0.35 放寬到 0.40 —— **同一個機制又發生一次**
 *
 ...
 * 「在 26.3~32.0 之間亂跳」的混沌超標，是訊號。而它是
...
const LIMITS = {
   ...
   *
   * 【曾放寬到 1.35，同日改回】拿掉飛行員過載硬夾之後實測掉到
   ...
   */
  /**
   * 【1.2 → 1.0，理由是這條線壓在自己的雜訊帶裡】
   *
   * 1.2 是 K-4 換裝（ab5587d）那一輪定的：MK 108 讓 TTK 由 0.53 s 壓到
