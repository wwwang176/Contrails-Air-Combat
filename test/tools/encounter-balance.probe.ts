 ...
 *
 * 【為什麼要這一支】`ai-withdraw-anchor.test.ts`（單一種子）在開啟新閂鎖後
 * 量到戰損比由 **1.005 變成 4.893**、存活由 15:17 變成 5:18。那是一個嚴重
 * 到必須決定要不要退回的訊號。
 *
 * 但同一輪的護送關消融（兩張卡 × 五次微擾）量到的是**己方戰鬥機存活變好**
 * （配對中位數 +42 與 +119 機秒）。兩個結果衝突。
 *
 * 單一種子答不了這個問題 —— `2026-08-22-extend-recovery-design.md` §2.5 已經
 * 證明這個系統對初始條件極度敏感。這一支用五次微擾重量。
 *
 * 【量什麼】只量三件事，其餘不管：
 *
 * ```
 *   存活數        兩隊各自，300 秒結束時
 *   累計傷害      兩隊各自受到的
 *   最大半徑      ai-withdraw-anchor 的全域護欄用的那個量
 * ```
 *
