 ...
 * ## 判準
 *
 * 【不要量「速度補回來**而且**回到戰場要幾秒」】兩者在這個場景恆等：飛機
 * **3~5 秒**就把 `cornerRatio` 由 0.75 補到 0.95，而且只跑了 400 m，1 km 的
 * 回家判準從頭到尾都成立，量到的永遠是補速度那一段。
 *
 * 而 `extend` 持續很久 —— 它的解除條件不是速度，是**相對**能量閂鎖
