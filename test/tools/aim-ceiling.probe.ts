 ...
 *
 * 【它要回答什麼】主判準門檻是 `AIM_ERROR_FLOOR = 2°`，
 * 現況是 5.42~7.42°。問題是：2 度是最高了嗎，還有辦法更高嗎？
 *
 * 那是兩個問題，這支只答得出第二個的一半。
 *
 * ## 一、門檻能訂多高 —— **4°（= 2 × HIT_CONE）**，理由在下面
 *
 * 拿現況最小值 5.42° 當門檻是照著現況畫靶（spec §7.5 的第三次
 * 教訓）。補了兩張掃描，答案才有依據。
 *
 ...
 *
 * 【問題】不改程式碼的話，`AIM_ERROR_FLOOR` 從 2° 可以改成
