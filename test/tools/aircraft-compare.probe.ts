...
// ── 重量與尺寸 ────────────────────────────────────────
add('空重（模型質量）', P51D.mass, BF109K4.mass, 'kg', 0, false)
add('翼面積', P51D.wing.area, BF109K4.wing.area, 'm²', 2, true)
add('翼展', P51D.wing.span, BF109K4.wing.span, 'm', 2, true)
add('翼負荷', P51D.mass / P51D.wing.area, BF109K4.mass / BF109K4.wing.area, 'kg/m²', 1, false)

// ── 直線性能 ──────────────────────────────────────────
add('海平面極速', maxLevelSpeed(P51D, 0) * MS_KMH, maxLevelSpeed(BF109K4, 0) * MS_KMH, 'km/h', 0, true)
add(`${ALT} m 極速`, maxLevelSpeed(P51D, ALT) * MS_KMH, maxLevelSpeed(BF109K4, ALT) * MS_KMH, 'km/h', 0, true)
add('7,000 m 極速', maxLevelSpeed(P51D, 7000) * MS_KMH, maxLevelSpeed(BF109K4, 7000) * MS_KMH, 'km/h', 0, true)
add('海平面爬升率', maxClimbRate(P51D, 0).rate * 60, maxClimbRate(BF109K4, 0).rate * 60, 'm/min', 0, true)
add(`${ALT} m 爬升率`, maxClimbRate(P51D, ALT).rate * 60, maxClimbRate(BF109K4, ALT).rate * 60, 'm/min', 0, true)
add('實用升限', serviceCeiling(P51D), serviceCeiling(BF109K4), 'm', 0, true)

// ── 機動 ──────────────────────────────────────────────
add('失速速度（海平面 1 G）', stallSpeed(P51D, 0, 1) * MS_KMH, stallSpeed(BF109K4, 0, 1) * MS_KMH, 'km/h', 0, false)
add(`角速（${ALT} m）`, cornerSpeed(P51D, ALT) * MS_KMH, cornerSpeed(BF109K4, ALT) * MS_KMH, 'km/h', 0, false)
add(`瞬時迴旋率（${ALT} m、角速）`, turnAtCorner(P51D), turnAtCorner(BF109K4), '°/s', 1, true)
add(`持續迴旋率（${ALT} m）`, bestSustainedTurnRate(P51D, ALT) * RAD, bestSustainedTurnRate(BF109K4, ALT) * RAD, '°/s', 2, true)
add('滾轉率 @ 400 km/h', maxRollRate(P51D, ALT, 400 / MS_KMH) * RAD, maxRollRate(BF109K4, ALT, 400 / MS_KMH) * RAD, '°/s', 0, true)
add('滾轉率 @ 650 km/h', maxRollRate(P51D, ALT, 650 / MS_KMH) * RAD, maxRollRate(BF109K4, ALT, 650 / MS_KMH) * RAD, '°/s', 0, true)
add('結構 G 限（正）', P51D.limits.gPositive, BF109K4.limits.gPositive, 'G', 1, true)
add('不可超越速度 VNE', P51D.limits.vne * MS_KMH, BF109K4.limits.vne * MS_KMH, 'km/h', 0, true)
add('零升阻力係數 cd0', P51D.drag.cd0, BF109K4.drag.cd0, '', 4, false)

// ── 火力 ──────────────────────────────────────────────
add('火力 DPS', batteryDps(P51D.battery), batteryDps(BF109K4.battery), '傷害/s', 1, true)
add('槍口初速（瞄準用那挺）', P51D.battery.sight.muzzleVelocity, BF109K4.battery.sight.muzzleVelocity, 'm/s', 0, true)
add('匯聚距離（不可比，見下）', P51D.battery.convergence, BF109K4.battery.convergence, 'm', 0, null)
add('命中盒體積合計', boxVolume(P51D), boxVolume(BF109K4), 'm³', 1, false)
...
const W = 30
console.log(`=== ${P51D.name} vs ${BF109K4.name}（模型實算，交戰高度 ${ALT} m）===\n`)
