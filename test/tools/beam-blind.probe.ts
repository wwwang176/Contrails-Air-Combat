 ...
 *
 * 【場景擺錯的話，量出來的結論會自洽而且全錯】`beam` 那一格要量的是
 * 「敵人離我 800 m **在前方**往左橫飛」。寫成「觀測儀在敵機**正右方**、
 * 兩機**同向並排**」的話那不是橫越，是編隊：相對速度為零、視線角度永遠
 * 不變，**幾何本身就是死局**，預測誤差會量到 0.70°（幾乎貼著地板），
 * 足以支撐一套「破防軸與航向平行、AI 破一次就穩住」的假說。真正的橫越
 * 是 5.50°，與正後方的 5.85° 幾乎一樣 —— 側面沒有盲區。
 *
 * 上下也很容易顛倒：`high`/`low` 若寫成「觀測儀在敵機上方」，那是「敵人
 * 在我下方」，方向剛好反過來。
 *
 * **幾何是判準的一部分。** 預測誤差、有效窗長、正常模式佔比這三個指標
 * 互相印證不代表結論成立 —— 它們會一起印證同一個錯誤前提。
 *
