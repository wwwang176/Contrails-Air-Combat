 ...
 *   npx vite-node test/tools/glb-vertex-diff.probe.ts
 * 報：兩邊頂點數、GLB 裡在程式版找不到同位置（1e-5）的頂點數與最遠距離。
 * 槳盤（載入時重做的 CircleGeometry）與隱藏物件都排除。
 *
 * **只剩 He 111 比得下去** —— 其餘機種的 GLB 都不是程式版匯出來的。
 */
...
const build = buildHe111
const def = HE111_MODEL
...
console.log(`he111: 程式版 ${a.length} 頂點、GLB ${b.length} 頂點；`
