 ...
 * ```
 *
 * 兩者相除就是**每一發子彈換到多少傷害**。原本預期這是三案分得開的地方，
 * 實測不是 —— 三個值的勝負、平手數與戰鬥長度一格都沒動，每秒傷害只差 11%：
 *
 * ```
 *   fireDelay          勝負(ACE:VET)  平手  平均長度   扣扳機   傷害  每秒
 *   0.3（＝瞄準，甲）     11 : 1       12    238.2 s   96.1 s   2497  26.0
 *   0.1（乙）             11 : 1       12    238.2 s   96.0 s   2760  28.7
 *   0（丙）               11 : 1       12    238.2 s   96.0 s   2766  28.8
 * ```
 *
 * 【為什麼差這麼少】扣扳機的**總時間**幾乎相同，差的只是哪些 0.3 秒窗口。
 * 而且 AI 不會刻意做假動作 —— 扳機延遲真正的差別（玩家橫滾拉開的瞬間 AI
 * 會不會跟著停火）這支量尺踩不到。它能證明的只有「改這個不會把 AI 的空戰
 * 能力弄壞」。**判準是試飛。**
 *
