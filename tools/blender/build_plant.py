...
【避讓】十二座構件與廠內道路上不擺佈景 —— 佈景沒有命中盒，疊上去會看到
炸彈穿過管架在構件上爆。手動加東西時自己避開（`KEEPOUTS` 有座標）。
"""
import bpy, math, os
...
# 8.8 cm 高砲位（相對廠區中心）。沙包圍一圈。
# **與 src/world/leuna.ts 的 FLAK_SITES 同一份座標** —— 內 8、中 16、外 24
FLAK_SITES = [(0, -2700), (1900, -1900), (2700, 0), (1900, 1900),
              (0, 2700), (-1900, 1900), (-2700, 0), (-1900, -1900),
              (750, -3850), (2150, -3250), (3250, -2150), (3850, -750),
              (3850, 750), (3250, 2150), (2150, 3250), (750, 3850),
              (-750, 3850), (-2150, 3250), (-3250, 2150), (-3850, 750),
              (-3850, -750), (-3250, -2150), (-2150, -3250), (-750, -3850),
              (350, -5100), (1650, -4850), (2850, -4250), (3850, -3350),
              (4550, -2250), (5000, -1000), (5100, 350), (4850, 1650),
              (4250, 2850), (3350, 3850), (2250, 4550), (1000, 5000),
              (-350, 5100), (-1650, 4850), (-2850, 4250), (-3850, 3350),
              (-4550, 2250), (-5000, 1000), (-5100, -350), (-4850, -1650),
              (-4250, -2850), (-3350, -3850), (-2250, -4550), (-1000, -5000)]
...
def _wall_run(i, salt):
    ...
    【只往外推不往內縮】往內會切過街廓裡的設備，而避讓表只認可炸構件與
    道路 —— 牆穿過一排儲槽不會有任何一條護欄變紅。
    """
...
def build_flak(b):
    """砲位的掩體。**廠區局部座標，跟著廠區轉** —— 砲位是廠區的防空陣地。

    【一座一個方塊】從投彈高度看，一座砲位只有 6 px；圍一圈小盒子的話一座要
    144 個三角形，而畫面上仍然只是地上一個方塊。真的高砲陣地從空中看本來就是
    一圈方形土堤。
    """
    n = 0
    sand = fixed_mat('LP_PlantSand')
    for fx, fz in FLAK_SITES:
        add_box(b, 'flak_berm', sand, fx, -fz, 0.6, 13.0, 13.0, 1.2)
        n += 1
    return n
